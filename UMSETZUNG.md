...
| Zeitbasis-Bug (PRIO HOCH): Medienzeit = Grain-Index auf TAI, 8 Ursachen gefixt (Nachtrag 271); Switcher-Pfad live bestätigt, NEUE WHEP-Video-Regression root-caused (Nachtrag 272) — ZWISCHENSTAND | teilweise | Nutzerauftrag "fix timebase bug". Harness+Switcher-Pfad (headless-Handy-Kamera über echtes WebRTC → Switcher → Sonde) live sauber: 0 Standbilder, 0 Index-Lücken. NEU GEFUNDEN (noch NICHT gefixt): WHEP-Monitor empfängt im neuen Zeitbasis-Modus 0 Video-RTP (Audio unbetroffen) — Race in `MxlVideoInput::new()` (`sync_state_with_parent()` vor der externen Verlinkung durch den Aufrufer) live per GST_DEBUG bestätigt, betrifft potenziell auch omp-switcher/omp-video-mixer-me/omp-audio-mixer (noch ungeprüft). `OMP_MXL_TIMEBASE=arrival` als Rückweg (bringt die 253-256-Bugs zurück). OFFEN: Fix in `omp-mediaio` + alle Hot-Swap-Aufrufer erneut live verifizieren, danach echter Handy-Test. | 2026-09-23 |
