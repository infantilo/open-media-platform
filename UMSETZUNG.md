...
| Kapitel 15 Teil 1 (Workflow-Auflösungs-Setting) | teilweise | Orchestrator/UI-Infrastruktur vollständig (`Definition.Settings`, `Launcher.Start`-extraEnv, Workflow-Formular); von den betroffenen Nodes nur `omp-source` umgesetzt (live bis zur IS-04-Flow-Registrierung verifiziert: 960×540 statt 640×480) — `omp-switcher`/`omp-player`/`omp-video-mixer-me` brauchen denselben Handgriff als Folgearbeit, `omp-ograf` bewusst ausgenommen (Template-Auflösung) | 2026-07-17 |
