...
| `omp-viewer`: dynamische Audio-Eingänge (Pegelanzeigen) + `omp-audio-monitor` komplett redesignt (PCM/AudioWorklet statt MP3/`<audio>`) | teilweise (Rest-Bug offen) | Nutzerwunsch "Pegelanzeigen im Viewer, dynamische Eingangszahl (mxf-player-Gruppen routbar), Audio-Monitor ohne HTML-Player, Gruppenwahl-Dropdown". `omp-node-sdk::NodeHandle::add_receiver`/`remove_receiver` neu (erste dynamische Receiver-Fähigkeit in OMP, `Device::version`-Refresh-Bug live gefunden+behoben), `omp_mediaio::levels` aus `omp-audio-mixer` gehoben (zweiter Verbraucher), `omp_mediaio::pcm_stream` neu (roh statt MP3). `omp-audio-monitor` (Discovery-Dropdown, `AudioWorklet`-Wiedergabe, `selectSource`) vollständig live verifiziert (echte PCM-Datenrate bestätigt). `omp-viewer`s Pegel-Datenfluss (neue, zweite `gst::Pipeline` pro Prozess) hat einen live gefundenen, NICHT gelösten Rest-Bug (MXL-Reader-Thread hängt dauerhaft in `futex_wait`, Root Cause trotz `GST_DEBUG`/`/proc`-Untersuchung nicht abschließend identifiziert, kein GDB im Container) — Receiver-Erzeugung/Discovery/IS-05-Verdrahtung selbst funktioniert, nur die tatsächlichen Meter-Werte nicht. Details docs/decisions.md 2026-08-06. | 2026-08-06 |
