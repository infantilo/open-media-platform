...
| `omp-mediaio::mxl`: fehlender Backoff bei `OutOfRangeTooLate`/`FLOW_INVALID`-Reopen behoben (CPU-Spin), Viewer-Freeze bei `keyer.setSource` bleibt offen | teilweise | Nutzerreport: Viewer friert ein, sobald am Mixer eine OGraf-Quelle als DSK gewählt und DSK aktiviert wird. Zwei echte Busy-Loop-Bugs gefunden+behoben (kein Raten): `Err(OutOfRangeTooLate)` und der `Ok(new_reader)`-Erfolgsfall der `FLOW_INVALID`-Wiedereröffnung hatten beide, anders als der benachbarte `OutOfRangeTooEarly`-Zweig, keinen Backoff — bestätigt per `mxl-info` (Read-Zeit eingefroren, Write-Zeit läuft), `GST_DEBUG=3`, und (da `gdb`/`ptrace` in der Sandbox gesperrt sind) `/proc/<pid>/task/*/stat`+`wchan` (100% CPU, `wchan=0`, reiner Userspace-Spin). Fix: 5ms-`thread::sleep`-Backoff in beiden Zweigen, `read_loop` UND `read_audio_loop`. **Zweiter, tieferer Fund beim Nachtesten:** nach diesem Fix keine CPU-Sättigung mehr, aber der Freeze bei `keyer.setSource` (löst vollen Mixer-Rebuild aus) bleibt bestehen — temporäre Instrumentierung zeigte >18.000 Wiedereröffnungsversuche ohne eine einzige erfolgreiche Lesung, `/proc/<pid>/fd` zeigte den Reader ausschließlich mit Deskriptoren auf `(deleted)`-Flow-Dateien (Schreiber legt sie beim `keyer.setSource`-Rebuild komplett neu an statt sie wiederzuverwenden, anders als beim bereits funktionierenden `SetInputs`-Fall aus Nachtrag 64); ein unabhängiger frischer Reader (`mxl-info`) las denselben Flow zur selben Zeit problemlos. Root Cause nicht bis in die vendorte MXL-C++-Bibliothek zurückverfolgt. Workaround bis zur Behebung: betroffenen `omp-viewer` nach einem `keyer.setSource`-Wechsel neu starten (frischer `MxlContext`), nicht nur neu laden. Nebenfund derselben Sitzung: `make stop` beendet nur den Orchestrator, nicht die gestarteten Node-Prozesse (eigene Memory-Erinnerung). `cargo test -p omp-mediaio --features mxl` (8/8), `cargo build --workspace --bins`, `cargo clippy` sauber. Details: `docs/decisions.md` Nachtrag 78 (+ Ergänzung) | 2026-07-22 |
