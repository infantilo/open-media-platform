...
| UX/UI-Zero-Training-Audit + Enterprise-Produktionsreife-Roadmap (Nutzerauftrag, parallel zur obigen Zeile) | teilweise | Zwei kombinierte Rollen (UX/UI-Experte + autonomer Lead-Developer/Co-Product-Owner), per Fork parallel bearbeitet. Rahmenbefund: "Zero-Training" ist für die meisten OMP-Oberflächen die falsche Latte (professionelles Broadcast-Vokabular ist für trainierte Operator:innen korrekt) — echte Relevanz liegt in Fehlertoleranz. Konkret behoben: drei destruktive Aktionen ohne/mit unpassender Bestätigung (Rundown-Item/Cart/Audio-Kanal entfernen, Stop→Schwarzbild) bekamen das bereits vorhandene `<omp-confirm>`-Kit-Element statt `window.confirm()`/gar keiner Bestätigung, live per CDP verifiziert. Roadmap: höchste Priorität ist fehlendes `defer recover()` um sämtliche Orchestrator-Hintergrund-Goroutinen (0 Treffer für `recover()` im ganzen Orchestrator — ein Panic in `launcher.supervise`/`workflows.runStart`/`failover.tryPromoteForInstance`/etc. reißt den GANZEN Prozess mit), zweiter Punkt fehlende Liveness-Überwachung interner Rust-Worker-Threads (exakt die Bug-Klasse aus der Zeile oben). Größere Vorschläge (Setup-Wizard, weitere Bundles prüfen, `recover()`-Wrapper selbst) bewusst nur dokumentiert, nicht blind umgesetzt — nächster Schritt offen. | 2026-08-07 |
