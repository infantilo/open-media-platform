...
| Kapitel 15 Teil 3 (Rest): `omp-switcher` liest nicht-selektierte Eingänge in Lowres | teilweise | Live-Hot-Swap der MXL-Quelle bei jeder Auswahländerung (`isel`-Pad-Block, neues Ziel vor dem Cut auf Highres hochgestuft, vorheriges danach auf Lowres heruntergestuft — PGM zeigt nie Lowres). Drei echte Bugs live gefunden+behoben: (1) Segfault durch Element-`set_state(Null)` vom eigenen Streaming-Thread aus (Deadlock) — Callback löst jetzt nur noch die Verlinkung, Element-Auf-/Abbau strikt auf dem Kontroll-Thread. (2) `MxlVideoInput` (omp-mediaio) legt intern vier Elemente an, exponierte aber nur `tail` — `Drop` entfernte die anderen drei nie aus der Pipeline (unschädlich beim Abbau der ganzen Pipeline, aber ein realer Leck bei chirurgischer Einzel-Entfernung); behoben mit demselben `pub elements: Vec<gst::Element>`-Muster, das `MxlAudioInput` (omp-audio-mixer, C11) bereits nutzt. (3) Ein fehlgeschlagener Swap ließ den Eingang zuvor dauerhaft aus der Buchführung verschwinden (weder abgebaut noch zurückgetragen) — Rückgabetyp gibt den unangetasteten alten Zweig im Fehlerfall jetzt explizit zurück. **Viertes Problem bleibt offen, bewusst dokumentiert:** ein echtes Speicherleck, das mit der Swap-*Anzahl* (nicht der Rate) skaliert, unter künstlichem Stresstest bis zum OOM-Kill (sauberer Auto-Neustart, kein Datenverlust) — drei gezielte Gegenmaßnahmen (Cleanup-Fix, Select-Kommando-Bündelung, Reader-Drain-Wartezeit) reduzierten es nicht messbar; ohne Heap-Profiling (kein `valgrind`/`heaptrack` in dieser Sandbox) nicht weiter eingrenzbar. Nutzerentscheidung (drei Optionen vorgelegt): vorerst so committen für normalen Sendebetrieb (Sekunden-/Minutentakt), nicht empfohlen für 24/7-Dauerbetrieb/Hochfrequenz-Automatisierung ohne Neustart. Live verifiziert: korrekte Highres/Lowres-Flow-Wahl gegen die echte NMOS-Registry gegengeprüft über mehrere Umschaltungen, kein PGM-Glitch per Konstruktion (Swaps berühren nur nicht-aktive Pads), kein Absturz bei realistischem Bedienertempo (0.5-1.5s, 100+ Umschaltungen). `cargo test`/`cargo clippy --workspace` grün. `omp-video-mixer-me` (fg/bg/DVE, komplexer) bleibt offen | 2026-07-20 |
...
| Kapitel 15 Teil 3 (Rest 2): OOM-Root-Cause gefunden + behoben (in omp-mediaio) | erledigt (Bibliotheks-Fix), Mixer-Feature weiterhin offen | Dedizierte Debug-Sitzung wie vom Projektinhaber verlangt (neue Herangehensweise, nicht derselbe Live-Test) — zwei Minimal-Reproduktionsversuche gebaut (`nodes/omp-mediaio/examples/mxl_*_repro.rs` für reine MXL-Reader-Hypothesen, alle widerlegt: weder sequentielles Reopen desselben Flows noch echte gleichzeitige/mehrfädige Reader auf demselben Flow lösen den Bug aus; `nodes/omp-video-mixer-me/examples/oom_repro.rs` für den GStreamer-Teil, außerhalb des vollen Mixer-Nodes). **Fund 1:** `MxlVideoInput`/`MxlAudioInput` (`omp-mediaio::mxl`) riefen `sync_state_with_parent()` nie für ihre eigenen vier intern angelegten Elemente auf — beim allerersten Pipeline-Aufbau unschädlich (kaskadierendes `set_state(Playing)` erfasst alle Kinder), bei jedem Hot-Swap in eine bereits laufende Pipeline blieben sie beobachtbar dauerhaft in `Null` hängen (per direkter `Element::state()`-Abfrage nachgewiesen: `appsrc`/`videoconvert`/`videoscale`/`videorate` blieben `Null`, während die vom Aufrufer selbst gebaute Zusatzkette korrekt `Playing` erreichte). **Fund 2** (unabhängig, per `GST_DEBUG=appsrc:5` bestätigt): `appsrc` hatte kein `leaky-type`/`max-buffers` gesetzt — `max-bytes` (Default 200000) löst ohne `block`/`leaky-type` nur ein ignoriertes Signal aus, keine echte Begrenzung; das GStreamer-eigene Debug-Log zeigte die interne Queue in einem beobachteten Fehlerfall unbegrenzt weiterwachsen ("queue filled", weit über die 200000-Byte-Grenze hinaus), während stromabwärts nichts mehr ankam. Beide Funde behoben in `omp-mediaio::mxl` (`sync_state_with_parent` + `Element::state()`-Wartephase; `leaky-type=upstream` + `max-buffers=5` auf dem `appsrc`) — gilt automatisch auch für `omp-switcher` (dieselbe Funktion, dessen bekannter langsamer Leck-Verdacht damit ebenfalls entschärft sein sollte, nicht gesondert nachgetestet diese Runde). **Live verifiziert, beide Ebenen:** der isolierte Reproduktionsversuch bleibt jetzt über 40 aufeinanderfolgende Swaps hinweg mit exakt konstantem RSS stabil (vorher unbegrenzt wachsend); der echte, volle `omp-video-mixer-me`-Node (uncommitted Hot-Swap-Code unverändert obenauf) hielt 20 reale `select`+`autoTrans`-Zyklen über die echte HTTP-API aus (fast 7× die ursprünglichen drei Bedienschritte, die zum OOM-Kill führten) mit einem RSS-Gesamtzuwachs von nur 80 KB (statt der zuvor dokumentierten +522 MB in einem einzigen Aufruf) — Prozess blieb durchgehend am Leben, keine Abstürze. **Weiterhin offen, unabhängig vom OOM:** ein separater, noch nicht root-gecauster Folgefehler — der Pad-Block-Mechanismus in `swap_input_resolution` läuft bei wiederholten Swaps auf demselben `isel`-Sink-Pad ab einem gewissen Punkt zuverlässig in einen Timeout (im echten Mixer-Log als "Timeout beim Warten auf den blockierten Pad-Unlink" sichtbar) — funktional bedeutet das, dass eine Auflösung nach dem ersten erfolgreichen Swap u. U. nicht mehr wechselt (alter Zweig bleibt unverändert bestehen), aber es entsteht **kein** Speicherverlust und **kein** Absturz mehr, da Fund 2s Fix jede weitere Wachstumsmöglichkeit unabhängig davon deckelt. Root Cause dieses Restfehlers bleibt für eine weitere Sitzung offen. Entscheidung über das weitere Vorgehen mit dem Mixer-eigenen Hot-Swap-Code (`main.rs`/`pipeline.rs`, weiterhin uncommitted) beim Projektinhaber erfragt | 2026-07-20 |
