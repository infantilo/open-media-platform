...
- **D8** Deterministische Ende-zu-Ende-Latenz & A/V/Daten-Synchronität
  (`ARCHITECTURE.md` §15, dort als „bisher größte fehlende Fähigkeit"
  eingeordnet — **noch nicht begonnen**, diese Gliederung ist reine
  Vorausplanung, Nutzerwunsch 2026-07-29: „Kompensation zumindest
  planen und in Arbeitsschritte teilen"). Live-Anlass: ein Test-Workflow
  `src(1080p50)→scaler→` (Video, 2 Hops) neben `audiomixer`-internem
  Testton (Audio, 1 Hop) zeigte an den jeweiligen Ausgängen ~±15 ms
  Drift (gelegentlich ~20 ms, knapp eine 1080p50-Framedauer) — **ohne
  jede aktive Kompensation**, da §15 bisher nur die Voraussetzung
  (Origin-Index-Erhalt, C13-Nachtrag 2, 2026-07-12) liefert, nicht den
  ...
  **D8 Teil 3 (Delay-Zuweisung entlang zu kurzer Pfade).** Für jeden
  ...
  Orchestrator die fehlende Differenz einem delay-fähigen Node entlang
  dieses Pfades per `setOutputDelay()` zu — bevorzugt möglichst spät im
  Pfad (§15.1 Punkt 3, Tally/Preview-Zwischenzustände nicht unnötig
  verzögern). Nutzt die bereits vorhandene Origin-Index-Erhaltung
  (`GstReferenceTimestampMeta`, C13-Nachtrag 2): `Ausgangs-Grain(N) =
  Eingangs-Grain(N) + D`. Fehlt auf einem zu kurzen Pfad ein
  delay-fähiger Node ganz, ist das ein Konfigurationsfehler analog Teil
  2 (klare Ablehnung, kein stiller Teil-Ausgleich). **Verifikation:**
  echter Live-Workflow mit zwei absichtlich unterschiedlich langen
  Pfaden (z. B. Video über `scaler`+`video-mixer-me`, Audio direkt) und
  gesetztem `targetLatencyFrames`; das Kopf-Wallclock-Skew-Verfahren von
  oben zeigt danach messbar reduzierten Skew gegenüber dem
  unkompensierten Zustand.

  **D8 Teil 4 (Audio-/Daten-Pfad getrennt + wiederverwendbares
  Mess-Werkzeug).** Kein Kopieren des Video-Frame-Budgets — Audio läuft
  mit eigener Einheit (Samples) parallel durch dieselbe Mechanik (§15.1
  Punkt 5); Audio-Follow-Video (§13.2) bleibt davon ausdrücklich
  getrennt (Latenzangleichung sorgt nur für gleichzeitiges Eintreffen
  gleich alter Signale, AFV braucht zusätzlich die explizite
  Tally-Kopplung). Das für die A/V-Sync-Messung oben von Hand gebaute
  Kopf-Index/Wallclock-Skew-Verfahren (mxl-info `Head index` durch
  `Grain/sample rate`, **nicht** `Last write time` — letzteres erwies
  sich bei Audio-Flows als nicht verlässlich aktualisiert) als
  wiederverwendbares Diagnose-Werkzeug festhalten (Skript oder
  Orchestrator-Diagnose-Endpoint), damit D8 Teil 2/3 sich live
  verifizieren lassen, ohne das Verfahren jedes Mal neu zu erfinden.
  **Verifikation:** dasselbe Werkzeug läuft gegen einen Audio- und
  einen Video-Pfad gleichzeitig und liefert für beide plausible,
  konsistente Skew-Werte.

- **D9 (IS-05-01-Basis-Discovery-Endpunkte, erledigt 2026-08-18).**
  Nutzeranfrage nach vertiefter AMWA-NMOS-Konformitätstestung (IS-04/
  IS-05/IS-09). D2 (2026-07-13) hatte bereits festgehalten, dass
  IS-05-01 (Connection API) gegen unsere Nodes sofort mit 0 Tests
  abbricht, weil die Basis-Discovery-Pfade fehlten (nur `staged`/
  `active` pro Receiver, Schritt B1) — "Kandidat für später". Genau das:
  `nodes/mock/internal/connection` (Go) und `nodes/omp-node-sdk/src/
  connection.rs` (Rust, mit `transport_urn`-Feld + `.with_transport()`-
  Override für den einzigen echten RTP-Sender `nodes/playout`, Default
  MXL für alle anderen) bekamen die vollständigen
  `/x-nmos/connection/v1.1/`-Basis-Discovery-Ressourcen (Wurzel-,
  `single/`-, Sender-/Receiver-Listing, `{id}/`-Root, `constraints/`,
  `transporttype/`). Vier echte Bugs live am AMWA-Tool-Lauf gefunden und
  behoben (Trailing-Slash-Teilbaum-Falle in Go UND das spiegelverkehrte
  Pendant in Rust, Wurzel-Wildcard verschluckte `/bulk/*`, Fehlerantworten
  waren `text/plain` statt dem geforderten `application/json` +
  fehlende CORS-Header). IS-09 (System API) hat noch keinerlei
  Code-Berührung — zu groß für dieselbe Sitzung, eigener künftiger
  Schritt. Blackmagic-DeckLink-Anfrage (parallel gestellt) per
  `AskUserQuestion` vertagt: echte Hardware existiert laut Nutzer
  anderswo im Setup, §24.6/§24.7s "kein Capture-Karten-Pfad" damit nicht
  mehr dauerhaft, aber diese Sitzung priorisierte NMOS zuerst (Nutzerwahl).
  **Verifikation bestanden:** `go build/vet/test ./...` (mock-Modul, 6
  neue Testfunktionen), `cargo build/clippy -D warnings/test --workspace`
  (5 neue SDK-Unit-Tests) grün. Vier echte `nmos-test.py suite
  IS-05-01`-Läufe gegen den lokal laufenden Mock-Node (Podman,
  `docker.io/amwa/nmos-testing:latest`), jeweils nach einem Fix erneut
  ausgeführt: 0 → 8 → 14 → 17 bestandene Tests. 19 verbleibende
  `Fail`-Ergebnisse einzeln als CI-`--allow`-Ausnahme benannt (kein
  stilles Ignorieren, gleiche Methode wie D2) — Ursache in vier
  benannten, bewusst nicht geschlossenen Lücken: fehlender echter Sender
  zum Verbinden (`-senders 0`, B1-Scope-Grenze), `transport_params`-
  Feldtiefe/PATCH-Merge, `activation_time` im Activation-Schema, PATCH-
  Validierung + echte Bulk-POST-Semantik. `.github/workflows/ci.yml`
  (`amwa-nmos-testing`-Job) baut/startet den Mock-Node jetzt und wertet
  IS-05-01 mit derselben Ausnahmeliste aus (lokal gegen die echte
  Ergebnisdatei vorab verifiziert, `exit 0`). Details: `docs/
  decisions.md` Nachtrag 142.

- **D10 (Blackmagic DeckLink SDI/IP, Teil 1: Ingest, erledigt
  2026-08-18).** Direkte Nutzeranweisung im Anschluss an D9: "§24.6/
  §24.7 'kein Capture-Karten-Pfad' aufheben und umsetzen" — hebt die
  bisher zweimal bestätigte Entscheidung auf (`ARCHITECTURE.md`
  §24.6/§24.7, dort jetzt als aufgehoben markiert), Grund war stets
  "keine testbare Hardware", die laut Nutzer inzwischen an anderer
  Stelle im Setup existiert. Volles Design + Referenz-Analyse
  `/home/infantilo/PIPELINE CONTROLLER` (echte, produktiv gelaufene
  DeckLink-Integration, `lib/OutputEngine.js`/`MasterPipeline.js`/
  `server.js`) in `docs/END-GOAL-FEATURES.md` Kapitel 20. Neuer Node
  `nodes/omp-decklink`: `decklinkvideosrc`+`decklinkaudiosrc` (GStreamer,
  `gst-plugins-bad`, hier bereits installiert verifiziert) → `MxlVideoOutput`/
  `MxlAudioOutput`, strukturell wie `omp-source`s Pipeline. Modus bewusst
  fest (`OMP_DECKLINK_MODE`, sechs aus PIPELINE CONTROLLER übernommene,
  dort bereits verifizierte Modi — kein `auto`, MXL-Flows brauchen die
  Auflösung vorab), `channels`-Enum-Validierung ({2,8,16}) vor
  Pipeline-Aufbau (derselbe reale Fallstrick, den PIPELINE CONTROLLERs
  Kommentar dokumentiert), `signal`-Property gepollt (kein Bus-Event
  fürs Plugin) + Alarm bei Signalverlust. **Echter Bug live gefunden
  und behoben** (ohne Hardware, betraf den Ablehnungspfad selbst):
  ein fehlgeschlagener `PLAYING`-Übergang ohne anschließendes
  `set_state(Null)` ließ den Prozess mit Coredump abstürzen (bekannte
  `gstreamer-rs`-Falle, dreimal reproduziert, dann dreimal sauber mit
  `exit 1` nach dem Fix bestätigt) — `gst-launch-1.0` mit demselben
  Element allein zeigte den Absturz NICHT, also kein Plugin-Bug.
  **Bewusst nicht Teil dieser Runde:** Ausgabe-Richtung (Teil 2,
  `decklinkvideosink`/`decklinkaudiosink`), Host-Inventar/Placement-
  Claim-Release für exklusive I/O-Karten-Ressourcen (`ARCHITECTURE.md`
  §6.1, seit D6 Teil 4 unverändert offen), interlaced Modi, PTP-
  Kartentakt als Pipeline-Clock. **Verifikation bestanden:** `cargo
  build/clippy -D warnings --workspace` grün (neue Crate + Workspace-
  Mitgliedschaft), drei reale Prozessläufe ohne Hardware (klarer
  Fehler statt Absturz, `OMP_DECKLINK_MODE`/`OMP_DECKLINK_AUDIO_
  CHANNELS`-Validierung greift). **Nicht hier verifizierbar** (§0
  Punkt 7 gilt für den Rest weiter): echter Signalempfang, `signal`-
  Korrektheit gegen echtes Eingangssignal, Bild-/Tonqualität — braucht
  den Host mit der echten Karte. `deploy/catalog.json` neuer Eintrag.
  Details: `docs/decisions.md` Nachtrag 143, `docs/END-GOAL-
  FEATURES.md` Kapitel 20.

- **D10 Teil 2 (Ausgabe-Richtung, erledigt 2026-08-18).** Direkter
  Nutzerauftrag im Anschluss an Teil 1: "Teil 2 (Ausgabe-Richtung)
  umsetzen". `omp-decklink` wird gerichtet (`OMP_DECKLINK_DIRECTION=
  ingest|output`, wie in 20.2 geplant, hier umgesetzt) — `main.rs`
  bekommt eine `Direction`-Weiche, `pipeline.rs` eine zweite Sektion
  (`OutputConfig`/`run_output`/`build_output`) neben der (dafür
  umbenannten `IngestConfig`/`run_ingest`) Ingest-Sektion. Quellwahl
  per echtem IS-05-Receiver-PATCH (zwei Receiver Video/Audio, gleiches
  `is_video`-Flag-Muster wie `omp-recorder`), Pipeline wird bei jeder
  Verbindungsänderung neu gebaut (wie `omp-2110-gateway`s
  Output-Richtung) — Video ist die Anker-Verbindung (PIPELINE
  CONTROLLERs eigenes Muster: Video-Zweig immer, Audio optional),
  Audio kann unabhängig verbunden werden, aber nicht ohne Video.
  Dieselbe Modus-Tabelle wie Teil 1 setzt feste Ziel-Caps vor
  `decklinkvideosink`; `decklinkaudiosink` hat anders als
  `decklinkaudiosrc` kein `channels`-Property (geprüft, nicht
  angenommen) — Kanalzahl kommt aus expliziten Caps. Derselbe
  Coredump-Bugtyp wie Teil 1 präventiv mitgefixt (`ActiveOutputPipeline`
  trägt jetzt eine `Drop`-Absicherung, da diese Pipeline mehrfach über
  die Prozesslaufzeit neu entsteht, anders als die einmalige
  Ingest-Pipeline). **Stärker verifiziert als Teil 1:** echter
  End-to-End-Lauf mit lokaler NMOS-Registry+NATS, echtem `omp-source`-
  Flow und echtem IS-05-PATCH gegen den laufenden `omp-decklink`-
  Output — Registry-Auflösung/`MxlVideoInput`-Verbindung funktionierten
  nachweislich, erst `decklinkvideosink`s `set_state(Playing)`
  scheiterte mangels Karte, sauber, Prozess blieb am Leben, zweimal
  wiederholt ohne Unterschied, kein Coredump. Testinfrastruktur
  danach vollständig aufgeräumt. `cargo build/clippy -D warnings/test`
  grün. Bewusst weiterhin nicht Teil: Host-Inventar/Placement-Claim-
...
- **D12 (ARCHITECTURE.md §19, vollständig erledigt 2026-08-18, Teil 1-3)**
  ...
  volle Begründung/Abwägung, Nachtrag 147 für die Teil-1-Umsetzung).
  Architektur steht vollständig in ARCHITECTURE.md §19.3; hier die
  Drei-Teile-Gliederung für je eine Sitzung.

  **D12 Teil 1 (erledigt, 2026-08-18) — Grundgerüst: Single-Node-Raft,
  Transport, Status-API.** Neues Paket `orchestrator/internal/cluster`
  (`cluster.go`/`fsm.go`/`transport.go`): `hashicorp/raft` v1.7.3 +
  `hashicorp/raft-boltdb/v2` v2.3.1 eingebunden (Begründung s. §19.3
  Punkt 1 — schließt die Postgres-Abhängigkeit der ursprünglichen
  Leader-Wahl-Idee, Minimal-Dependency-Regel damit erfüllt trotz zweier
  neuer Bibliotheken). `raft.NetworkTransport` über eigenen TCP-Port
  (`OMP_RAFT_LISTEN`, Default `127.0.0.1:8300`) via eigenem
  `streamLayer` (raft.StreamLayer, echtes TCP statt eines
  NATS-Pub/Sub-Nachbaus, s. §19.3 Punkt 2), optional mTLS über eine neue
  `mtls.ServerTLSConfig` (ergänzt die bestehende `ClientTLSConfig`,
  gemergt zu einer Peer-tauglichen `*tls.Config` in
  `cluster.peerTLSConfig` — Raft-Instanzen sind sich gegenseitig
  gleichrangige Peers, nicht Client/Server in getrennten Rollen wie
  Orchestrator↔Node) — reuse derselben `OMP_MTLS_*`-Zertifikate wie D3,
  kein zweiter PKI-Pfad. `raft.FileSnapshotStore` + BoltDB-Log/Stable-
  Store unter `OMP_RAFT_DATA_DIR` (Default `../data/raft/<nodeID>`).
  FSM minimal wie geplant (`FSM.Apply` zählt nur einen Versionszähler
  hoch — Teil 3 füllt den echten Inhalt). Frische Instanz ohne
  bestehenden Raft-Log bootstrapt sich selbst — als Ein-Knoten-Cluster
  (`OMP_CLUSTER_PEERS` leer, Default) oder mit der vollen, bei allen
  Gründungsmitgliedern identisch gesetzten Peer-Liste
  (`raft.BootstrapCluster` mit der kompletten `raft.Configuration` auf
  jeder Instanz gleichzeitig — empirisch verifiziert, dass das
  zuverlässig konvergiert, s. Verifikation unten; kein Sonderfall-Code
  für „nur eine Instanz", das ist der Normalfall). Neuer Endpunkt
  `GET /api/v1/cluster/status` (`ClusterService`-Interface,
  `cluster_handlers.go`, authentifiziert wie `GET /api/v1/hosts`) liefert
  Leader-ID/-Adresse, Term, angewandten/letzten Log-Index, FSM-Version,
  volle Peer-Liste. `main.go`: `cluster.New()` vor allen bisherigen
  Hintergrund-Loops konstruiert (`defer clusterNode.Shutdown(...)`),
  diese laufen in Teil 1 noch unverändert immer auf jeder Instanz
  (Aktiv/Passiv-Gating erst Teil 3) — reine Additions-Sitzung, kein
  Verhaltensumbau der bestehenden Loops.

  **Verifikation (bestanden):** `go build/vet/test ./...` grün
  (`internal/cluster` neu: `ParsePeers`-Fälle, Ein-Knoten-Bootstrap wird
  Leader, Drei-Knoten-Bootstrap repliziert einen echten `Apply()` auf
  alle drei FSM-Instanzen, Leader-`Shutdown()` löst echte Neuwahl unter
  den verbleibenden zwei aus — alles über echtes TCP/Loopback zwischen
  echten `*raft.Raft`-Instanzen im Testprozess, kein gemockter
  Transport); `internal/mtls` um `ServerTLSConfig`-Tests erweitert.
  **Zusätzlich live gegen drei echte, separate OS-Prozesse verifiziert**
  (nicht nur der Testprozess oben): drei reale
  `orchestrator`-Binaries (`OMP_LISTEN`/`OMP_NODE_ID`/`OMP_RAFT_LISTEN`/
  `OMP_RAFT_DATA_DIR` je eindeutig, `OMP_CLUSTER_PEERS` identisch)
  gegen dieselbe echte Dev-Postgres/-NATS gestartet — `GET
  /api/v1/cluster/status` (echter HTTP-Login mit dem Dev-Admin-Konto)
  zeigte auf allen drei Instanzen übereinstimmend genau einen Leader +
  zwei Follower; ein echtes `kill -9` des Leader-Prozesses ließ einen
  der beiden verbliebenen Follower binnen Sekunden zum neuen Leader
  werden (Term 2→4, per API auf beiden Überlebenden bestätigt). Eine
  vierte, separat gestartete Solo-Instanz ohne `OMP_CLUSTER_PEERS`
  bestätigte das unveränderte Default-Verhalten (Ein-Knoten-Cluster,
  sofort eigener Leader, `GET /api/v1/nodes`/`/healthz` unverändert
  funktionsfähig — keine Regression für den bisherigen Single-Host-
  Betrieb).
  **Bewusst nicht Teil 1:** Join/Leave-API (Teil 2), jede Änderung an
  `workflows`/`placement`/`launcher` (Teil 3).

  **D12 Teil 2 (erledigt, 2026-08-18) — Live-Mitgliedschaft: Join/Leave-API,
  Mehrknoten-Wachstum.** `POST /api/v1/cluster/join` (Admin-Verb, nur auf
  dem aktuellen Leader wirksam — `cluster.Node.Join` ruft `raft.AddVoter`
  für die beitretende Instanz auf, sonst Weiterleitung über
  `forwardToLeader`); `DELETE /api/v1/cluster/members/{id}` das
  Gegenstück (`raft.RemoveServer`). **Design-Korrektur gegenüber der
  ursprünglichen Skizze:** statt die HTTP-Adressen statisch in
  `OMP_CLUSTER_PEERS` vorzubelegen, kündigt jede Instanz ihre eigene
  HTTP-Adresse (`OMP_ORCHESTRATOR_URL`, wiederverwendet statt eines
  neuen Feldes) automatisch bei jedem Führungswechsel selbst über einen
  neuen, kleinen FSM-Kommandotyp an (`cluster.watchLeadership` +
  `CommandSetPeerHTTPAddr`, `fsm.go`) — das eigentliche Peer-
  Adressbuch lebt damit repliziert im FSM statt in lokaler Config,
  jede Instanz (inkl. neu beigetretener) lernt es beim Nachziehen des
  Logs automatisch mit. Einfacher HTTP-Reverse-Proxy
  (`httputil.NewSingleHostReverseProxy`, `forwardToLeader` in
  `cluster_handlers.go`): eine Nicht-Leader-Instanz leitet
  `/api/v1/cluster/join`/`/members/{id}` transparent an die per
  `LeaderHTTPAddr()` aufgelöste echte Leader-Adresse weiter. Neue
  Beitritts-Instanzen starten mit `OMP_CLUSTER_JOIN=true`
  (`cluster.Config.SkipBootstrap`) — bootstrappen sich NICHT selbst,
  warten passiv auf `Join()` durch eine bestehende Leader-Instanz
  (Standard-Beitritts-Muster aus `raft.Raft.BootstrapCluster`s eigener
  Doku: "bootstrap a single server … then invoke AddVoter()").
  **Empirisch korrigierte Annahme:** `raft.Config.ShutdownOnRemove`
  (Default true) fährt laut Quelltext (`raft.go` `leaderLoop`) NUR den
  Leader herunter, wenn er SICH SELBST entfernt — eine entfernte
  Nicht-Leader-Instanz fährt ihren lokalen `*raft.Raft` nicht
  automatisch herunter, erkennt ihre Entfernung aber korrekt in der
  eigenen `GetConfiguration()`/Status()-Sicht (live verifiziert, s.
  `docs/decisions.md` Nachtrag 148) — ursprünglich anders angenommen,
  vor der Umsetzung durch einen gezielten Testlauf statt durch
  Doku-Vertrauen geklärt.
  **Verifikation (bestanden):** `go build/vet/test ./...` grün (neue
  Tests: `TestLeaderSelfAnnouncesHTTPAddr`,
  `TestJoinAddsVoterAndAnnouncesHTTPAddr`,
  `TestJoinOnFollowerFailsWithErrNotLeader`,
  `TestLeaveRemovesVoterAndPeerHTTPAddr`). **Zusätzlich live mit drei
  echten, separaten OS-Prozessen verifiziert:** Instanz `a` startet solo
  (Ein-Knoten-Leader wie Teil 1); Instanz `b` startet mit
  `OMP_CLUSTER_JOIN=true` und wartet ("no known peers, aborting
  election"); ein echter `POST /api/v1/cluster/join` gegen `a` fügt `b`
  hinzu (Cluster 1→2, `peerHttpAddrs` korrekt für beide gefüllt); Instanz
  `c` startet ebenso wartend, ihr Join-Request wird bewusst an den
  FOLLOWER `b` geschickt (nicht an den Leader `a`) — die Antwort trägt
  nachweislich `"nodeId":"a"` (`b` hat transparent weitergeleitet, nicht
  lokal geantwortet), Cluster wächst 1→2→3, auf allen drei Instanzen per
  `GET /api/v1/cluster/status` bestätigt. Echtes `kill -9` von `a`:
  `b` wird binnen Sekunden neuer Leader (Term 2→3), kündigt seine
  eigene HTTP-Adresse automatisch neu an. Ein `DELETE
  /api/v1/cluster/members/a` (gegen den neuen Leader `b`) entfernt den
...
3. **Kein Blockly/kein JS-Framework für den Prozess-Editor** — die
   ...
   **Nutzerentscheidung 2026-09-22 (nach Abschluss von Phase 5 Teil 3):
   eigener Editor auf `ui/graph`-Basis (empfohlene Option) bestätigt.**
...
**Status 2026-09-23 (Nachtrag 283): implementiert + live verifiziert,
exakt nach diesem Phasenplan/dieser Phase-2-Auflösung.** Details:
docs/decisions.md Nachtrag 283, Statustabelle unten. Drei bewusste,
dokumentierte Lücken bleiben offen (kein Cross-Org-Superadmin-Bypass,
`human-tasks?assignee=` und `DELETE /asset-relationships/{id}`
ungefiltert) — s. dortige Begründung.
...
| Kapitel 9 Teil 1 + Raspberry-Pi-4-HDMI-Gateway (Nutzerwunsch 2026-07-28) | **geplant, noch nicht begonnen** | Nutzer-Feedback 2026-07-28: „ich möchte billig rechner wie einen raspberry pi 4 als gateway zu HDMI nutzen können als viewer oder multiviewer oder audio breakout" — mit eigener Protokoll-Vorabeinschätzung (RTP/WebRTC 20–50ms, MXL/RDMA <5ms aber nicht Pi-tauglich, ST 2110 1–5ms, SRT 40–100ms). Deckt sich mit `docs/END-GOAL-FEATURES.md` §9 (bereits am 2026-07-19 spezifiziert, bislang **komplett unbegonnen** — kein Eintrag hier vor diesem) — §9.3 Pfad B beschreibt exakt diesen Anwendungsfall ("ein dediziertes Decoder-Gerät … direkt am Monitor, kein Chrome-Tab dazwischen"), §9.5 Teil 1 ist der passende Einstiegs-Slot. MXL/RDMA und echtes ST 2110 scheiden für ein Pi4 aus denselben Gründen aus, die der Nutzer selbst schon benennt (kein RDMA-NIC, kein PTP-HW, Bandbreite) — nicht neu zu entscheiden. WebRTC bleibt außen vor wie in §9.6 Frage 1 beantwortet (dedizierter Monitor, kein Browser-Tab-Anwendungsfall). **Scope für die erste Scheibe (Vorschlag, mit Nutzer noch nicht final abgestimmt):** (1) klären, ob ein zusätzlicher MXL-Sender am `omp-viewer`-/`omp-multiviewer`-Compositor-Ausgang ausreicht, damit das bereits fertige `omp-srt-gateway` (D4) ihn unverändert bridgt (offene Frage aus §9.5 Teil 1); (2) RTP als zweite, einfachere LAN-Transportoption direkt mitevaluieren/ergänzen (Nutzerwunsch — auf einem verkabelten LAN unnötiger SRT-Overhead, niedrigere Latenz, Pi4 hat Hardware-H.264-Decode über V4L2); (3) Tag-1-Minimalversion auf der Pi4-Seite bewusst ohne neuen Node/Cross-Build: eine dokumentierte `gst-launch-1.0`-Empfangspipeline (SRT oder RTP → `autovideosink`/`kmssink`) direkt in `docs/HANDBUCH.md`, kein eigenständiger OMP-Node nötig für den ersten Nachweis. **Bewusst nicht Teil dieser ersten Scheibe** (spätere Ausbaustufe, eigener Schritt): volle Graph-Integration der Pi4 als echter IS-04/05-Node (ARM/aarch64-Cross-Build von `omp-viewer` inkl. GStreamer/MXL, s. §7.4-Cross-Compiling-Ethos in `ARCHITECTURE.md`), Audio-Breakout speziell über AES67 (`omp-aes67-gateway`, Kapitel 19 Teil 0/3, bereits fertig — als Alternative zu RTP/SRT für den reinen Audio-Fall zu prüfen, aber separat), JPEG-XS (§9.5 Teil 3, ausdrücklich risikobehaftet). **Verifikation (geplant):** der Software-Pfad (MXL-Flow → RTP-/SRT-Sender → Empfang+Decode in einem zweiten lokalen Prozess) ist vollständig in dieser Sandbox prüfbar, ohne echte Hardware; der eigentliche HDMI-Output auf einer echten Raspberry Pi 4 braucht das reale Gerät (anders als bei professionellem 2110-Equipment aber günstige Consumer-Hardware, die der Projektinhaber selbst bereitstellen müsste) — bis dahin gilt dieser Schritt als durch Software-Loopback verifizierbar, aber nicht als Endnutzer-Anspruch "läuft am echten Pi" abgeschlossen. **Ausdrücklich noch nicht ausgeführt** (Nutzeranweisung 2026-07-28: nur als Schritt formulieren, nicht gleich umsetzen) — nächste Sitzung: mit Nutzer RTP-vs-SRT-Default und Tag-1- vs. Graph-Integrations-Scope endgültig abstimmen, dann §0 Punkt 2 (ein Schritt pro Sitzung) anwenden. | 2026-07-28 |
| Scaler-Zielformat im grafischen Role-Designer wählbar | erledigt | Nutzerreport: "scaler node hat kein auswählbares ausgangsformat". Rückfrage ergab: Nutzer arbeitet im grafischen Rollen-Designer (`ui/graph/role-designer.ts`) — der hatte laut eigenem Kopfkommentar bewusst kein Format-Feld (nur das ältere Text-Formular `workflows-view.ts`). Neues geteiltes `ROLE_FORMATS` in `ui/graph/roles.ts` (löst auch die bisherige Duplikat-Liste in `workflows-view.ts` ab, eine Quelle für beide Editoren). Jede Rollen-Kachel bekommt eine zusätzliche Zeile (`foreignObject` mit `<select>`, `TILE_HEIGHT` um `FORMAT_ROW_HEIGHT` gewachsen), `DraftRole.format` neu im Typ. Live per echtem CDP-Klicktest verifiziert: Login, grafischen Designer geöffnet, Scaler-Rolle per Klick hinzugefügt, Format-Dropdown zeigt alle 22 Presets, "1080p50" gewählt, "Anlegen" geklickt — abgefangener echter `POST /api/v1/workflows`-Body enthält `"format":"1080p50"`. Test-Workflow danach gelöscht. `deno check` grün | 2026-07-29 |
| Scaler "erzeugt keinen Output-Stream": Root-Cause gefunden + behoben (Orchestrator-Wiring nicht format-bewusst) | erledigt | Zweiter Teil desselben Nutzerreports. Live-Repro (`omp-source`→`omp-scaler`, keine explizite `fromSender`): `omp-scaler`s Log zeigte `pipeline error: connect <flow> failed: flow_def: frame_width fehlt` — der Receiver war per IS-05-PATCH auf `src`s **Audio**-Sender verbunden, nicht den Video-Sender. Root Cause in `orchestrator/internal/workflows/service.go`: `findSender` nahm bei leerem Label blind `node.Senders[0]`, unabhängig vom Format — seit `omp-source` (Audio-Companion-Pattern) mehrere Sender unterschiedlichen Formats hat, registrierungsreihenfolge-abhängig. Fix: Receiver wird jetzt zuerst aufgelöst (sein `Format` steht direkt am IS-04-Resource), `findSender` bekommt ein `preferFormat`, das den passenden Sender bevorzugt (Label schlägt weiterhin alles); derselbe Mechanismus für den Crosspoint-Zweig (`omp-switcher`/`omp-video-mixer-me`, fest `formatVideo`, da beide heute reine Video-Crosspoints sind). **Zweiter, beim Live-Verifizieren gefundener Fund:** unter mehreren format-passenden Sendern landete die Wahl auf dem `" Lowres"`-Vorschau-Companion (640×360) statt dem Hauptsignal — keine Fehlermeldung, nur leise falsche (niedrigere) Qualität; zusätzlicher `isLowresCompanion`-Ausschluss behoben. Live-Ende-zu-Ende-Verifikation nach dem Fix: `connectedFlowId` zeigt jetzt den echten `src`-Vollbild-Sender, `mxl-info` bestätigt den Scaler-Zielfluss `Active: true` mit tatsächlich fortschreitendem `Head index` (kontinuierlich bei 50fps). `go build`/`go test ./internal/workflows/...` grün. Test-Workflows danach gestoppt+gelöscht, keine verwaisten Prozesse. Details/A-V-Sync-Anschlussmessung: s. nächste Zeile (D8) | 2026-07-29 |
| A/V-Sync-Live-Messung am Sink (Anlass für D8, s. oben) | erledigt (Messung) | Dritter Teil desselben Nutzerreports: "teste audio video synchronität". Testworkflow `src(1080p50)→scaler→` (Video) neben `omp-audio-mixer`-internem Testton (Audio) — beide Ausgänge fast gleichzeitig per `mxl-info` abgefragt, `Head index / Grain-Rate` (nicht `Last write time`, s. u.) gegen Wallclock verglichen. Befund: Video-Ausgang ~70-90ms hinter Wallclock, Audio-Ausgang ~50-86ms — Video-zu-Audio-Skew schwankt zwischen etwa -13ms und +17ms (unter einer 1080p50-Framedauer), **unkontrolliert** (keine aktive Kompensation vorhanden, s. `ARCHITECTURE.md` §15). Nebenfund: `mxl-info`s `Last write time`-Feld wird bei Audio-Flows nicht zuverlässig aktualisiert (blieb über Sekunden eingefroren, obwohl `Head index` nachweislich in Echtzeit fortschrieb, per Zwei-Punkt-Messung bestätigt) — für künftige Messungen `Head index/Grain-Rate` verwenden, nicht `Last write time`. Test-Workflow danach gestoppt+gelöscht. Ausführliche Arbeitsschritt-Planung für die eigentliche Kompensation: s. D8 oben (`ARCHITECTURE.md` §15) | 2026-07-29 |
| Neuer Node `omp-audio-monitor` + `omp-audio-mixer` Solo/PFL-Monitor-Bus | erledigt | Nutzerwunsch: "wir müssen auch das audio abhören können. kann der viewer auch audio oder brauchen wir ein audio monitoring node? ... audio mixer eine pre-listen funktion". Per Rückfrage entschieden: dedizierter neuer Node statt Audio in `omp-viewer` (dessen Vorschau ist MJPEG-Multipart, grundverschieden von Audio-Streaming). **`omp-audio-monitor`** (neue Crate, exaktes Strukturmuster von `omp-viewer`/`omp-scaler`: IS-05-Drag&Drop-Quellwahl, `MxlAudioInput`): neues `omp_mediaio::audio_stream`-Modul (eigenes Cargo-Feature `audio-stream`) — MP3 statt Ogg/Opus bewusst (Framing-Grund: MP3-Frames sind einzeln decodierbar, ein spät verbindender Client braucht keinen Header-Replay wie bei Ogg, exakt das Internet-Radio-Prinzip), `lamemp3enc` (bereits installiertes `gstreamer1.0-plugins-ugly`, 128 kbit/s), Broadcaster ohne "letzten Chunk" (anders als `preview.rs`, hier kein Sofortbild-Äquivalent nötig). UI-Bundle: `<audio controls>` gegen den K4-Stream-Proxy, gleiches `?access_token=`-Fallback-Muster wie `omp-viewer`. **Solo/PFL** (`omp-audio-mixer`): pro Kanal ein `tee` direkt nach `level` (post-EQ/Kompressor, pre-Fader — der korrekte Prefader-Abgriffspunkt) speist zusätzlich zum unveränderten Hauptpfad einen `volume`("pfl-gain")-Zweig in einen neuen zweiten `audiomixer` ("pfl-mixer"); ein analoger Tee am Master-Ausgang speist "master-pfl-gain", der auf `1.0` steht (Monitor spiegelt Programm) solange kein Kanal soloed ist, und auf `0.0` fällt (`recompute_master_pfl_gain`), sobald mindestens ein Kanal `channel.<id>.setPfl` aktiviert — dieselbe Semantik wie ein klassischer Konsolen-PFL-Weg. Zweiter, eigenständiger MXL-Sender ("… Monitor") über einen zweiten `MxlAudioOutput` in derselben Pipeline. Neuer UI-"Solo"-Knopf neben "Mute". **Live gefundener + behobener Bug (`omp-mediaio::mxl`):** `MxlAudioOutput::new` hatte sein internes `valve`-Element fest `"mxl_audio_output_valve"` benannt (nie per `get_by_name` gebraucht) — ein Node mit ZWEI `MxlAudioOutput`s in derselben Pipeline (Programm- + Monitor-Ausgang) ließ `pipeline.add()` beim zweiten Aufruf mit "Failed to add element" scheitern (GStreamer erlaubt keine zwei gleichnamigen Elemente in einer Bin), Crash-Loop live reproduziert; Fix: `.name(...)` entfernt, GStreamer vergibt automatisch eindeutige Namen — betraf jeden potenziellen künftigen Zwei-Ausgänge-Fall, nicht nur diesen. Live-Ende-zu-Ende-Verifikation: `omp-audio-monitor` an den Monitor-Sender verbunden, echter `ffprobe`-Check bestätigt gültiges 48kHz-Stereo-MP3; Zwei-Kanal-Test (440Hz/660Hz Testtöne) per `ffmpeg`-Bandpass+`volumedetect` zeigt Baseline beide Bänder bei ~-13.8dB, nach `setPfl(ch1,true)` fällt die 660Hz-Bandenergie auf -36.6dB (Kanal 2 im Monitor praktisch stumm) während 440Hz unverändert bleibt, nach Un-Solo wieder beide bei ~-13.8dB. `cargo build --workspace --bins`, `cargo test -p omp-mediaio` (8/8), `cargo clippy --workspace` (nur vorbestehende, unveränderte Warnungen in anderen Nodes) grün. Katalog-Eintrag ergänzt, Test-Workflows danach gestoppt+gelöscht, keine verwaisten Prozesse | 2026-07-29 |
| Audio-Follow-Video Ende-zu-Ende live verifiziert (mit neuem Audio-Monitor abgehört) | erledigt | Letzter Teil desselben Nutzerreports: "audio follow video workflow end to end testen". Echter Workflow: zwei `omp-source`, ein `omp-video-mixer-me`, ein `omp-audio-mixer` (ein Kanal, `followTarget` = `src1`s Node-ID, `followMode=cut`), ein `omp-audio-monitor` an `amix`s Programm-Sender (nicht den neuen PFL-Bus) verbunden. Kanal manuell erst stummgeschaltet (Baseline) — echter Abruf des MP3-Stroms zeigt `-91dB` (Rauschgrund/Stille). `crosspoint.take(src1)` am Mixer → `channel.ch1.mute` fällt automatisch (per `omp.tally.<node_id>`-NATS-Bus, C10/C11-Kopplung) auf `false` — echter Stream-Abruf zeigt jetzt `-13.9dB` (hörbarer Testton). `crosspoint.take(src2)` (src1 damit von PGM) → `mute` springt zurück auf `true`, Stream wieder bei `-91dB`. Bestätigt die komplette Kette produktionsnah: Bildschnitt → Tally-Event → AFV-Kopplung → tatsächlicher Audio-Pegel am neuen Abhörweg, nicht nur der API-Zustand. Test-Workflow danach gestoppt+gelöscht, keine verwaisten Prozesse | 2026-07-29 |
| Fünf-Bugs-Report 2026-07-30: Flow-Editor-Gruppen-Nesting, Video-Mixer-M/E-PGM-Race (+ UID-Stop/Restart-Bug als Nebeneffekt behoben), sprechende Rollennamen | erledigt | Nutzerreport in einer Nachricht, vier echte Root-Causes plus ein bereits funktionierendes Feature bestätigt: (1) **"neue Nodes im laufenden Workflow/Gruppe landen im Root"** — Live-Test zeigte: für einen LAUFENDEN Workflow (`#renderRunningWorkflowScope`, flow-canvas.ts) funktionierte das Nesting bereits korrekt (`#workflowScopePendingInstanceIds`, Nachtrag vom 2026-07-26); der tatsächliche, bisher komplett unbehandelte Fall war die ECHTE B5-Gruppe (`#groupTree`/`#enterScope`, `#startInstance` kannte nur den Workflow-Zweig) — live reproduziert (neuer Katalog-Node blieb nach Klick "+ Source" innerhalb einer offenen Gruppe am Root). Neues `addMember()` (`ui/graph/groups.ts`, reine Funktion + 3 Tests) plus `#groupScopePendingInstances`-Map (instanceId→groupId, da der Nutzer die Gruppe vor der NMOS-Registrierung schon wieder verlassen haben könnte) + `#reconcileGroupScopePendingInstances()` in `#fetchAndRender()`. (2) **"Positionen/Anordnung im Workflow speichern"** — bei genauerer Live-Prüfung bereits vorhanden: ein Drag innerhalb einer laufenden Workflow-/Gruppen-Ansicht läuft über dieselbe generische `#onPointerUp`/`#saveLayout()`-Pipeline wie jede andere Kachel, per echtem CDP-Drag+Netzwerk-Mitschnitt bestätigt (`PUT /api/v1/layouts/default` mit neuer Position, per Reload/GET verifiziert) — kein Code-Fix nötig, vermutlich nur durch Bug (1) als kaputt wahrgenommen (neu hinzugefügte Nodes "verschwanden" ja aus der Gruppen-Ansicht). (3) **"source→scaler→videomixer-M/E schaltet nicht auf PGM, PGM bleibt schwarz"** — Root Cause per Orchestrator-Log gefunden (nicht geraten): ein frisch gestarteter Scaler ist manchmal schon per IS-04 discoverbar, bevor sein MXL-Flow für andere Prozesse tatsächlich lesbar ist (`get_flow_def`: "Flow not found") — `omp-video-mixer-me`s `build_one_input` überspringt so einen Eingang korrekt (kein Absturz), aber `inputs_changed` verglich bisher nur die Sender-ID-MENGE: blieb sie unverändert, gab es nie einen zweiten Versuch, der Eingang blieb für die gesamte Pipeline-Lebensdauer dauerhaft schwarz. Fix (`pipeline.rs`): `missing_input_ids()`-Hilfsfunktion + `MISSING_INPUT_RETRIES=5` — der Leerlauf-Tick der Haupt-Loop versucht einen unvollständigen Eingang jetzt bis zu 5× erneut, unabhängig von einer echten Mengenänderung, bevor er als dauerhaft tot gilt (kein endloses Rebuild-Geflacker für echte Registry-Geister). Live mit mehreren Stop/Start-Zyklen reproduziert (Race trat praktisch bei jedem schnellen Neustart auf, 1-3 Skips bis zur Selbstheilung beobachtet) und nach dem Fix durchgehend erfolgreich verifiziert. (4) **"Video-Mixer M/E verliert Quellen bei Stop/Restart"** — entpuppte sich als dieselbe Ursache wie (3): der Orchestrator hat für genau dieses Symptom bereits eine Alias-basierte State-Restore-Infrastruktur (`orchestrator/internal/workflows/state.go`, `senderAliasPrefix`, `restoreStateRetries=8`), deren eigene Doku denselben MXL-Verzögerungsbug schon beschrieb, aber nur mit einem großzügigeren Zeitbudget "gehofft" hatte, statt ihn deterministisch zu schließen — der (3)-Fix macht die Selbstheilung zuverlässig, End-zu-Ende mit echtem Stop/Start-Zyklus verifiziert (`programSenderId` löst nach Neustart automatisch auf die neue Scaler-Sender-ID auf). (5) **"sprechende Namen für Sources/Streams"** — der Rollenname IST bereits das Sender-/Crosspoint-Label (`OMP_LABEL`), aber nirgends im GRAFISCHEN Editor umbenennbar (nur automatisch vergeben, `uniqueRoleName`) — neues `renameRole()` (`ui/graph/role-designer-logic.ts`, reine Funktion + 6 Tests, benennt referenzierende Verbindungen mit um) plus Doppelklick-zu-Umbenennen-UI in BEIDEN grafischen Editoren (`role-designer.ts` für die Vorlagen-Bearbeitung, `flow-canvas.ts`s `#renderEditableRoleTile` fürs Live-Bearbeiten eines pausierten/laufenden Workflows — zwei separate Renderpfade, dieselbe geteilte Logik). Live per echtem CDP-Doppelklick+Tastatureingabe in beiden Editoren verifiziert (inkl. Escape-Abbruch, Duplikat-Namen-Ablehnung mit Toast, PUT-Rückbestätigung). `cargo build --workspace --bins`, `cargo test -p omp-video-mixer-me`, `cargo clippy -p omp-video-mixer-me` sauber; `deno check ui/**/*.ts` + `deno test ui/` (79 Tests) grün. Alle Test-Workflows/-Instanzen danach gestoppt+gelöscht, Test-Gruppe aus dem Layout entfernt, keine verwaisten Prozesse | 2026-07-30 |
| K7 Teil 4 (Hot-Standby, automatischer Cross-Host-Failover) | erledigt | Nutzerauftrag "K7 4 fertig machen". `Role.StandbyFor` (additiv, keine Migration) + Validierung; zwei Trigger — Crash-Loop-Bremse erschöpft (`launcher.FailoverObserver`) und Host komplett offline (`RunFailoverWatcher`-Ticker, `hostOfflineTimeout=20s`, wiederverwendet `hostMetricsTracker`); `promoteStandby` (`failover.go`, neu) analog `rewireAfterRestart` + Bedienzustand-Übernahme (`state.go`, neue periodische `captureOneRoleState`). Zwei echte Races live gefunden+behoben: (1) ein noch nicht abgeschlossenes `rewireAfterRestart` eines der fünf erfolgreichen Neustarts VOR dem Crash-Loop-Ende konnte nach einer Übernahme die Runtime auf die tote Primär-Instanz zurückdrehen — neue `stillBacks()`-Freshness-Prüfung, wiederholt vor JEDER einzelnen Connection (ein einziger Check vor/nach der ganzen Schleife reichte nicht, `applyConnection`/`waitForCrosspointInput` kann pro Verbindung selbst ~10-20s blockieren, währenddessen real den `view`-Receiver auf den toten Sender zurückgepatcht — live per direktem `GET .../connection/v1.1/.../active` am Node bestätigt); (2) `promoteStandby`s eigener naiver Freshness-Check schlug immer fehl, weil zu diesem Zeitpunkt selbst noch nicht persistiert war — Fix: Übernahme sofort committen, bevor der langsame Teil beginnt. UI: `role-designer.ts` "Standby für"-Dropdown + `standbyCandidates()` (roles-logic, reine Funktion), `flow-canvas.ts` gestrichelte Standby-Kachel + `workflow.failover`-Toast. Live verifiziert (echter Orchestrator, echte Prozesse, sechs Testläufe bis der Race-Fix stand): Crash-Loop-Trigger (sechsmal `kill -9`) → Übernahme stabil, Bedienzustand (`programSenderId`) exakt übernommen, `view`s Downstream-Connection direkt am Node bestätigt umgeschaltet; Host-Offline-Trigger mit echtem zweiten `omp-host-agent`-Prozess (reales Bootstrap-Token, reale NATS-Telemetrie), Host-Agent hart beendet, nach `hostOfflineTimeout` Übernahme rein über Telemetrie-Stille ausgelöst (kein Crash-Loop-Ereignis nötig) — `omp-source` statt `omp-video-mixer-me` als Redundanzpaar gewählt, da Letzteres durch die vorangegangenen Crash-Tests als "überbucht" profiliert war und von der Placement-Engine korrekt remote abgelehnt wurde (kein Bug, echtes Kapitel-14-Verhalten). `go build/vet/test ./...` + `deno check`/`deno test ui/` grün, alle Test-Workflows/-Prozesse/der zweite Host-Agent danach gestoppt/gelöscht, keine verwaisten Prozesse. Bewusst nicht Teil dieser Sitzung: §19 Orchestrator-Active-Passive (Teil 5), ST-2110-2022-7 (Teil 3), Genlock-Äquivalenz (Teil 6, aspirational), `replicas`/mehr als ein Standby pro Primärrolle (YAGNI). Details: `docs/decisions.md` Nachtrag 113 | 2026-08-03 |
| D8 Teil 3 (Delay-Zuweisung entlang zu kurzer Pfade) | erledigt | Direkte Fortsetzung D8 Teil 1/2. Nutzerentscheidung: `setOutputDelay` diese Runde für BEIDE vermessenen Video-Nodes real implementiert (`omp-scaler` UND `omp-video-mixer-me`, nicht nur der einfachere originerhaltende Scaler-Fall). `omp_mediaio::mxl::MxlVideoOutput`/`write_loop` bekommen einen von außen übergebenen, pipeline-neuaufbau-stabilen `Arc<AtomicU64>` (Signaturänderung an 8+ Aufrufstellen, sechs davon No-Op); Indexberechnung in reine, testbare `compute_write_index`-Funktion extrahiert. **Echter Designfehler live gefunden+behoben:** die erste Fassung fror den Delay-Offset für den ursprungslosen Zweig (Freilauf-Zähler, z. B. Mixer-PGM) nach dem ALLERERSTEN Frame dauerhaft ein — der Orchestrator ruft `setOutputDelay` aber erst NACH `awaitRegistration` auf, ein Node wie der Mixer ("produziert immer etwas", §5 Punkt 6) hat dann oft schon mit Delay=0 geschrieben, das zugewiesene Delay hätte NIE gewirkt (live reproduziert: mehrere manuelle `setOutputDelay`-Aufrufe gegen einen laufenden Mixer zeigten trotz HTTP 200 keinerlei Skew-Änderung). Fix: `delay`/Wallclock-Position werden jetzt bei JEDEM Frame frisch gelesen statt einmalig geankert, Monotonie-Schutz bleibt unverändert. Nebenbefund: `omp-source` hatte gar keine deklarierte Video-Latenz (blinder Fleck seit D8 Teil 1/2, nie mit echter Quelle im budgetierten Pfad getestet) — `0/0` Frames ergänzt (strukturell begründet: eine Testquelle setzt per Definition selbst den Ursprung). Orchestrator: `CatalogLatency.SupportsDelayCompensation` (additiv), neue `delayassignment.go` (`computeDelayPlan` sucht rückwärts die späteste delay-fähige Rolle je zu kurzem Pfad, lehnt fehlende Fähigkeit oder widersprüchliche Defizite derselben Rolle ehrlich ab), zweiter synchroner `Start()`-Preflight neben `checkLatencyBudget`, Anwendung in `runStart` nach den Connections. Live verifiziert per sauberem A/B-Vergleich frisch gestarteter Prozesse (kein manuelles Nachjustieren auf demselben Prozess — das ließ beim ersten Versuch den Monotonie-Schutz auf dem höchsten je gesetzten Wert "einrasten" und täuschte Wirkungslosigkeit vor): Scaler Ø-Skew ohne Delay ≈ -1,9, mit Delay=3 ≈ +1,0, Delta ≈ 3 (exakt); Mixer Ø-Skew ohne Delay ≈ +1, mit Delay=5 ≈ +7, Delta ≈ 6 ≈ 5 (nach dem Fix); isolierte Quelle mit `targetLatencyFrames=3` (kein delay-fähiger Node) → `Start()` HTTP 400, kein Prozess gestartet. Kopf-Index-Skew gegen echte MXL-Flow-IDs (`GET /x-nmos/query/v1.3/flows`, nicht die Sender-IDs — Verwechslung lieferte reproduzierbar "Failed to create flow reader", von Bash-Arithmetik auf leeren Strings zunächst als Falsch-Null maskiert). `cargo build/test/clippy`, `go build/vet/test`, `deno check/test` grün. Bewusst nicht Teil dieser Sitzung: Audio-/Daten-Pfade (D8 Teil 4), Re-Berechnung bei laufender Graph-Änderung, Delay-Neuanwendung nach einem K7-Teil-1-Crash-Neustart (`rewireAfterRestart` ruft `applyDelayPlan` nicht auf — dokumentierte Lücke). Details: `docs/decisions.md` Nachtrag 114 | 2026-08-03 |
| D6 Teil 4 (Automatisierte Placement-Eskalation, README "Offen"-Punkt 1) | erledigt | Nutzerauftrag "Punkt 1 fertig machen" — löst die bisher advisory-only Placement-Engine (D6 Teil 1-3) mit echter Ausführung ein, gemäß dem bereits am 2026-07-13 in `ARCHITECTURE.md` §6.1 spezifizierten Zielbild: pro Workflow-Rolle konfigurierbare Eskalationsstufen (`Role.Placement.Escalation`) statt eines globalen Schalters — advisory (Default, unverändert), auto-confirm-window (Timer, Default 30s, manuelles Confirm/Cancel per API/UI) und auto (sofort). Zwei Nutzerentscheidungen vorab per `AskUserQuestion` geklärt: alle drei Stufen vollständig diese Runde, kein Node-Typ-Ausschluss (State-Transfer bleibt best-effort wie bei K7 Teil 4). Neues `orchestrator/internal/workflows/migration.go`: `executeMigration` ist das eigentliche Make-before-break-Protokoll (§6.1 Punkt 3) — neue Instanz auf dem von der Placement-Engine vorgeschlagenen Zielhost, `awaitFreshRegistration` (aus K7 Teil 4 wiederverwendet), Bedienzustand+Connections umziehen, ERST DANACH die alte Instanz stoppen; vor jedem Seiteneffekt erneuter `stillBacks`-Freshness-Check (gleiche K7-Teil-4-Lektion). `internal/placement` bleibt workflow-unwissend: neues optionales `AdviceObserver`-Interface, von `publishChanges` bei jeder Host-Alarm-Änderung aufgerufen. Idempotenz/60s-Cooldown verhindert Doppel-Trigger pro 5s-Placement-Tick. **Echter Bug live gefunden+behoben** (nicht im Unit-Test sichtbar, erst beim echten `POST`/`PUT /api/v1/workflows`-Roundtrip): `Role.Placement` zunächst als Wert-Struct — Gos `encoding/json` behandelt `omitempty` bei Struct-Werten nie als leer, jede Rolle jeder Workflow-Definition hätte dauerhaft ein bedeutungsloses `"placement":{}` mitgeschleppt. Fix: Pointer-Feld (`*RolePlacementPolicy`) + nil-sichere `Role.PlacementEscalation()`/`PlacementConfirmWindowSeconds()`-Methoden. UI: `role-designer.ts` dritte Dropdown-Zeile pro Rollen-Kachel (gleiches Baumuster wie Format-/Standby-Dropdown), `hosts-view.ts` Live-Countdown-Banner (1s-Ticker) mit "Jetzt ausführen"/"Abbrechen". `go build/vet/test ./...` grün (neue `migration_test.go`, sechs Szenarien: advisory-No-Op, auto-Happy-Path inkl. Make-before-break-Reihenfolge, auto-confirm-window-Ablauf, Confirm, Cancel, OnAdviceCleared-Abbruch — alle über direkte `OnAdviceRaised`-Aufrufe wie `failover_test.go`s Fake-Muster, kein `-race`-Lauf nötig für den Verifikations-Standard dieses Projekts, ein vorbestehender `fakeStore`-Map-Sharing-Race betrifft gleichermaßen bereits bestehende K7-Teil-4-Tests). `deno check`/`deno test ui/` (84/84)/`deno bundle` grün. Live gegen den echten laufenden Orchestrator verifiziert: Login, Wegwerf-Workflow mit `placement`-Feld real angelegt (deckte den Wire-Format-Bug auf) und nach dem Fix erneut bestätigt, `GET /api/v1/placement/advice`/`/migrations` beide echte `200 []`, sauber wieder gelöscht. Bewusst nicht gefahren: ein echter Zwei-Host-Überlast-Roundtrip (deckt sich mit der bereits in §6.1 dokumentierten Testbarkeits-Grenze der advisory-Vorstufe — Single-Host-Dev-Maschine). **Bewusst nicht Teil dieser Runde** (§6.1 selbst nennt sie separat): I/O-Karten-Claim/Release, GPU/NIC-Telemetrie, Cloud-Kostenfaktor-Scoring — README-"Offen"-Liste entsprechend nachgezogen. Details: `docs/decisions.md` Nachtrag 115 | 2026-08-04 |
| PIPELINE CONTROLLER: OMP-MXL-Ausgang startete intermittierend nicht (`gst-mxl-rs`-Bug root-ursächlich behoben) | erledigt | Nutzerauftrag "pipeline controller node startet die pipeline darin nicht, debugge und fixe". Nutzerverdacht (Pfade/UI-Configs) live geprüft und ausgeschlossen. Echter Root Cause: `MxlSink::set_caps` (`third_party/mxl/rust/gst-mxl-rs/src/mxlsink/imp.rs`, gitignored/vendort) rief bei JEDEM Aufruf bedingungslos einen neuen `create_flow_writer` für dieselbe Flow-ID auf — GStreamer ruft `set_caps` legitim mehrfach auf, wenn Caps sich nachträglich präzisieren (traf reproduzierbar nur den Audio-Zweig des `mxl`-Ausgangstyps, `deploy/pipeline-controller/patches/0001-mxl-output.diff`, nie Video mit seinen von Anfang an fixen Caps) — der zweite Aufruf kollidierte mit dem eigenen, bereits aktiven ersten Writer ("the UUID belongs to a flow with another active writer"), gst-mxl-rs machte daraus einen fatalen Pipeline-Fehler statt die Renegotiation zu ignorieren. Fix: `set_caps` jetzt idempotent (überspringt Neuanlage, wenn für diese Pad-Art bereits ein Writer aktiv ist). Zweiter, direkt angrenzender Bug im C++-Kern (`lib/internal/src/FlowManager.cpp`): `createOrOpenContinuousFlow` (Audio-Pfad) räumte das temporäre Flow-Verzeichnis nur im Exception-Fall auf, nicht im normalen "Flow existiert bereits"-Rückgabepfad — anders als der diskrete/Video-Pfad mit unbedingtem `defer`-Cleanup; Fix überträgt dasselbe `defer`-Muster. Eine erste Spur (Race zwischen den beiden `mxlsink`-Elementen bei `garbage_collect_flows()`) wurde per Stresstest widerlegt (unter `GST_DEBUG` sogar mehr Fehlschläge), der dafür eingebaute prozessweite `DOMAIN_LOCK` bleibt als harmlose Absicherung drin, war aber nicht die Lösung. 19/19 echte Container-Boots nach beiden Fixes fehlerfrei (5 davon gegen das frisch gebaute Produktions-Image ohne jeden Bind-Mount-Override), `mxl-info` bestätigt echten laufenden Datenfluss. **Risiko dokumentiert:** `third_party/mxl/` ist komplett gitignored (`deploy/dev/install-mxl.sh` klont bei festem Tag) — ein künftiger Versions-Bump würde beide Fixes stillschweigend zurücksetzen, wie schon bei Nachtrag 78/79 bewusst in Kauf genommen statt eines Wrapper-Workarounds, da die Bugs strukturell im vendorten Sink/Kern liegen. Details: `docs/decisions.md` Nachtrag 116 | 2026-08-04 |
| PIPELINE CONTROLLER: Adapter-Totalausfall (SSE-Deadlock) + falsche MXL-Auflösung root-ursächlich behoben, Ende-zu-Ende gegen echte Playlist verifiziert | erledigt | Direkte Fortsetzung desselben Nutzerauftrags. Zwei GRUNDSÄTZLICH ANDERE Bugs (nicht der Nachtrag-116-Fund) erzeugten trotzdem denselben äußeren Eindruck. **Bug 1:** `omp-pipeline-controller`s Rust-Adapter (`#[tokio::main(flavor = "current_thread")]`, EIN Runtime-Thread) fror beim Laden der eingebetteten Oberfläche KOMPLETT ein, nicht nur `/events` selbst — `ui.html` öffnet beim Laden unbedingt `EventSource('/events')`, `proxy.rs`s `proxy()` las jede Antwort per `read_to_vec()` bis EOF, ein `text/event-stream`-Body endet nie, der eine hängende Read blockierte den GANZEN Adapter (live bestätigt: `/healthz`/`/descriptor.json`/jede PC-Route antwortete auf gar nichts mehr, während PIPELINE CONTROLLERs eigener interner Port per `podman exec` weiter normal antwortete). Fix: `text/event-stream` wird an den Headern erkannt, bevor der Body gelesen wird — sofortiger synthetischer 501 statt endlosem Read; zusätzlich `timeout_recv_body` (10s) als zweite Absicherung. **Bug 2 (der eigentliche Grund für "bunte Linien"):** `intervideosrc channel=rec_pgm_v` (Patch 0001) hatte keine expliziten Caps — beim allerersten Verbindungsaufbau (bevor `intervideosink` in `MasterPipeline.js` den ersten Frame geliefert hatte) negoziierte `videoconvert` gegen einen GStreamer-Fallback statt der echten 1920×1080@25-Master-Auflösung. Live bestätigt (nicht vermutet): `flow_def.json` zeigte `"MXL Test Flow, 240p30"`, `frame_width: 320`, `frame_height: 240` — durch die in Nachtrag 116 gefixte Idempotenz bekommt eine spätere korrekte Neuverhandlung keinen zweiten Flow-Writer mehr, die falsche Auflösung blieb für die gesamte Ausgangs-Laufzeit bestehen, jeder tatsächlich große GStreamer-Buffer wurde in einen viel zu kleinen MXL-Grain-Payload hineinkopiert → diagonal verschobene Streifen beim Lesen. Per Debug-Build isoliert bestätigt: bei 1920×1080 (48-pixel-aligned) stimmen `payload.len()` und `data.buf.len()` exakt überein — kein Stride-Bug in `gst-mxl-rs` selbst, ausschließlich die falsch verhandelte Auflösung war die Ursache; per echtem `mxlsink`→`mxlsrc`-Rundlauf (host-seitig) zusätzlich bestätigt, dass Schreiben/Lesen bei KORREKTER Auflösung byte-exakt korrekt sind. Fix: Auflösung+Framerate werden vom Adapter jetzt explizit mitgegeben (`configure_mxl_output`: neue Felder `width`/`height`/`mxlFrameRateNum`/`mxlFrameRateDen`, neue gemeinsame Konstanten `PROGRAM_WIDTH`/`PROGRAM_HEIGHT`/`PROGRAM_FPS_NUM`/`PROGRAM_FPS_DEN` in `main.rs`, dieselben Werte wie die IS-04-Sender-Registrierung) und direkt nach `intervideosrc` als expliziter Capsfilter erzwungen — rückwärtskompatibel (fehlen width/height, altes unbeschränktes Verhalten). `server.js`s Settings-Whitelist (Patch 0002) um die neuen Felder erweitert, sonst hätte `POST /api/outputs/settings` sie stillschweigend verworfen. **Nutzerwunsch, per Rückfrage bestätigt** (Video-/Audio-Sink-Dropdown, nicht der Ausgangs-Sink-Typ): neuer Patch `0003-headless-sink-restriction.diff` — "Video-Output"/"Audio-Output"-Einstellungen boten bisher Auto/X11/Wayland/XV/PulseAudio/ALSA an, allesamt im headless MXL-only-Container funktionsunfähig; auf die einzige funktionierende Option (`fakesink`) beschränkt. **Alle vier Fixes gemeinsam Ende-zu-Ende gegen eine echte, einfache Playlist verifiziert** (nicht nur Idle-Zustand): `POST /api/playlist/set` mit SMPTE-Event, `POST /api/playlist/start` → `_state:"playing"`, echtes JPEG-Frame aus dem Viewer-Preview zeigt saubere SMPTE-Farbbalken ohne jedes Artefakt, `flow_def.json` zeigt die korrekte 1080p25-Auflösung von der allerersten Verhandlung an. Fünf komplette Container-Boot-Zyklen gegen das finale Produktions-Image, alle fehlerfrei. Nebenbefund: Festplatte lief während der wiederholten Image-Neubauten voll (170GB+ ungenutzte Podman-Images/verwaiste Buildah-Zwischencontainer aus dieser und früheren Sitzungen) — `podman image prune`/`buildah rm --all` schufen Platz, dabei auch einen nie gestarteten (Status "Created", keine Daten) `omp-step-ca`-Container mitentfernt, folgenlos per `make mtls-up` neu anlegbar. `cargo build --workspace --bins`/`cargo build -p gst-mxl-rs` grün. Details: `docs/decisions.md` Nachtrag 117 | 2026-08-04 |
| gst-mxl-rs/C++-Fixes aus Nachtrag 116 als echte Patches versioniert | erledigt | Nutzerauftrag "gst-mxl-rs/C++-Fixes in third_party/mxl/ übernehmen" — schließt die in Nachtrag 116/117 offen dokumentierte Verlustgefahr (lebte nur lokal, wäre bei nächstem `install-mxl.sh`-Versions-Bump stillschweigend verlorengegangen). `third_party/mxl` ist selbst ein echtes Git-Repo (detached HEAD auf dem gepinnten Tag) — `git diff` darin lieferte einen exakten Patch gegen den offiziellen Upstream-Stand. Neues `deploy/dev/mxl-patches/0001-mxlsink-set-caps-idempotent-and-continuous-flow-cleanup.diff` (alle drei Nachtrag-116-Dateien), `install-mxl.sh` wendet ihn nach jedem Clone/Checkout automatisch an — gleiches Muster wie `deploy/pipeline-controller/patches/*.diff`, nur mit `git apply` statt `patch -p1`. Zwei Idempotenz-Absicherungen: `checkout` läuft nur noch, wenn nicht bereits exakt auf dem Ziel-Tag (`git describe --tags --exact-match`); jeder Patch wird per `git apply --check --reverse` auf "schon angewendet" geprüft und dann übersprungen statt mit Fehler abzubrechen. Ein künftiger echter Versions-Bump bricht kontrolliert ab, falls der Patch nicht mehr passt, statt die Fixes still zu verlieren. Live verifiziert: Skript zweimal gegen den bereits gepatchten Checkout gelaufen, erkennt "Version aktuell"/"Patch schon drin" korrekt, voller `cmake`-Rebuild fehlerfrei, `git status` zeigt danach weiterhin genau die drei erwarteten Dateien, `cargo build -p gst-mxl-rs` grün. Details: `docs/decisions.md` Nachtrag 118 | 2026-08-04 |
| PIPELINE CONTROLLER als OMP-Microservice mit echten MXL-Ein-/Ausgängen | erledigt | Nutzerwunsch: "das ganze [PIPELINE CONTROLLER]-Projekt samt all seinen Funktionen wie es ist in ein einzelnes Node packen" — nach Rückfrage präzisiert: **kein** eigenes OMP-Play/Stop/Cue-Interface, PIPELINE CONTROLLERs komplette Bedienung (143 REST-Routen: Playlist/Grafik/Player/Assets/Voiceover/Record/...) bleibt exakt wie bisher dessen eigene Sache, nur das Web-UI zieht als `<iframe>` ins Flow-Editor-Kachelpanel um, echte MXL-Ein-/Ausgänge fürs Routing im DMF-Verbund. Plan `/home/infantilo/.claude/plans/wild-chasing-goose.md`. Vier Teile, alle live verifiziert: **(1) Additiver PIPELINE-CONTROLLER-Patch** (`deploy/pipeline-controller/patches/*.diff`, nur im Container-Build angewendet, die echte Arbeitskopie unter `/home/infantilo/PIPELINE CONTROLLER` bleibt unangetastet, Trennungsgebot `CLAUDE.md`): neuer `mxl`-Sink-Typ in `lib/OutputEngine.js` (symmetrisch zu `rtmp`/`srt`/`udp`/`file`), zwei `mxlsink`-Zweige (Video `format=v210`, Audio `format=F32LE`) — PIPELINE CONTROLLER hatte bisher nur `mxlsrc` (Lesen), nie `mxlsink`. **(2) Containerfile** (`deploy/pipeline-controller/{Containerfile,build.sh}`): Debian-Bookworm + Node 20 + GStreamer-Runtime/-Dev (für `gst-kit`s einmaligen `node-gyp`-Build) + Puppeteer-Abhängigkeiten; nutzt OMPs eigenes, bereits gebautes `gst-mxl-rs`-Plugin (`third_party/mxl/rust/target/debug/libgstmxl.so`) statt PIPELINE CONTROLLERs eigenem `scripts/install-mxl.sh` — vermeidet Versions-/ABI-Drift zwischen zwei unabhängig gebauten `libmxl`-Kopien im selben Domain-Verbund. Zwei echte Bugs beim Live-Verifizieren gefunden+behoben: `PC_DATA_DIR=/data` macht `server.js` von der im Repo mitgelieferten, Host-absolute-Pfade-tragenden Dev-`settings.json` unabhängig — Containerfile schreibt jetzt eine eigene Minimalfassung (`fakesink`/`fakesink`, `authEnabled:false`) plus `audio_config.json` (pro Audiogruppe ein *eigenes* `sink`-Feld, unabhängig von `settings.json.audioSink` — sonst `autoaudiosink`→"Connection refused" ohne PulseAudio im Container); `libmxl.so` wird von `gst-mxl-rs` per bloßem `dlopen("libmxl.so")` geladen — PIPELINE CONTROLLERs eigener `env/mxl.env`-Sourcing-Trick (`process.env.X = …` zur Laufzeit) wirkt nachweislich nur für `GST_PLUGIN_PATH` (GStreamer liest es frisch bei `gst_init()`), nicht für `LD_LIBRARY_PATH` (glibcs dynamischer Lader übernimmt das nicht rückwirkend für spätere `dlopen()`-Aufrufe) — Fix: `LD_LIBRARY_PATH` als Image-`ENV`, nicht nur in `env/mxl.env`. **(3) Launcher-Erweiterung** (`orchestrator/internal/launcher/{catalog.go,podman.go}`): neues, bewusst opt-in `CatalogEntry.MxlAccess`-Feld (Podman-Container bekommen die lokale MXL-Domain sonst grundsätzlich nicht gemountet — reale Vertrauens-Erweiterung ggü. schwach geprüftem Fremdcode, gleicher Grund wie das bewusst fehlende `--network=host`, deshalb kein globales Verhalten). Live per neuem `TestLauncherPodmanMxlAccessMount` verifiziert (echter `podman exec`, nicht nur Kommandozeilen-Konstruktion): mit `MxlAccess:true` sichtbar, ohne nicht. **(4) Neuer Adapter-Node `nodes/omp-pipeline-controller`**: kein Steuer-Proxy — registriert stattdessen zwei echte NMOS-Sender (Programm-Video/-Audio, `omp-source`-Muster) und zwei Receiver (Video/Audio); bei IS-05-Connect löst `livesource.rs` `sender_id`→`flow_id` auf (Registry-Query, gleiches Muster wie `omp-viewer`) und schreibt PIPELINE CONTROLLERs eigene `POST /api/live-sources` per GET-then-merge (andere, manuell konfigurierte Live-Quellen bleiben erhalten); startet `node server.js` selbst als Kindprozess und reicht dessen komplettes Web-UI + REST-API transparent über den einen vom Launcher veröffentlichten Port durch (`proxy.rs`, `ureq::Agent::run` mit `http_status_as_error(false)` — bekannte Einschränkung: PIPELINE CONTROLLERs SSE-Kanal funktioniert durch diesen Proxy nicht, `RawResponse` ist nicht streaming-fähig, Bedienung bleibt aber voll funktionsfähig). UI-Bundle (`ui/bundle.js`) rendert nur ein `<iframe src="{webUiUrl-Param}">` — descriptor.json hat bewusst nur diesen einen Read-Only-Parameter, keine Methoden. **Ende-zu-Ende live verifiziert** (echter Orchestrator, echte Registry, kein Mock): Katalog-Import (`POST /api/v1/catalog`, `mxlAccess:true`) bestand den echten C9-Check; echte `omp-source`→Flow-Editor-Connect (`POST /api/v1/graph/edges`) auf den PC-Receiver — `GET /api/live-sources` am laufenden Container zeigt die korrekt verkabelte `mxl://…`-URI mit denselben Flow-IDs, die `mxl-info` in der echten Domain bestätigt; PC-Programmausgang (SMPTE-Idle mit "PC Test Programm"-Label) auf einen echten `omp-viewer` verbunden — extrahiertes JPEG-Frame aus dem K4-Stream-Proxy zeigt ein echtes 640×360-Farbbalkenbild; `curl` direkt auf den Adapter-Port liefert PIPELINE CONTROLLERs echtes `ui.html` (`<title>Broadcast Controller</title>`) mit `Content-Type: text/html`. `go build`/`go vet`/`go test ./...` (alle Pakete, kein Flake diesmal), `cargo build`/`cargo clippy -p omp-pipeline-controller` sauber. Test-Instanzen/Katalog-Eintrag danach gestoppt+gelöscht+entfernt, keine verwaisten Podman-Container. **Bewusst nicht Teil dieser Sitzung** (im Plan dokumentiert): Mehr-Host-Verallgemeinerung der direkten iframe-Portanbindung (§18, erst relevant bei echter zweiter Maschine), SSE-Live-Push durch den Proxy, weitere PIPELINE-CONTROLLER-Funktionen als OMP-Methoden (laut Nutzervorgabe dauerhaft nicht geplant), Mehrkanalfähigkeit (`supervisor.js` — ein Katalog-Eintrag/Container = ein Kanal, mehrere Importe decken das bereits ab) | 2026-07-31 |
| PIPELINE CONTROLLER: SSE-Events-Proxy + Format-Pin + Live-Quellen-Sync (Vorsitzung) live verifiziert, echter gst-kit-Ressourcenleak (`Pipeline.stop()` bestätigt kein `GST_STATE_NULL`) gefunden+behoben (Patch 0007) | erledigt | Direkte Fortsetzung von Nachtrag 116-118. Echter Image-Build + Instanz-Start über den Orchestrator, alle drei Vorsitzungs-Fixes (0004 Format-Pin, 0005 Live-Quellen-Sync, 0006 SSE-Events via `ExtraPort`) einzeln gegen die echte Instanz verifiziert (u. a. CDP-Browser-Check von `ui.html`s SSE-Status). Beim Verifizieren echten, sitzungsübergreifend akkumulierenden GStreamer-Thread-Leak gefunden (`gst-kit`s `Pipeline.stop()` resolved bei Zustandswechsel-Timeout statt zu rejecten, PC prüfte das nie) — neuer Patch `0007-pipeline-stop-async-timeout-leak.diff` (Ergebnis-Check + Retry mit längerem Timeout an den drei wichtigsten Stellen), Thread-Zahl nach Fix über drei Neuaufbau-Zyklen stabil (vorher: unbegrenzt wachsend). | 2026-08-06 |
| Neuer Node: `omp-mxf-player` (MXF-Player mit ORF-Programmgruppen-Audio-Shuffle, "Audio Tonspurerweiterung Q3 2026") | erledigt | Nutzerauftrag (Vorsitzung, ORF-intern, `Audio Tonspurerweiterung Q3 2026 PD.pdf`) — 8 MXF-Mono-Tonspuren, 13 offizielle Presets, fünf gleichzeitig aktive MXL-Sender (Programmton/Hörfilm-AD/Originalton/Dolby-E/5.1-diskret) + ein Video-Sender. `filesrc ! mxfdemux` statt `uridecodebin` (reale ORF-MXF-Dateien: 8 separate Mono-Essenzspuren, `ffprobe`-verifiziert). Live gegen echte ORF-Testdatei verifiziert: Preset-Routing per Pad-Probe UND per externem `mxlsrc`-Consumer bestätigt (unterschiedliche Presets routen tatsächlich unterschiedliches echtes Audiosignal auf unterschiedliche Sender, nicht nur wirkungslose Matrix-Properties). Kein Zusammenhang mit dem separaten, noch offenen K2-Teil-2(MXF)-Punkt im regulären Phasenplan. | 2026-08-06 |
| `omp-mxf-player`: Programmgruppen/Shuffle-Presets Postgres-gestützt statt hartcodiert (dynamische Node-Typ-Einstellungen) | erledigt | Nutzerwunsch "Shuffle Presets und Output Groups dynamisch definieren, für einfachere künftige Anpassungen". Neue generische `node_type_settings`-Tabelle (Migration 0013, gleiches Blob-Muster wie `catalog_entries`/`layouts`) + `GET`/`PUT /api/v1/node-types/omp-mxf-player/settings` (PUT admin-only, Ganz-Dokument-Ersatz mit Schema-Validierung). Node lädt beim Start per Service-Token vom Orchestrator (ARCHITECTURE.md §24.1-Muster, wie `omp-playout-automation`), fällt bei jedem Fehler auf die bisherigen ORF-Standardwerte zurück. Programmgruppen wirken restart-on-apply (neue NMOS-Sender nur bei Instanzstart), Presets sofort für neu gecute Items. Neue Editor-Sektion im generischen Flow-Editor-Panel (Node-Typ-Erkennung über charakteristische Params, kein neuer Framework-Mechanismus), Ganz-Dokument-Save statt PATCH-per-Feld. Live per echtem CDP-Browser-Test verifiziert (Gruppe per UI hinzugefügt, gespeichert, frische Instanz zeigt den zusätzlichen NMOS-Sender). Nebenfund beim Verifizieren: echter `mxfdemux`-Plugin-Thread-Leak bei wiederholtem `cue()`/`take()` (dieselbe Bug-Klasse wie der `gst-kit`-Fund derselben Sitzung) — `pipeline.rs`s Teardown-Pfad wartet jetzt auf den bestätigten `GST_STATE_NULL`-Abschluss statt ihn zu verwerfen (mildert das Problem deutlich: vorher scheiterte Zyklus 2 fast immer, jetzt erst Zyklus 5), der verbleibende Rest sitzt nachweislich im Plugin selbst (`mxfdemux3:sink`/`4:sink`-Threads überleben ihren eigenen bestätigten NULL-Übergang) und bleibt als offener Folgepunkt dokumentiert. | 2026-08-06 |
| `omp-viewer`: dynamische Audio-Eingänge (Pegelanzeigen) + `omp-audio-monitor` komplett redesignt (PCM/AudioWorklet statt MP3/`<audio>`) | erledigt | Nutzerwunsch "Pegelanzeigen im Viewer, dynamische Eingangszahl (mxf-player-Gruppen routbar), Audio-Monitor ohne HTML-Player, Gruppenwahl-Dropdown". `omp-node-sdk::NodeHandle::add_receiver`/`remove_receiver` neu (erste dynamische Receiver-Fähigkeit in OMP, `Device::version`-Refresh-Bug live gefunden+behoben), `omp_mediaio::levels` aus `omp-audio-mixer` gehoben (zweiter Verbraucher), `omp_mediaio::pcm_stream` neu (roh statt MP3). `omp-audio-monitor` vollständig live verifiziert (2026-08-06). Der damals offene Rest-Bug (`omp-viewer`s Meter-Datenfluss lieferte nie Werte) ist am 2026-08-07 root-caused+behoben: kein MXL-/GStreamer-Problem, sondern ein Rust-Ownership-Bug — `audio_meters.rs::Branch` hielt nur die geklonten `gst::Element`s, nicht den `MxlAudioInput`-Wert selbst, dessen `Drop` den frisch gestarteten Reader-Thread nach der allerersten Iteration lautlos beendete (`omp-audio-mixer::pipeline::ChannelBranch`s `_external_input`-Muster fehlte hier). Live voll verifiziert (Add→Read→Remove→Re-Add-Zyklus, echte `/levels`-SSE-Werte, Video-Pfad gegenverifiziert). Details docs/decisions.md 2026-08-06/2026-08-07 (Nachtrag 123). | 2026-08-07 |
| UX/UI-Zero-Training-Audit + Enterprise-Produktionsreife-Roadmap (Nutzerauftrag) | teilweise | Zwei kombinierte Rollen (UX/UI-Experte + autonomer Lead-Developer/Co-Product-Owner). Rahmenbefund: "Zero-Training" ist für die meisten OMP-Oberflächen die falsche Latte (professionelles Broadcast-Vokabular ist für trainierte Operator:innen korrekt) — echte Relevanz liegt in Fehlertoleranz. Konkret behoben, in zwei Runden: fünf destruktive Aktionen ohne/mit unpassender Bestätigung (Rundown-Item/Cart/Audio-Kanal entfernen, Stop→Schwarzbild, `omp-player` Video-/Jingle-Playlist entfernen) bekamen das bereits vorhandene `<omp-confirm>`-Kit-Element statt `window.confirm()`/gar keiner Bestätigung — ALLE eigenständigen Node-UI-Bundles systematisch geprüft, zwei bewusste Nicht-Treffer dokumentiert (`crosspoint.unpin`, `removeAudioInput` — kein Signalfluss betroffen). Live per CDP verifiziert (beide Runden). Roadmap: höchste Priorität ist fehlendes `defer recover()` um sämtliche Orchestrator-Hintergrund-Goroutinen (0 Treffer für `recover()` im ganzen Orchestrator — ein Panic in `launcher.supervise`/`workflows.runStart`/`failover.tryPromoteForInstance`/etc. reißt den GANZEN Prozess mit), zweiter Punkt fehlende Liveness-Überwachung interner Rust-Worker-Threads. Größere Vorschläge (Setup-Wizard, `recover()`-Wrapper selbst) bewusst nur dokumentiert, nicht blind umgesetzt — nächster Schritt offen. | 2026-08-07 |
| Orchestrator-Shell (Engineering-UI) modernisiert (Nutzerauftrag "orchestrator ui muss auch professionell, modern und einfach werden") | teilweise | `ui/design-tokens.css` (Studio-Dark-Theme) existierte bereits vollständig, wurde aber in der Shell kaum genutzt (`shell.ts`/`console-board.ts` 0 Treffer, `flow-canvas.ts` nur 9) — roher Browser-Default-Look trotz durchdachtem Token-System. Höchster Hebel: `ui/shell/*.ts`+`flow-canvas.ts` rendern ausnahmslos ins Light-DOM (kein `attachShadow`), ein globaler `button`/`select`/`input`/`textarea`-Reset in `design-tokens.css` reskinnt dadurch die GESAMTE Shell auf einen Schlag, ohne Shadow-DOM-Grenzen-Problematik und ohne `ui/kit/omp-button.ts`s bewusst andere "Hardware-Taste"-Optik für Operator-Konsolen zu berühren. Danach gezielter Feinschliff: Login-Screen/User-Widget (`auth.ts`, vorher hartkodiertes `sans-serif`/Hex statt Tokens), Workflow-Filter-Select (`app-shell.ts`), Node-Katalog/Buttons im Standard-Tab (`flow-canvas.ts`, restliche selten geöffnete Panels bewusst als Folgepunkt offen gelassen), `.omp-btn-danger` für alle "Löschen"-Buttons workflow-/admin-weit, `.omp-h1`/`.omp-empty` für Überschriften/Leerzustände in allen sechs View-Tabs, drei hartkodierte Akzentfarben in `scheduler-view.ts` → Tokens. `deno check`/`deno test` grün (84/84), live per CDP alle sieben Tabs verifiziert (`Network.setCacheDisabled` nötig, sonst servierte der Browser eine gecachte alte `design-tokens.css` — live gefundener Test-Fallstrick). Details docs/decisions.md Nachtrag 125. | 2026-08-07 |
| Roadmap Punkt 1: `safego.Go` (Panic-Recovery für alle zehn Orchestrator-Hintergrund-Goroutinen) | erledigt | Löst den in der UX-Audit-Roadmap gefundenen höchste-Priorität-Punkt (0 Treffer für `recover()` im ganzen Orchestrator). Beim Verifizieren (`go test ./...` mit `OMP_POSTGRES_URL` auf die echte Dev-Postgres — die von `make test` selbst vorgesehene Nutzung) echten Datenverlust ausgelöst: `workflows/store_test.go`s Test-Setup leerte bedingungslos die komplette `workflows`-Tabelle, "Regieplatz 1"/"PC-MXL-Test" waren weg, kein brauchbares Backup vorhanden (Nutzerentscheidung: selbst neu anlegen, kein Rekonstruktionsversuch). Root Cause sofort behoben (neues Paket `internal/dbtest`: Tests verbinden jetzt IMMER zu einer von der Ziel-DSN abgeleiteten, isolierten `<db>_test`-Datenbank, nie zur echten App-Datenbank — strukturell erzwungen, nicht nur Konvention), betraf `workflows`/`snapshots`/`launcher` (`instances`+`catalog_entries`); alle anderen DB-Test-Pakete bereits korrekt scoped, nie betroffen. Live verifiziert: derselbe Vorfall-auslösende Befehl erneut ausgeführt, `omp`-Datenbank nachweislich unverändert. Details docs/decisions.md Nachtrag 128. | 2026-08-07 |
| Bugreport "Nodes verlieren Settings/UID beim Workflow-Neustart" behoben | erledigt | Zwei Root Causes. (1) `captureRoleState`/`restoreRoleState` griff nur bei Nodes mit eigener `/state`-Route (bisher nur die zwei Mixer) — generischer Fallback auf alle schreibbaren Parameter (`descriptor.json`+`/params/<name>`) ergänzt, deckt jetzt z. B. `omp-source::pattern` ab; rückwärtskompatible neue `role_state`-Hülle (`roleStateEnvelope`). (2) `node_id`/`device_id` wurden bei jedem Prozessstart neu gewürfelt, auch bei reinem Workflow-Neustart — `omp-node-sdk::node::start` leitet sie jetzt deterministisch aus einem neuen `OMP_ROLE_SEED`-Env-Var her (Orchestrator setzt ihn nur in `runStart`/`runRestartRole`, NICHT in `executeMigration`, wo die alte Instanz während des Starts der neuen weiterläuft). Live an TEST1 verifiziert: `pattern` überlebte zwei Stop/Start-Zyklen inkl. Node-Rebuild, alle 5 Rollen-Node-IDs blieben über beide Zyklen identisch. Details docs/decisions.md Nachtrag 129. | 2026-08-10 |
| Roadmap Punkt 2: generische Worker-Thread-Liveness-Überwachung (Rust-Nodes) | erledigt | Löst den zweiten Punkt aus der Nachtrag-123-Roadmap. `health::Status.status` war seit jeher hartkodiert `"ok"`; `media_ready` ist sticky und kann die Bug-Klasse "Thread stirbt still, kein Panic" strukturell nicht erkennen. Neues Modul `omp-node-sdk::liveness` (`LivenessMonitor`, generisch, additiv), `omp-mediaio::mxl`s 4 interne Reader-/Schreiber-Threads bekommen einen `heartbeat`-Zähler (tickt jeden Durchlauf, unabhängig von `flowed`). `heartbeat_loop` meldet `status:"degraded"`, sobald ein registrierter Worker seit dem letzten 5s-Tick nicht vorankam. Referenz-Verdrahtung am historischen Bug-Beispiel: `omp-viewer`s Audio-Meter-Reader-Thread UND dessen äußere Schleife. Live per `nats sub omp.health.<id>` verifiziert: `status` bleibt `"ok"` sowohl im Ruhezustand als auch während ein echter Audio-Meter-Zweig lief (`addAudioInput`→Verbindung→`/levels`-SSE lieferte echte Events) — kein Fehlalarm. Restliche ~15 Node-Typen mit `pipeline::run`-Thread noch nicht angeschlossen (dokumentierter Folgepunkt, Mechanismus selbst fertig). Details docs/decisions.md Nachtrag 130. | 2026-08-10 |
| Worker-Thread-Liveness auf alle Node-Typen ausgeweitet (Nutzerauftrag "alle node types an livenessmonitor") | erledigt | Direkte Fortsetzung von Nachtrag 130. 18 Node-Crates/34 Dateien: elf Nodes mit dem einheitlichen `pipeline::run`-Muster (mechanisch identisch), plus Sonderfälle mit eigener Verdrahtung — `omp-2110-gateway` (zwei Rollen-Funktionen), `omp-aes67-gateway` (zwei Pipeline-Rollen + dauerhafter SAP-Announce-Thread, Heartbeat bewusst in der inneren 200ms-Schleife statt nur einmal pro bis zu 30s-Announce-Intervall, sonst falscher Fehlalarm), `omp-audio-monitor`/`omp-multiviewer` (Inline-Closures), `omp-fabrics-gateway` (kein GStreamer, reine Fabrics-Relay-Threads in `omp_mediaio::fabrics`, dortiger echter TCP-Test mit angepasst), `omp-srt-gateway` (einzige echte Verhaltensänderung: `bus.iter_timed(ClockTime::NONE)`, unbegrenzt blockierend, auf dieselbe `timed_pop`-1s-Poll-Kadenz wie der Rest des Codebase umgestellt — funktional unverändert, aber jetzt tick-fähig auch bei ruhigem Bus). Bewusst offen gelassen: `omp-ograf`s zwei interne Zusatz-Threads (Node-Typ selbst schon über seinen primären Thread abgedeckt) und die gemeinsam genutzten `omp_mediaio`-HTTP/SSE-Annahme-Threads (Preview/Levels/Audio-Stream, kein Kern-Medienpfad). Live an TEST1 verifiziert: fünf verschiedene Node-Typen gleichzeitig gestartet, alle `status:"ok"`; `omp-viewer`-Audio-Meter-Pfad erneut end-to-end mit jetzt zwei gleichzeitig registrierten Workern durchgespielt, weiterhin `"ok"`. `cargo build --workspace --bins` + `cargo test --workspace --lib` komplett grün. Details docs/decisions.md Nachtrag 131. | 2026-08-10 |
| omp-ografs zwei restliche interne Threads an LivenessMonitor angeschlossen | erledigt | Schließt die letzte in Nachtrag 131 offen gelassene Lücke. Alpha-Key-Brücken-Thread: Standardmuster, Zähler durch `Pipeline::build` bis `PipelineHandle` durchgereicht. GLib-`main_loop.run()`-Thread: kein editierbarer Schleifenkopf (ein einziger blockierender Aufruf) — stattdessen periodischer `gst::glib::timeout_add(1s, ...)` auf dem Default-`MainContext`, VOR dem `thread::spawn` registriert; feuert nur, wenn der Thread den Context tatsächlich dispatcht (erste Nutzung von `glib::timeout_add` in diesem Codebase). Live verifiziert: eigenständige Katalog-Instanz, drei aufeinanderfolgende 5s-Health-Ticks beobachtet, durchgehend `status:"ok"` mit allen drei Workern (`pipeline`/`alpha-key-bridge`/`glib-main-loop`). `cargo build --workspace --bins` + `cargo test --workspace --lib` grün. Damit sind alle Node-internen Threads bis auf die gemeinsam genutzten `omp_mediaio`-HTTP/SSE-Annahme-Threads abgedeckt. Details docs/decisions.md Nachtrag 132. | 2026-08-10 |
| Gemeinsam genutzte omp_mediaio-HTTP/SSE-Annahme-Threads an LivenessMonitor angeschlossen | erledigt | Schließt die letzte offene Lücke. Fünf Module (`preview`/`levels`/`audio_stream`/`pcm_stream` in `omp_mediaio`, plus `omp-pipeline-controller::sse_proxy`) nutzten `Server::incoming_requests()` (blockiert unbegrenzt) — dieselbe Einschränkung wie bei `omp-srt-gateway` (Nachtrag 131): ein ruhiger, gesunder Listener ohne verbundene Clients hätte nie getickt. Umgestellt auf `Server::recv_timeout(1s)`, funktional unverändert für jeden tatsächlich verbindenden Client. Live verifiziert: `omp-viewer`s `/preview` liefert weiterhin echte JPEG-Frames, `omp-audio-mixer`s `/levels` weiterhin echte SSE-Events, `omp-audio-monitor`s `/pcm-stream` liefert sofortiges `200 OK`; `status:"ok"` über mehrere Health-Ticks mit den neuen Workern (`preview-accept`/`levels-accept`/etc.) stabil. `cargo build --workspace --bins` + `cargo test --workspace --lib` grün. Damit sind jetzt wirklich alle Node-internen Hintergrund-Threads erfasst. Details docs/decisions.md Nachtrag 133. | 2026-08-10 |
| Security/Auth-Hardening (ARCHITECTURE.md §20.4): vier Lücken behoben, §20.4 selbst korrigiert | erledigt | Bestandsaufnahme zeigte §20.4 als veraltet — D3 (mTLS + §12-Nutzer-/Rollenmodell) ist seit 2026-07-13/14 fertig, nicht der beschriebene unscheduled/Stub-Zustand. Vier konkrete Lücken behoben: (1) `POST /api/v1/auth/login` unbegrenzt oft aufrufbar — neues `internal/auth.LoginLockout` (in-memory, testbare `now`-Injektion) sperrt nach 5 Fehlversuchen/15min für 15min. (2) Login-Ereignisse landeten nie im Audit-Log — `handleLogin` protokolliert jetzt 200/401/429 inkl. Nutzername. (3) `?access_token=`-Fallback galt für JEDEN Endpunkt statt nur die drei, die ihn brauchen (SSE, natives `import()` für Node-UI-Bundles, Node-Stream-Proxy) — explizite Pfad-Allowlist statt Header-Heuristik; eine erwogene `Accept: text/event-stream`-Heuristik hätte den `import()`-Fall live gebrochen, vor dem Ausrollen per Grep über alle Node-UI-Bundles gefunden. (4) §20.4 selbst korrigiert. Live verifiziert: 5×401+429-Sperre gegen echten Dev-Orchestrator, Audit-Log bestätigt per SQL, Allowlist positiv+negativ gegen echte Node-Instanz getestet. `go build`/`go vet`/`go test ./...` grün inkl. neuer Tests. Details docs/decisions.md Nachtrag 134. | 2026-08-10 |
| Security/Auth-Hardening fortgesetzt: Token-Revocation, CORS bestätigt, Bypass-Sweep | erledigt | Fortsetzung von Nachtrag 134 (Punkte 5-9 der ursprünglichen Lücken-Liste). Bypass-Sweep über alle verbleibenden `PeerClient`-Aufrufstellen: keine neuen Funde, nur Low-Blast-Radius-Aktionen. CORS geprüft: kein Endpunkt setzt `Access-Control-*`-Header, bereits sicherer Default, keine Code-Änderung. Token-Revocation: `users.sessions_epoch` (neuer monotoner Zähler pro Nutzer, fest im JWT eingebettet, exakter Abgleich bei jedem Request) statt eines ersten, verworfenen Zeitstempel-Entwurfs — der brach live an einem echten Race (JWT-`iat` hat nur Sekundenauflösung, Postgres `now()` Mikrosekunden, ein Login unmittelbar nach einem Widerruf wurde fälschlich abgelehnt), per Testlauf gegen echte Postgres gefunden, nicht durch bloßes Nachdenken. Neuer Endpunkt `POST /api/v1/auth/users/{name}/revoke-sessions`; Passwort-Reset widerruft jetzt automatisch alle Sitzungen mit. mTLS-für-Rust-Nodes/NATS-Verschlüsselung (Punkte 7/8) nach kurzer Aufwandsrecherche bewusst auf eigene Sitzung verschoben (Nutzerentscheidung). Live gegen echten Dev-Orchestrator verifiziert: sofortiger Neu-Login nach Widerruf ohne Wartezeit funktioniert. `go build`/`vet`/`test ./...` grün. Details docs/decisions.md Nachtrag 135. | 2026-08-10 |
| Rollen-Designer: Speichern-Button auf `.omp-btn-primary` (letzter Politur-Punkt der UX/UI-Audit-Reihe) | erledigt | `role-designer.ts` hat kein "dirty"-Tracking wie `flow-canvas.ts`s Workflow-Bearbeiten-Speichern-Button — immer im aktiven Entwurfsmodus, daher immer primär statt bedingt. MXF-Preset-Editor bereits seit Nachtrag 127 korrekt (Gap-Notiz war veraltet); "×"-Entfernen-Glyph ist SVG-`<text>`, kein `<button>`, `.omp-btn-danger` greift dort nicht — bewusst unverändert (Design-Zeit-Entwurf, kein Live-Zustand). `deno check`/`deno test ui/` grün (84/84), auf CDP-Test verzichtet (reine CSS-Klasse, gleiche Klasse bereits mehrfach live verifiziert). Details docs/decisions.md Nachtrag 136. | 2026-08-10 |
| Kapitel 13 Teil 1: Host-Zonen im Flow-Editor (Zonen-Rendering, Live-CPU/RAM, Toolbar-Toggle, Auto-Default ab 2 Hosts) | erledigt | Reines Frontend-Rendering (§13.1: Host-Zuordnung existiert serverseitig bereits vollständig, `Instance.HostID`, kein neuer Endpunkt nötig). Feste vertikale Lanes (Nutzerwahl §13.5 Frage 1), Auto-Default ab 2 registrierten Hosts (Nutzerwahl §13.5 Frage 2). Zwei getrennt gemerkte Positions-Sätze für Root-Node-Kacheln (`LayoutBlob.positions` bleibt immer das freie Layout, neues `LayoutBlob.hostViewPositions` die separat gemerkte Lane-Anordnung) ohne Umbau der ca. 50 bestehenden `#positions`-Zugriffsstellen — Toggle tauscht nur den Inhalt der Root-Node-Einträge, `#saveLayout()` trennt beim Speichern wieder korrekt. Gruppen-/Workflow-Kacheln bewusst ohne Zonen-Zuordnung (können mehrere Hosts aggregieren). Live gegen echten Orchestrator/Postgres/NATS verifiziert: zweiter echter `omp-host-agent`-Prozess registriert, echte Remote-`omp-source`-Instanz gestartet, per CDP (Deno-natives WebSocket) im echten Headless-Chromium bestätigt — Auto-Default korrekt aktiv, vier Zonen (lokal/zwei Hosts/Unzugeordnet) mit korrektem Online-Punkt (lebendiger vs. beendeter Host-Agent) und live CPU/RAM, Remote-Kachel korrekt in ihrer Host-Lane, Toggle aus/an mit korrektem Positions-Wechsel bestätigt per `GET /api/v1/layouts/default`. `deno check`/`deno test ui/` grün (84/84). Teil 2 (Kanten-Klassifizierung/MXL-Warnstil, Zonen-Kollaps) und Teil 3 (Drag = begleiteter Host-Umzug) bewusst nicht Teil dieser Sitzung, im Dokument bereits vorgeplant. Details docs/decisions.md Nachtrag 137. | 2026-08-10 |
| ARCHITECTURE.md: letzte Nennungen eines konkreten kommerziellen Cloud-Produktionsplattform-Produktnamens entfernt (Nutzerauftrag, §20.7-Konvention) | erledigt | §1/§9/§10 hatten trotz des 2026-07-11-Vendor-Neutralitäts-Passes noch vier Nennungen eines einzelnen Produktnamens (Vision-Beispiel, MXL-Kompatibilitäts-Werbeaussage, Austauschbarkeits-/Drift-Risiko-Beispiele) — entfernt, Kernaussagen ohne Produktnamen erhalten. Generische Hersteller-Sammel-Nennungen (Tiger-Team-Mitgliedschaft neben mehreren anderen Großvendoren, ein Hardware-Interop-Beispiel, eine Presse-Quelle) bewusst unangetastet, gleiche engere Auslegung wie beim 2026-07-11-Pass. Historische Zitate in `docs/decisions.md` bleiben Append-Only unverändert. Details docs/decisions.md Nachtrag 138. | 2026-08-10 |
| Audit-Log-Pagination-Test: fester Seiten-Deckel durch tatsächlichen Tabellenbestand ersetzt (echten CI-Fund behoben) | erledigt | Beim Prüfen "ist alles getestet, was wir haben" (Nutzerfrage) `go test ./...` gegen echte Dev-Postgres laufen lassen — `TestListCursorPaginatesThroughAllEntries` schlug real fehl ("too many pages"). Root Cause: die geteilte Dev-`audit_log`-Tabelle ist über die Projektlaufzeit auf 3225+ Zeilen gewachsen (u. a. durch diese Sitzung eigene Login-Lockout-/Revocation-Live-Tests), der Test paginiert aber bis zum tatsächlichen Tabellenende — der hartkodierte Deckel (200 Seiten à 10) war für den heutigen echten Bestand längst zu klein, keine Pagination-Logik-Regression. Fix: Deckel wird jetzt aus `SELECT count(*)` zur Testlaufzeit abgeleitet statt geschätzt — wächst die Tabelle weiter, bricht der Test nicht erneut. `go test ./internal/audit/...` grün, kompletter `go test ./... -p 1` gegen echte Dev-Postgres ebenfalls grün. Bei derselben Gelegenheit `nodes`-Rust-Workspace (`cargo test --workspace --lib`, mit `deploy/dev/mxl.env` gesourced) geprüft: ein vereinzelt geflaktes `three_concurrent_readers_same_flow_do_not_hang` (ein Reader erhielt 0 statt ~125 Frames) trat nur einmal bei voller `--workspace`-Parallellast auf, 8 weitere gezielte Wiederholungen (isoliert und mit `--features mxl,fabrics`, seriell und parallel) liefen alle sauber durch — deutet auf CPU-Kontention unter Volllast, nicht auf eine reale MXL-Reader-Race, aber noch nicht abschließend robust gemacht (offen für eine künftige Sitzung, kein Code-Fix heute). | 2026-08-10 |
| Kapitel 13 Teil 2: Kanten-Klassifizierung (MXL-Warnstil über Zonengrenzen) + Zonen-Kollaps | erledigt | Nutzerauftrag "starte wo wir zuletzt aufgehört haben" → Teil 2, plus live gemeldeter Zusatzbug "gruppen nodes werden nicht im host gruppen fenster angezeigt". Bug root-caused per CDP-Test und behoben: B5-Gruppen-Kacheln bekommen jetzt eine Zone (eindeutig → deren Lane, uneinheitlich → neue Pseudo-Zone "Gruppen über mehrere Hosts"), vorher blieben sie komplett außerhalb der Zonen-Darstellung hängen (`#rootNodeTileIds`/`#rootNodeTiles` → `#rootZoneTileIds`/`#rootZoneTiles`, neue `#zoneIdForGroup`). Transport-Feld neu durch den ganzen Stack durchgereicht (IS-04 `transport` → `registry.SenderView`/`ReceiverView` → `graph.Port` → UI) für die Kanten-Klassifizierung: eine Kante zwischen unterschiedlichen Zonen mit MXL-Sender rendert gestrichelt in `var(--omp-error)` mit §13.3-Tooltip. Zone einklappbar (▾/▸ im Zonen-Kopf, session-lokal): versteckte Kacheln bekommen bewusst keine `#portLocation`-Eintragung, wodurch `#renderEdge()`s bestehender Guard ihre Kanten automatisch mit ausblendet, ohne eigene Kanten-Filterlogik. Nebenfund beim Verifizieren mit einem echten zweiten Host: eine gemerkte Lane-Position landete nach einer Zonen-Reihenfolgen-Verschiebung (neuer Host registriert) optisch in der falschen Nachbar-Lane — `#arrangeIntoLanes()` prüft jetzt, ob die gemerkte Position tatsächlich innerhalb der aktuellen Lane-Grenzen liegt. `go build`/`go vet`/`go test ./...` (orchestrator+host-agent) und `deno check`/`deno test ui/` (84/84) grün. Live gegen echten Orchestrator/Postgres/NATS mit echtem zweiten `omp-host-agent` per CDP verifiziert: lokale Gruppe korrekt in lokaler Lane, gemischte Gruppe korrekt in neuer Pseudo-Zone ohne fälschlichen Online-Punkt, echte hostübergreifende MXL-Verbindung korrekt im Warn-Stil (`getComputedStyle` bestätigt `rgb(239, 83, 80)`), Zonen-Kollaps versteckt/zeigt Kachel+Kante nachweislich, Stale-Lane-Fix bestätigt. Teil 3 (Drag = begleiteter Host-Umzug) weiterhin offen, hängt an ungeklärter §13.5 Frage 3. Details docs/decisions.md Nachtrag 139. | 2026-08-12 |
| Kapitel 13 Teil 3: Drag = begleiteter Host-Umzug (eigenständige Nodes) | erledigt | §13.5 Frage 3 per Rückfrage beantwortet ("ja, bauen"). Architektur-Fund beim Verdrahten: ein laufender Workflow zeigt sich in der Host-Ansicht immer als EINE kollabierte Kachel (Nutzerentscheidung 2026-07-26) — einzelne Rollen-Kacheln erscheinen dort nie individuell und sind daher kein erreichbares Drag-Ziel; §13.3s "Rahmen zeichnet über Zonengrenzen" beschrieb noch das ältere §12.3b-Modell. Per zweiter Rückfrage Scope auf eigenständige (nicht Workflow-gebundene) Nodes verengt statt die Host-Ansicht für laufende Workflows grundlegend zu erweitern. Zwei unabhängige Backend-Pfade: `workflows.Service.MigrateRole` (Workflow-Rollen, nutzt das bestehende D6-Teil-4-Make-before-break-Protokoll `executeMigration`, fertig+getestet, aber noch OHNE UI-Anschluss — Folgearbeit) und neues Paket `orchestrator/internal/instancemigrate` (eigenständige Instanzen: Kanten aus dem Live-Graph erfassen, stoppen, auf Zielhost neu starten, Registrierung abwarten, Kanten über Port-Rolle/Index statt ID neu verbinden — an die UI angeschlossen). Frontend: `#handleZoneMigrationDrop()` erkennt per neuem `#zoneIdAtX()`-Lane-Hit-Test einen Zonenwechsel beim Drop, `confirmDialog()` (gleiches `<omp-confirm>`-Muster wie `#stopInstance`), `POST /api/v1/instances/{id}/migrate`. `go build`/`go vet`/`go test ./...` (inkl. 8 neuer Tests über beide Pfade) und `deno check`/`deno test ui/` (84/84) grün. Live gegen echten Orchestrator mit echtem zweiten `omp-host-agent` per CDP verifiziert: Drag der Viewer-Kachel in die Zone des zweiten Hosts → Bestätigungsdialog mit korrektem Text → nach Bestätigen alte Instanz gestoppt, neue mit `hostId` des Zielhosts, gleiches Label übernommen, Kante automatisch neu verbunden (bestätigt per `GET /api/v1/graph`); migrierte Kachel erscheint in korrekter Zonen-Lane, zonenübergreifende MXL-Kante rendert im Teil-2-Warnstil, Live-Vorschau der migrierten Instanz läuft. Zwei CDP-Testwerkzeug-Gotchas dabei gefunden (Testinfrastruktur, kein Produktbug): synthetische `PointerEvent`s haben keinen aktiven Pointer für `setPointerCapture` (echte Maus-/Touch-Gesten funktionieren einwandfrei) — CDP-natives `Input.dispatchMouseEvent` nötig; die Katalog-Palette liegt z-index-über dem Canvas, ein Klickpunkt hinter ihr trifft die Palette statt die Kachel. Details docs/decisions.md Nachtrag 140. | 2026-08-12 |
| README/Benutzerhandbuch aktualisiert (bessere Feature-Liste + ehrliche Grenzen-Liste), LICENSE ergänzt, Copyright-Prüfung | erledigt | Zwei Copyright-Funde VOR jeder Doku-Arbeit per Rückfrage geklärt: kein LICENSE-File trotz "Open Source"-Positionierung (Apache-2.0 gewählt, Text wortgleich aus dem bereits vorhandenen `third_party/mxl/LICENSE.txt` übernommen), unreferenzierte "OpenMediaPlatform CI.jpeg" (Studio-Fotos unklarer Herkunft) entfernt. Dritter, schwerwiegenderer Fund WÄHREND der Screenshot-Arbeit: das OGraf-Panel zeigte im Operator-Konsole-Screenshot standardmäßig ein importiertes (gitignored, nie committetes) Drittanbieter-Template (`02700111`, echte deutsche Broadcast-Fachbegriffe als Feldnamen) statt des eigens dafür vorhandenen `hello-lower-third` — `show()` setzt nur den On-Air-Zustand, nicht die Panel-eigene Dropdown-Auswahl; per Shadow-DOM-Traversal im laufenden Browser korrigiert, Screenshot neu geschossen. Alle elf dokumentierten Screens neu aufgenommen (CDP, 1600×1000) gegen ein realistisches Demo-Setup (Workflow „Regie 1", sechs Rollen, `operator1`-Rollenbindung, zweiter echter Host-Agent, ein Zeitplan) plus ein neuer, zwölfter Screenshot `host-zonen.png` (alle vier Zonen-Typen inkl. der Kapitel-13-Teil-2-Pseudo-Zone, plus eine per echter Instanz-Migration erzeugte MXL-Warnkante); `regieplatz-1.png` entfernt (Inhalt jetzt im selben Operator-Konsole-Screenshot enthalten). README neu strukturiert (Feature-Liste nach Kategorien, neuer "What OpenMediaPlatform does not do"-Abschnitt, neuer License-Abschnitt); Benutzerhandbuch auf die neuen Screenshots + Kapitel-13-Teil-2/3-Fähigkeiten synchronisiert. Live gefundener, nicht behobener Produktbug notiert (nicht Scope dieser Sitzung): zwei gleichzeitig als Workflow-Rollen startende `omp-source`-Instanzen können sich eine Registrierungs-Race teilen, wobei genau eine ihren Video-Sender nicht registriert (Audio bleibt unberührt) — Solo-Neustart der Rolle behebt es zuverlässig, root cause offen für eine künftige Sitzung. Test-Infrastruktur aufgeräumt, Demo-Workflow „Regie 1" bewusst laufend gelassen. Details docs/decisions.md Nachtrag 141. | 2026-08-12 |
| Kapitel 13 Teil 4: Workflow-Kacheln in die Host-Zonen-Lanes einbinden + Host-Anzeige in der Operator-Konsole | erledigt | Zwei live gemeldete Nutzerfunde beim Ausprobieren des Multi-Host-Demo-Setups: „Regie 1 sollte multihost sein, wird aber im Floweditor nicht im Host-Fenster gezeichnet" und „im Regieplatz expandiert ist nicht ersichtlich, welcher Node wo gehostet ist". Fund 1 — `#renderWorkflowTiles()` (Kapitel 12 Teil 2) war komplett unabhängig vom Host-Zonen-System (`#buildHostZoneLayer`/`#hostZones`/`#arrangeIntoLanes`, Kapitel 13 Teil 1-3): eine laufende Workflow-Kachel blieb bei aktiver Host-Ansicht immer an ihrer freien Vor-Host-Ansicht-Position hängen, unabhängig davon, auf welchen Hosts ihre Rollen tatsächlich liefen — genau das von Teil 3 dokumentierte Scope-Limit ("läuft in der Host-Ansicht immer als EINE kollabierte Kachel"), hier aber das Rendering-Symptom davon behoben statt nur die Drag-Migration. Neue `#zoneIdForWorkflow(wf)` (liest `wf.runtime[role].hostId`, das die Workflow-API bereits pro Rolle mitliefert — Frontend-Typ ergänzt, kein neuer Endpunkt) mit derselben „einheitlich vs. mixed"-Logik wie das bestehende `#zoneIdForGroup`; Workflow-Kacheln jetzt in denselben Lane-Stapel wie echte Kacheln eingebunden (Zonen-Höhe, „N Kacheln"-Zähler, Einklapp-Ausblendung, Positions-Persistenz bei Host-Ansicht Ein/Aus — alles vorher nur für Node-/Gruppen-IDs geführt). Fund 2 — `ConsoleEntry` (Operator-Konsole) hatte kein Host-Feld: `shell.ts#fetchConsoles` reichert jetzt client-seitig per Join gegen `GET /api/v1/instances` + `/api/v1/hosts` an (beide nur `requireAuth`, ein reiner Operator kann sie lesen, kein neuer Endpunkt nötig — gleiches „reine Client-Arbeit"-Prinzip wie §13.1); `console-board.ts` zeigt ein Host-Badge pro Kachel, `console-view.ts` hängt den Host-Namen an die Tab-Beschriftung. `deno check`/`deno test ui/` (84/84) grün. Live per Puppeteer/CDP verifiziert (Chromium bereits aus einer früheren Sitzung unter `/home/infantilo/node_modules/puppeteer` vorhanden, kein neues Tooling nötig) — nicht gegen `omp-video-mixer-me`/`omp-ograf` (siehe Nebenbefund unten), sondern gegen einen risikofreien Testworkflow (zwei `omp-source`-Rollen über beide Hosts verteilt): Workflow-Kachel landete exakt auf der berechneten Pixel-Position der „mixed"-Zonen-Lane, Operator-Konsole zeigte „Test-Kamera-A · Regie-Host-A"/„Test-Kamera-B · Regie-Host-B" — Screenshots bestätigt. Committed (`2766976`). Nebenbefund derselben Sitzung, kein Produktbug: `ps -o pcpu` liefert in dieser Crostini/Podman-Sandbox bei jungen, vielfädigen Prozessen falsche Werte (0 % über Sekunden, dann einzelne Ausreißer wie 4559 %) — eine dadurch ausgelöste Stunden lange „CPU-Livelock"-Untersuchung (inkl. testweise geschriebenem, dann auf Nutzerwunsch wieder verworfenem Rust-Patch in `omp-mediaio::mxl::read_loop`) stellte sich per präziser `/proc/<pid>/task/*/stat`-Messung als Messfehler heraus; reale Last für zwei echte Video-Streams liegt normal bei ~150–230 %. Details/Messmethode in den Memory-Einträgen dieser Sitzung, nicht in `docs/decisions.md`. | 2026-08-13 |
| Backup/Restore über das Browser-UI (inkl. neuem Supervisor-Prozess) | erledigt | Nutzerwunsch 2026-08-13 ("können wir ein generelles Backup/Restore über das Browser-UI ermöglichen"), in zwei Teilen: Backup zuerst (risikolos, kein neuer Prozess — neues Paket `orchestrator/internal/backup` spiegelt `backup-omp.sh` 1:1, drei Admin-Endpunkte, neuer fünfter Administration-Unterreiter). Für Restore explizite Nutzerentscheidung zwischen "nur Backup, Restore bleibt CLI" und "kleiner Supervisor-Prozess davor" — Zweiteres gewählt: der Orchestrator kann sich nicht selbst befehlen, sich mitten in der Restore-Antwort zu stoppen (Restore verlangt einen gestoppten Prozess, s. `restore-omp.sh`-Kopfkommentar). Neues eigenständiges Go-Modul `supervisor/` (`omp-supervisor`, lauscht NUR auf `127.0.0.1:8091`, bewusst kein zusätzliches Auth-Schema — Loopback-Bindung ist die Vertrauensgrenze, dokumentierte Entscheidung kein Versehen) — führt keine eigene Prozess-/Restore-Logik neu ein, ruft stattdessen `deploy/dev/{stop,start}-omp.sh` als Subprozesse auf (PID-Handling/Port-Checks/UI-Bau/healthz-Warten bereits vorhanden+getestet) und spiegelt `restore-omp.sh`s `gunzip -c \| podman exec -i omp-postgres psql ...` exakt. Automatisch von `start-omp.sh` mitgestartet (idempotent), überlebt `make stop` (nur `make stop ARGS=--all` beendet ihn mit), neu in `GO_MODULES`/`make status`. Orchestrator-seitig `POST /api/v1/admin/restore` (VerbAdmin, `confirm:true` hart erforderlich, kein impliziter Default) ruft den Supervisor über neues Paket `internal/supervisorclient` auf; dessen Handler antwortet SOFORT (eigene Goroutine für die eigentliche Arbeit, 400ms Vorlauf-Delay) — die Orchestrator-Antwort an den Browser geht damit noch heraus, bevor der Prozess tatsächlich stirbt. Frontend: Backup-Liste + Dropdown, Restore verlangt exaktes Eintippen des gewählten Dateinamens (schärfere Variante von `restore-omp.sh`s "yes"-Eingabe) plus zusätzlichem `confirmDialog()`; während des Vorgangs ersetzt ein Overlay den gesamten Abschnitt ("Server wird neu gestartet …"), das per `/healthz`-Polling (2s Anfangsverzögerung, dann 1s-Takt) automatisch `window.location.reload()` auslöst, sobald der Orchestrator wieder antwortet. `go vet`/`go test` (orchestrator inkl. echter Postgres-Tests, supervisor, neues `internal/supervisorclient`) und `deno check`/`deno test ui/` (84/84) grün. Sicherheitsmethodik für die live-Verifikation der destruktivsten Aktion der Plattform: vor jedem echten Restore-Testlauf ein frisches Backup als Fallback UND ein disponibler "Kanarienvogel"-Testnutzer angelegt, dessen Verschwinden nach dem Restore den tatsächlichen Datenrücksprung belegt, ohne echte Daten (Regie-1-Workflow, echte Nutzerkonten) zu riskieren — dreimal wiederholt (direkt gegen den Supervisor, End-to-End gegen den echten Orchestrator-Endpunkt, und ein drittes Mal komplett durch die echte Browser-UI per Puppeteer/CDP inkl. abgefangenem `confirmDialog()` und tatsächlich beobachteter Seiten-Navigation durch `waitForNavigation()`) — jedes Mal Kanarienvogel korrekt verschwunden, Regie-1-Workflow und echte Nutzerkonten jedes Mal unversehrt, kompletter Zyklus 8–10s. | 2026-08-13 |
...
| Host-Setup-Wizard (Bare-Metal/VM/AWS, ARCHITECTURE.md §18.3/§18.8/§18.9) | erledigt | Nutzerauftrag 2026-08-27 ("wie könnte aus unserem orchestrator aus ein neuer host eingerichtet werden oder [...] auf eine AWS ausgelagert werden [...] sowas wie ein Wizard") — neues `<omp-host-wizard>` (`ui/shell/host-wizard.ts`) führt Admins durch Zielumgebung+Label, Token-Erzeugung, editierbares Provisionierungs-Skript (Shell-Snippet bzw. EC2-User-Data, Copy-Button) und Live-Warten auf Anmeldung (SSE+Poll-Fallback) bis der Host mit seinen Capabilities erscheint — kein neuer Backend-Endpunkt, nur eine UI-Schicht über den bereits bestehenden Bootstrap-Token-Flow (§18.3). `hosts-view.ts` bekommt den auslösenden, admin-only sichtbaren Button. Live per Puppeteer gegen den echten Orchestrator verifiziert (Kachel → Token → generiertes Skript → simulierte Host-Agent-Registrierung → Wizard erkennt sie live → Host erscheint in der Liste), `deno check`/`deno test` grün. Commit 0f753c9. | 2026-08-27 |
| Cluster-Sub-Tab in Administration (Raft-Join/Leave-UI, ARCHITECTURE.md §19.3) | erledigt | Direkter Nutzerauftrag ("fahre fort") im Anschluss an den Host-Wizard, gleicher Anlass. Neuer sechster Admin-Sub-Tab "Cluster" (`admin-view.ts`): Status-Karte (Node-ID/Zustand/Term/Log-Index/Leader), Peer-Tabelle mit Leader-Badge und Entfernen-Button (`confirmDialog`, gleiches Muster wie Nutzer/Bindungen/Katalog), Formular "Weiteren Orchestrator hinzufügen" mit editierbaren Node-ID-/Raft-Adresse-/HTTP-Adresse-Feldern und live generiertem Start-Skript (`OMP_NODE_ID`/`OMP_RAFT_LISTEN`/`OMP_RAFT_DATA_DIR`/`OMP_CLUSTER_JOIN`, Variablennamen aus `config.go` verifiziert statt geraten), "Jetzt beitreten lassen" ruft `POST /api/v1/cluster/join` direkt auf — kein neuer Backend-Endpunkt, nur eine UI-Schicht über die bereits bestehende D12-Teil-2-API. Live gegen einen echten zweiten Orchestrator-Prozess verifiziert (Beitritt → 2 Peers, Entfernen → 1 Peer, per Puppeteer-UI-Klick UND direktem API-Gegencheck). Dabei ein Test-Skript-Fehler live erlebt und dokumentiert: der erste Testlauf wählte den "Entfernen"-Button per Position statt per Zeileninhalt und entfernte dadurch node-1 (die echte, langlaufende Dev-Instanz) statt des Wegwerf-Test-Peers node-2 — ein Bug im Test, nicht in der Komponente (die Zeile wurde exakt wie geklickt entfernt, belegt durch den zweiten, zeilengenau zielenden Lauf). Dev-Umgebung danach sauber wiederhergestellt: Raft-Datenverzeichnisse enthalten keine Geschäftsdaten (die liegen in Postgres/Patroni, §19.3/§19.4) — node-1 nach Löschen+Neustart wieder als sauberer Ein-Knoten-Leader, Hosts/Workflows in Postgres nachweislich unverändert. `deno check`/`deno test` grün. | 2026-08-27 |
| Backup-Restore per Browser-Upload | erledigt | Nutzerfund 2026-08-27 ("backup kann downgeloaded werden, aber nicht per upload im Browser wiederhergestellt werden — derzeit nur aus der Dropdownbox"). `backup.Service.Import([]byte)` (neu, `internal/backup/backup.go`) nimmt hochgeladene Bytes entgegen, validiert nur, dass es sich um gültiges gzip handelt (gleicher Vertrauensstand wie `Create()` — den SQL-Inhalt prüft erst `psql` beim tatsächlichen Restore), vergibt IMMER einen neuen serverseitigen Namen (nie den Client-Dateinamen, verhindert Path-Traversal/Kollision von vornherein statt es erst bei `Path()` abzufangen) und wendet dieselbe Rotation wie `Create()` an. Neuer Endpunkt `POST /api/v1/admin/backups/upload` (admin-only, 512-MiB-Deckel via `http.MaxBytesReader`). UI (`admin-view.ts`, Backup/Restore-Tab): Datei-Input neben „Restore", lädt hoch und wählt die hochgeladene Datei automatisch in der bestehenden Restore-Dropdown/Bestätigungs-Kette aus — die getippte Namens-Bestätigung vor dem eigentlichen Zurückspielen bleibt unverändert Pflicht, nur ein zweiter Weg, eine Datei in die Liste zu bekommen, keine Abkürzung. Live verifiziert: Upload über die echte UI, Byte-für-Byte-Rundlauf per Download gegengeprüft; 8 neue Unit-Tests (`backup`+`httpapi`), volle Suiten grün. | 2026-08-27 |
| Cluster: Leader kann sich nicht mehr selbst als letztes Mitglied entfernen | erledigt | Nutzerfund 2026-08-27 ("verhindere dass ich den cluster leader selbst entferne, vor allem, wenn es keinen anderen gibt") — genau der Vorfall, der beim vorherigen Cluster-Tab-Testlauf desselben Tages live passierte (node-1 aus Versehen entfernt, musste per Datenverzeichnis-Löschen repariert werden). `cluster.Node.Leave()` lehnt jetzt hart ab, wenn der Leader sich selbst entfernen will UND er das einzige verbleibende Mitglied ist (`ErrLastVoterIsLeader`, 409) — das würde den Cluster dauerhaft unbrauchbar machen (kein Server mehr übrig, der je wieder eine Wahl abhalten könnte). Entfernt sich der Leader dagegen selbst, während ANDERE Mitglieder übrig bleiben, bleibt das erlaubt (unterstützter Vorgang, `raft.Config.ShutdownOnRemove` triggert eine Neuwahl unter den Verbleibenden) — dabei ein echter, bis dahin ungetesteter Vorbestehender Bug gefunden+behoben: der nachfolgende FSM-Cleanup-Apply in `Leave()` schlug in genau diesem Fall mit `raft.ErrRaftShutdown` fehl (die eigene Raft-Instanz fährt synchron währenddessen herunter), jetzt als erwarteter Fall abgefangen statt als Fehler durchgereicht. UI (`admin-view.ts`, Cluster-Tab): „Entfernen"-Button für den alleinigen Leader sichtbar deaktiviert (Tooltip), Selbst-Entfernung mit verbleibenden Mitgliedern zeigt stattdessen eine deutlichere Warnung im Bestätigungsdialog statt eines Blocks. Live verifiziert: UI-Button deaktiviert, direkter API-Aufruf liefert 409 mit klarer Meldung, Cluster-Mitgliedschaft danach unverändert; 4 neue Unit-Tests (`TestLeaveRejectsRemovingLastVoterSelf`, `TestLeaveAllowsLeaderRemovingItselfWhenOthersRemain` u. a.), volle Suiten grün. | 2026-08-27 |
| Kapitel 6 Teil 1 (Rundown-Fundament, `docs/END-GOAL-FEATURES.md` §6.5) | erledigt | Nutzerauftrag 2026-09-07 ("playout automation auf PIPELINE-CONTROLLER-Parität bringen"), Nachtrag 180 aktualisierte zuerst den Plan (neuer `startType: manual`, kein PC-Vorbild), Nutzerentscheidung dann: mit Teil 1 anfangen. Neuer `StartType`-Enum (`sequence`/`manual`) auf `ItemMeta`, `Playlist::peek_next()` (rein lesend) statt Prädikat-Parameter an `advance()` — `do_advance()` cued ein `manual`-Nachfolge-Item statt es automatisch zu nehmen. Neue `setStartType`-Methode (lokal, kein Player-Roundtrip). Reorder-Fallstrick gelöst: `load()` vergibt immer frische Item-IDs, `startType` wird jetzt positionell gegen die Player-Antwort gezippt statt bei jedem Reorder verloren zu gehen. UI: natives HTML5-Drag&Drop (Touch-Audit-Fund, feuert auf Touch-Geräten gar nicht) durch Pointer Events + `setPointerCapture` ersetzt, ✋/⏭-Umschalter pro Zeile, Wanduhr im Kopfbereich (Fixzeit-Countdown bewusst weggelassen, kommt erst mit Teil 3). `cargo build/test`(28/28)/`clippy`/`deno check` grün. Live gegen den echten Dev-Stack verifiziert (Player+Mixer+Automation-Instanz, echte Cue/Take/Auto-Advance-Zyklen per API) UND per echtem Chromium-Klicktest (hand-gebauter CDP-Treiber, kein Automatisierungs-Tool vorhanden) — dabei einen echten Bug gefunden, den API-Tests allein nicht gefangen hätten: `itemToLoadEntry()` im UI-Bundle schickte `startType` beim Reorder gar nicht mit, jeder Browser-Drag setzte alle Items lautlos zurück; behoben, zweiter Klicktest bestätigt den Fix. Details: `docs/decisions.md` Nachtrag 181. | 2026-09-07 |
| Kapitel 6 Teil 2 (Verfügbarkeits-Spalte, verkleinert — `docs/END-GOAL-FEATURES.md` §6.5) | teilweise | Nutzerauftrag "teil2". Scope-Korrektur vor jedem Code: "echte Clips + EOS" (ursprünglich Teil des Plans) blockiert auf einem bereits früher isolierten, unresolved GStreamer-Freeze-Bug in `omp-player`s EOS-Probe (`pipeline.rs`-Doku, `gdb`-Backtrace bereits gemacht, mehrere Hypothesen verworfen) — bewusst zurückgestellt statt in dieser Sitzung neu zu debuggen (§0 Punkt 2/9). As-Run-Log ebenfalls zurückgestellt (braucht neuen Orchestrator-Endpunkt/Postgres, eigener Schritt). Umgesetzt: `item_is_available()` gegen den gespiegelten `media_library`/`available_sources`-Stand, `GET items` liefert `available` pro Zeile, `do_take` blockiert (nicht `do_cue`) auf fehlender Quelle mit klarer Fehlermeldung, neue Verfügbarkeits-Spalte (✓/✗) im Rundown-UI. `cargo build/test`(31/31)/`clippy` grün. Live verifiziert: echte/fehlende Datei, tote Live-Quelle, Datei-Löschung während der Sitzung korrekt erkannt, `cue` erlaubt/`take` verweigert, per Chromium-Klicktest visuell bestätigt. Details: `docs/decisions.md` Nachtrag 182. Offen: echte Clips+EOS (blockiert), As-Run-Log (eigener Schritt), Clip-Browser-Event-Editor-Drawer. | 2026-09-07 |
| Kapitel 6 Teil 3 (Fixtime-Scheduler + Countdown — `docs/END-GOAL-FEATURES.md` §6.5) | erledigt | Nutzerauftrag "fahre fort". Neuer dritter `StartType::Fixtime` (`fixtime_hms: Option<String>`, "HH:MM:SS" lokale Wanduhr) — feuert unabhängig vom Sequenz-Fortschritt ("harter Unterbrecher", PC-Vorbild Pre-Cue/Gnadenfenster). Neue Abhängigkeit `chrono` (nur `clock`+`std`) — std kann keine zeitzonen-/DST-bewusste Wanduhr-Zerlegung. Reine Funktionen `parse_hms_to_secs`/`fixtime_action` (7 neue Tests) treiben einen eigenen 1s-Takt (`fixtime_loop`, komplett getrennt vom 200ms-Advance-Pfad): PreCue 5s vorher, Fire zur Zielzeit (springt die Cue-Position selbst dorthin, neue `do_fire_fixtime`), Skip+NATS-Alarm nach 30s Gnadenfenster. Cart hat Vorrang (Fire wartet statt zu erzwingen), Verfügbarkeitsprüfung (Teil 2) greift auch hier. Reorder-Fallstrick aus Nachtrag 181 diesmal proaktiv vermieden (`fixtimeHms` gleich mit `startType` in `itemToLoadEntry` ergänzt). UI: 3-Wege-Umschalter (Klick auf fixtime fragt Uhrzeit per `prompt()` ab), blaue Fixtime-Kennzeichnung, Kopfzeile zeigt Countdown zum nächsten Fixtime-Event. `cargo build/test`(38/38)/`clippy`/`node --check` grün. Live sekundengenau gegen den echten Dev-Stack verifiziert: PreCue exakt bei T-5s, Take exakt bei T (überschreibt laufendes Item ohne vorheriges Cue), Gnadenfenster-Skip korrekt (keine Take), NATS-Alarm per `nats sub` unabhängig bestätigt, Countdown+Badges per Chromium-Klicktest visuell bestätigt. Nebenfund (nicht behoben, außerhalb des Auftrags): Orchestrator abonniert `omp.alert.*` aktuell nirgends selbst, keine REST-„Alarme"-Ansicht dafür. Details: `docs/decisions.md` Nachtrag 183. | 2026-09-07 |
| Kapitel 6 Teil 4 (Transitions cut/mix — `docs/END-GOAL-FEATURES.md` §6.5) | erledigt | Nutzerauftrag "proceed". Vor Code nachgeschaut: der Plan-Bullet nannte auch "Break/Auto-Return" als offen — bereits fertig vorgefunden (`ActiveCart::elapsed_before_interrupt_ms`, C18), Plan korrigiert statt doppelt gebaut. Echte neue Arbeit: `Transition`-Enum (`Cut`/`Mix`) + `transition_rate_frames` auf `ItemMeta`, `take_on_targets()` verzweigt neu — `Mix` nutzt `crosspoint.setTransRate`(optional)+`crosspoint.autoTrans` statt `crosspoint.cut` (K3-Teil-2, bereits fertig, keine Mixer-Änderung). Bewusste Aufteilung der 8 Aufrufstellen: die 5 echten Take-Pfade respektieren das Item-Feld, die 3 Ausnahmen (Stop/Cart-Fire/Cart-Return) bleiben immer harter Cut. Neue `setTransition`-Methode (lokal, kein Roundtrip). Reorder-Fallstrick zum dritten Mal proaktiv vermieden (kein Klicktest-Fund diesmal nötig). Ehrliche v1-Grenze bestätigt: mit nur einem Ziel-Player ist "Mix" strukturell keine sichtbare Quell-zu-Quell-Blende. `cargo build/test`(38/38)/`clippy`/`node --check` grün. Live verifiziert: `setTransition` rundtrippt, Mix-Take löste nachweislich `crosspoint.transRate`-Wechsel (25→50) + abgeschlossene Rampe am Mixer aus, Reorder erhält `transition` beim ersten Versuch, ⇄/✂-Badges per Chromium-Klicktest bestätigt. Details: `docs/decisions.md` Nachtrag 184. | 2026-09-07 |
| Kapitel 6 Teil 5 (Grafik-Child-Events über omp-ograf — `docs/END-GOAL-FEATURES.md` §6.5) | erledigt | Nutzerauftrag "fahre fort" — größter Kapitel-6-Baustein bisher, erste echte Cross-Node-Integration (drittes Ziel `omp-ograf`, `targetGraphicsLabel`, bewusst optional anders als Player/Mixer). `GraphicsChild{templateId, data (rohes JSON), delayMs, durationMs, relativeTo: start\|end}` auf `ItemMeta`. Scheduling über eine `graphics_epoch` (statt aktiver Bereinigung) — erhöht sich bei allen 8 On-Air-Änderungs-Stellen, ein Ereignis mit alter Epoche gilt als storniert. `schedule_children()` berechnet Zeitpunkte über die reine Funktion `child_show_offset_ms()` (8 neue Tests), `resolve_variables()` löst `{{next:title}}` auf (6 neue Tests, PC-Vorbild "Teilmenge" bewusst nicht generisch). Eigener `graphics_loop` (250ms-Takt, komplett getrennt von den bestehenden Loops), sortiert fällige Ereignisse nach `fire_at` (Cart-Return-Nachhol-Fall bewusst entschieden: dieselbe zurückdatierte `onair_since` wie die C18-Restdauer-Logik). Neue `setChildren`-Methode (JSON-kodiertes Array wie `load`s `itemsJson`), UI bewusst ohne Feld-für-Feld-Formular (rohes JSON-Array im `prompt()`, Minimal-Muster wie Fixzeit/Rampendauer), 🎨-Badge mit Kind-Anzahl pro Zeile. Reorder-Fallstrick zum vierten Mal proaktiv vermieden — kein Live-Fund mehr nötig seit Nachtrag 181. `cargo build/test`(46/46)/`clippy`/`node --check` grün. Live gegen einen echten Vier-Instanzen-Stack (Player+Mixer+OGraf+Automation) sekundengenau verifiziert: Start- UND End-relatives Kind feuerten exakt im erwarteten Zeitfenster inkl. automatischem Hide, fehlendes Grafik-Ziel löste die erwarteten NATS-Alarme (unabhängig per `nats sub` bestätigt) statt stiller Verwerfung. Details: `docs/decisions.md` Nachtrag 185. | 2026-09-07 |
| Workflow-Editor-UX überarbeitet + Workflow "Playout" angelegt | erledigt | Nutzerauftrag: vor dem Test von Kapitel 6 erst das als "absolut nicht intuitiv" kritisierte Rollen-Bearbeiten überarbeiten. Drei parallele, funktional identische Bearbeitungswege (Text-Gitter im Formular, grafischer Designer, "Im Flow-Editor bearbeiten") auf einen konsolidiert: `editInFlowBtn` entfernt, das komplette Sechs-Spalten-Rollen/Verbindungen-Gitter im Text-Formular durch einen Zusammenfassungs-Block + Button zum grafischen `<omp-role-designer>` ersetzt (jetzt alleiniger Bearbeitungsweg, tote `#roleSelect()`-Hilfsmethode entfernt, Modal-Breite zurück auf Standard). Dabei einen echten, vorher übersehenen Bug gefunden+gefixt: `#submitForm()` brach bei 0 Rollen mit stillem `return` ab (kein Toast) — mit dem Text-Gitter weg wäre das für jedes neue Workflow eine Sackgasse geworden; jetzt Toast für fehlenden Namen, Backend-Fehler für fehlende Rollen kommt über den bestehenden Fehler-Toast-Pfad. Erweitert-Umschalter für die Rollen-Kachel-Detailanzeige (Host/Format/Standby/Placement) bewusst zurückgestellt (`TILE_HEIGHT`-Geometrie an ~12 Stellen, zu riskant für dieselbe Sitzung). `deno check`/`deno test` (92/92) grün, per echtem Chromium-Klicktest verifiziert (leerer Name→Toast, Bestandsworkflow-Rollen bleiben beim Speichern erhalten, Designer öffnet vorbefüllt). Anschließend Workflow "Playout" über den jetzt alleinigen grafischen Weg angelegt: vier Rollen (`omp-player-video`+`omp-video-mixer-me`+`omp-ograf`+`omp-playout-automation`), bewusst ohne deklarierte Connections — live bestätigt, dass der Mixer alle MXL-Sender im Domain selbst discovert (`crosspoint.inputs`) und die Automation Player/Mixer/Grafik rein über NATS-Label auflöst, keine Verkabelung nötig. Testweise gestartet (alle vier Instanzen sauber hochgelaufen, `connected:true` nach Setzen der drei Ziel-Labels), danach sauber gestoppt — Workflow liegt jetzt bereit, damit der Nutzer Kapitel 6 Teil 1-5 selbst durchtesten kann, bevor eine Entscheidung zu Kapitel 6 Teil 6 (Channel-Branding) ansteht. Details: `docs/decisions.md` Nachtrag 186. | 2026-09-07 |
| Kapitel 6 Teil 6 (neuer Node `omp-channel-player` gegen `omp-player`-Freeze-Bug + "Mix ist No-Op"-Grenze — `docs/END-GOAL-FEATURES.md` §6.5) | erledigt | Nutzerauftrag: Playout-Workflow-Live-Test (clip→live→clip) zeigte, dass der Mixer sichtbar nicht umschaltet. Diagnose (kein Raten, §0 Punkt 9): Automation selbst arbeitet korrekt (kein Bug); `omp-player`s `input-selector`-Active-Pad-Umschaltung friert das PGM-Bild nach dem ersten Take dauerhaft ein (neuer, bisher unbekannter Fund, verschieden vom bereits dokumentierten deaktivierten `add_eos_probe`-Freeze) — bestätigt per `mxl-info`+Viewer-Frame-Grabs. Unabhängig davon: mit nur einem Ziel-Player hat `crosspoint.inputs` strukturell immer nur EINEN Eintrag, `Transition::Mix` ist damit grundsätzlich wirkungslos, nicht nur ein Symptom (bereits im Zieldesign als "ehrliche v1-Grenze: Xfade nur zwischen zwei Player-Instanzen" benannt). Nutzerentscheidung: direkt auf die Zwei-Player-Architektur umsteigen statt den Freeze-Bug zu jagen. Umgesetzt (nur der Node selbst, noch NICHT in die Automation verdrahtet — Umfangsentscheidung §0 Punkt 2): neuer Node `omp-channel-player`, einzweigig/Isel-frei, `load()` baut den Zweig komplett neu statt Active-Pad umzuschalten — Bug strukturell umgangen. Element-Konstruktion kopiert/angepasst aus `omp-player` (TestPattern/generische File/Live) und `omp-mxf-player-direct` (MXF, Multi-Mono-Audio-Interleave "Audio-Shuffling", auf eine feste Ausgabegruppe verengt). Dabei live einen zweiten, neuen Bug gefunden+gefixt: `set_property_from_str` panikt intern bei ungültigem Enum-Wert (z. B. Tippfehler im `pattern`-Argument) und riss den Pipeline-Thread mit runter — betrifft `omp-player` in derselben Form (dort nicht mitgefixt, dokumentiert), hier per neuer `set_enum_property_checked()`-Vorabprüfung behoben. `cargo build/test`(9/9)/`clippy -D warnings` grün. Live verifiziert (zwei echte Instanzen über den Launcher, `mxl-info`+Viewer-Frame-Grabs): TestPattern- und MXF-Wiedergabe korrekt UND sichtbar unterschiedlich, `positionMs` lief in Echtzeit, Rückwechsel bewies wiederholte `load()`-Aufrufe funktionieren (kein Freeze reproduzierbar), Panic-Fix live reproduziert und verifiziert (vorher Absturz, danach sauberer Fehler + weiter bedienbar), beide Sender in der NMOS-Registry mit auflösbarem Video-Flow-Format sichtbar (Crosspoint-Discovery-Voraussetzung für Teil 7 erfüllt). `Live`-Quelltyp nicht live getestet (keine Testquelle verfügbar), Code 1:1 aus `omp-player` übernommen. Nebenbefund: der nötige Orchestrator-Neustart (Katalog wird nur beim Start geladen) beendete den laufenden Mixer-Prozess des "Playout"-Workflows — braucht vor dem nächsten Test einen manuellen Neustart der Mixer-Rolle. Details: `docs/decisions.md` Nachtrag 187 (+ Nachtrag desselben Tages: Workflow "Playout" auf zwei `omp-channel-player`-Rollen umgestellt und neu gestartet, echter Crossfade per Mixer-Crosspoint live per Frame-Grab bestätigt — dabei einen weiteren Mixer-Bug gefunden/operativ umgangen, kein Fix: der Mixer baut die Eingangs-Branch für einen Crosspoint-Kandidaten nur einmal bei der ERSTEN Discovery und versucht es nie erneut, falls dessen MXL-Flow zu dem Zeitpunkt noch nicht existierte — Kanal-Player immer vor dem Mixer starten). Teil 7 (Automation-Retargeting auf zwei Kanäle), Teil 8 (Audio-Mixer-Verdrahtung), Teil 9 (Live-Quellen-UI) offen. | 2026-09-08 |
| omp-ograf: CPU-Leerlauf ~20× gesenkt | erledigt | Nutzerauftrag "ograf burns cpu. fix that!". Thread-genaue `/proc`-Messung (kein Raten): 216-242% CPU im Leerlauf (`current: null`). Zwei Ursachen gefunden: (1) `spawn_alpha_key_bridge`s Alpha-Extraktion allozierte+nullte jeden Frame unnötig einen kompletten Puffer vor dem Überschreiben, gefixt via `Vec::with_capacity`+`chunks_exact`-Iterator; (2) per Debug-vs.-Release-Vergleich bestätigt, dass GStreamers eigene `tee`/`queue`-Threads (nicht Node-Code) den Löwenanteil ausmachen — volle 1280×720-BGRA-Frames laufen 25fps permanent durch vier Branches, auch wenn nichts gezeigt wird (`show`/`hide` schalteten bisher nur CSS um). Echter Fix: ganze Pipeline pausieren solange nichts gezeigt wird (`transition_pipeline()`, dieselbe bereits bewährte zweistufige `set_state`+`state()`-Choreografie wie beim initialen Aufbau, `NO_PREROLL`-Verhalten des live `wpesrc`s berücksichtigt), 300ms Gnadenfrist bei `hide()` vor dem Pausieren (damit der versteckte Zustand noch durchläuft, sonst friert der letzte SICHTBARE Frame ein). Vorab-Risikoprüfung: dokumentierter `wpesrc`-GMainLoop-Fallstrick nicht angetastet, `write_loop`s `None`-Behandlung in `omp_mediaio::mxl` bereits sicher (kein Spinnen) — Muster deckt sich mit `omp-mxf-player-direct`s bewährtem "Stop pausiert". `cargo build/test/clippy` grün. Live verifiziert (echte Workflow-Instanz, `mxl-info`+Viewer-Frame-Grabs, 3 Show/Hide-Zyklen): 216-242%→1-15% im Leerlauf, `show()` liefert nachweislich echten animierten Grafikinhalt + CPU zurück auf ~150%, `hide()` friert korrekt auf dem bereits versteckten Bild ein. Nebenbefund (nicht durch diese Änderung verursacht): ein Rollen-Neustart löste diesmal einen Stopp aller fünf Workflow-Instanzen aus (Bookkeeping-Fallstrick bei wiederholten Rollen-Neustarts über eine Sitzung), Workaround: einmal sauber stop+start. Details: `docs/decisions.md` Nachtrag 188. | 2026-09-08 |
| `omp-player` entfernt (zwei unbehobene Freeze-Bugs, durch `omp-mxf-player`/`omp-channel-player` abgelöst) | erledigt | Nutzerauftrag "entferne den omp-player, da wir ja zwei andere varianten davon haben". Vor dem Löschen Methodenverträge geprüft statt angenommen: `omp-playout-automation` fernsteuert seinen Zielplayer über `append`/`load`/`remove`/`cue`/`take` — davon implementiert `omp-channel-player` nur `load()` (Retargeting ist Kapitel 6 Teil 7-9, noch offen), `omp-mxf-player` dagegen den vollen Satz und ist als einziger der drei verbleibenden Player bereits live stress-getestet reliable. `nodes/omp-player/` entfernt, aus `nodes/Cargo.toml` und beiden Katalogeinträgen (`omp-player-video`/`omp-player-jingle`) in `deploy/catalog.json` entfernt, README.md/`docs/HANDBUCH.md` auf die drei real vorhandenen Player aktualisiert. Keine funktionalen Code-Abhängigkeiten außerhalb von `nodes/omp-player/` gefunden (nur Kommentare/Moduldoku). `cargo build --workspace --bins` (24 Crates) grün. **Bewusst nicht geprüft:** ob der gespeicherte "Playout"-Workflow in der Dev-Postgres noch eine `omp-player-video`-Rolle referenziert (laut Nachtrag 187 bereits auf zwei `omp-channel-player`-Rollen umgestellt, aber in dieser Sitzung kein laufender Orchestrator zum Nachprüfen verfügbar) — vor dem nächsten Start dieses Workflows kurz gegenprüfen. `ARCHITECTURE.md`/`UMSETZUNG.md`-Historie und `docs/NODE-TUTORIAL.md` bewusst nicht rückwirkend umgeschrieben. Details: `docs/decisions.md` Nachtrag 198. | 2026-09-11 |
| Workflow-Rollen-Migration in der Flow-Editor-UI (README-Lücke "no drag-to-move UI yet") | erledigt | Nutzerauftrag "starte mit workflow role migration in ui...". Backend (`POST /workflows/{id}/roles/{role}/migrate`, Kapitel 13 Teil 3) war bereits fertig, blockiert war nur die UI: `#openHostMigrateMenu` zeigte für Rollen-gebundene Kacheln einen "noch nicht unterstützt"-Toast statt des Host-Menüs. Bestehende Workflow-Filter-Ansicht (App-Bar-Dropdown) macht Rollen-Kacheln bereits einzeln rechtsklickbar — kein neuer Einstiegspunkt nötig, nur der blockierende Zweig durch `#confirmAndMigrateWorkflowRole()` ersetzt (ruft den Rollen- statt den Instanz-Endpunkt). Live verifiziert gegen eine echte Zwei-Host-Dev-Umgebung (`make hosts`) per hand-gebautem CDP-Skript + Wegwerf-Workflow: Rechtsklick öffnet das Host-Menü, Bestätigungsdialog zeigt den korrekten Make-before-Break-Text, nach Bestätigen wechselte `runtime.<role>.hostId` nachweislich über die Orchestrator-API auf den Zielhost (neue Instanz-/Node-ID). Dabei einen unabhängigen, bereits vorbestehenden Timing-Fund dokumentiert (kein Fix nötig): `#paletteHosts` füllt sich asynchron, ein Rechtsklick zu früh nach Workflow-Filter-Wechsel zeigt nur die lokale Zone. `deno check`/`deno test ui/` (92/92) grün. **Bewusst nicht Teil dieser Runde:** die kollabierte Root-Workflow-Kachel selbst bekommt kein eigenes Rollen-Migrations-Menü (Filter-Ansicht deckt den Anwendungsfall bereits ab, nur weniger entdeckbar) — eigener, größerer Schritt falls gewünscht. Details: `docs/decisions.md` Nachtrag 199. | 2026-09-11 |
| Host-Ansicht: Kacheln springen/überlappen nicht mehr beim Start neuer Nodes (Nutzerreport) | erledigt | Nutzerreport "positionen der bestehenden nodes/groups/workflow kacheln verschoben werden... node kacheln übereinandergelegt". Zwei echte Root Causes in `#arrangeIntoLanes` gefunden: (1) der bisherige Ein-Durchlauf-Kollisionscheck war abhängig von der instabilen Registry-Reihenfolge — eine neu erschienene Kachel konnte je nach Zufalls-Reihenfolge einer BESTEHENDEN Kachel ihren gemerkten Platz streitig machen, die dann sichtbar sprang. Fix: zwei Durchgänge (erst alle gemerkten Positionen deterministisch nach Y/ID reservieren, dann erst neue Kacheln in die verbleibenden Lücken einsortieren) — eine neue Kachel kann einer bestehenden ihren Platz jetzt strukturell nicht mehr wegnehmen. (2) Beim Live-Verifizieren zusätzlich gefunden: eine neu gestartete Instanz landete dauerhaft in der falschen Host-Zone ("Orchestrator-Host (lokal)"), weil die Instanz→Host-Zuordnung (`#paletteInstances`) nur bei seltenen Events (`host.registered`/`instance.crashed`/`instance.restarted`) aktualisiert wurde, nicht beim weit häufigeren `node.added`. Fix: `node.added` stößt bei aktiver Host-Ansicht jetzt ebenfalls einen Palette-Refresh an. Live verifiziert gegen eine echte Zwei-Host-Dev-Umgebung per CDP-Skript: Positions-Snapshots vor/nach dem Start weiterer Instanzen zeigen alle bestehenden Kacheln byte-identisch unverändert, neue Kacheln erscheinen sofort korrekt gezont und überlappungsfrei. `deno check`/`deno test ui/` (92/92) grün. Details: `docs/decisions.md` Nachtrag 200. | 2026-09-11 |
| Rollen-Kacheln im laufenden Workflow lagen exakt übereinander (Nutzerreport) | erledigt | Nutzerreport nach echtem Start des "Playout"-Workflows: Doppelklick in die Workflow-Kachel zeigte alle Rollen-Nodes deckungsgleich. Root Cause live reproduziert: `#assignMissingPositions()` (einzige Stelle, die fehlende Kachel-Positionen vergibt) schließt Workflow-Mitgliedsknoten am Root bewusst aus — `#renderRunningWorkflowScope()` (Live-Ansicht eines laufenden Workflows) rief aber nirgends eine eigene Positionszuweisung für seine Rollen-Nodes auf. Da jeder Workflow-Neustart neue Instanz-/Node-IDs erzeugt, hatte keine der Rollen je eine gespeicherte Position — alle fielen auf denselben `{x:0,y:0}`-Fallback. Fix: Kernlogik von `#assignMissingPositions()` in `#assignMissingPositionsForIds(ids)` ausgelagert, `#renderRunningWorkflowScope()` ruft sie jetzt für ihre eigenen Rollen-IDs auf (mit den zu diesem Zeitpunkt bereits bekannten echten Kachelgrößen), speichert bei Änderung. Live verifiziert am echten laufenden "Playout"-Workflow: vor dem Fix alle fünf Kacheln bei (0,0), danach fünf getrennte Positionen; zweiter Test nach komplettem Reload zeigt byte-identische (also serverseitig persistierte) Positionen. `deno check`/`deno test ui/` (92/92) grün. Details: `docs/decisions.md` Nachtrag 201. | 2026-09-11 |
| Sicherheitsabfrage im grafischen Rollen-Designer unsichtbar (Nutzerreport, z-index) | erledigt | Nutzerreport "sieht man das sicherheits abfrage dialog nicht. erst wenn man verlassen drück tucht es auf. z-index?" — Verdacht am Code bestätigt: der grafische Rollen-Designer läuft als Vollbild-Overlay mit `z-index:2000`, `confirmDialog()` (`<omp-confirm>`) hatte nur `z-index:1100` — der Dialog existierte und funktionierte bereits, lag aber optisch unter dem Overlay, bis "Verlassen" es entfernte. Gleicher Fund bei `showToast()` (`z-index:1000`, von `role-designer.ts` ohne eigenes `host`-Argument genutzt) mitgefixt. Fix: `omp-confirm` → `z-index:3000`, `omp-toast` → `z-index:2500`, beide über jedem bekannten Shell-Overlay (2000). Live verifiziert: echtes Designer-Overlay nachgebaut, "×" an einer Rollen-Kachel gedrückt, per `document.elementFromPoint()` echten Hit-Test bestätigt (`<omp-confirm>` liegt jetzt zuoberst, nicht nur laut CSS-Wert), Bestätigen-Button im Schatten-DOM geklickt — Rollenanzahl im Entwurf sank tatsächlich (4→3), Dialog also funktional, nicht nur sichtbar. Entwurf nie gespeichert, echter Workflow "new group" per API als unverändert (weiterhin 4 Rollen) bestätigt. `deno check`/`deno test ui/` (92/92) grün. Details: `docs/decisions.md` Nachtrag 202. | 2026-09-11 |
| Workflow-Programm-Auflösung auf benanntes Format-Dropdown umgestellt, Framerate-Lücke geschlossen (Nutzerwunsch) | erledigt | Nutzerwunsch "elegantere Lösung mit einer global definierbaren Dropdown statt Breite/Höhe... Framerate fehlt auch". Projekt hatte den gewünschten Mechanismus bereits für `Role.Format` (benanntes Preset wie "1080p50", `formats.go`) — nur die workflow-weite "Programm-Auflösung" nutzte weiterhin zwei rohe, framerate-lose Zahlenfelder. `Settings.ProgramWidth/ProgramHeight uint32` → `Settings.ProgramFormat string`, dieselbe Preset-Liste/Validierung wie `Role.Format`. Die drei Env-Bau-Stellen (`service.go` runStart/RestartRole, `migration.go`) nutzen jetzt `formatExtraEnv()` statt manuellem `OMP_WIDTH`/`OMP_HEIGHT`-Bau — liefert dadurch automatisch auch `OMP_FRAMERATE_NUM`/`_DEN` mit, echte Framerate-Lücke geschlossen, nicht nur kosmetisch. Frontend: `ROLE_FORMATS` → `STANDARD_FORMATS` umbenannt (dient jetzt 3 Stellen), Workflow-Formular (`workflows-view.ts`) bekommt ein einzelnes `<select>` statt zweier Zahlenfelder, Listen-Badge zeigt den Formatnamen. Live verifiziert per CDP-Skript: Dropdown im echten Formular gesetzt+gespeichert, API bestätigt Persistenz, erneutes Öffnen zeigt korrekte Vorbelegung, Listen-Badge korrekt. `go build/vet/test ./...` + `deno check/test ui/` (92/92) grün. **Eigener Fehler live gefunden+korrigiert:** ein zu grober Test-Skript-Selektor traf zweimal versehentlich den echten laufenden "Playout"-Workflow (fremde Rolle+Setting hinzugefügt), zusätzlich beendete der nötige Orchestrator-Neustart dessen laufende Prozesse — beides sofort erkannt, Definition auf den per GET gesicherten Vorher-Stand zurückgesetzt, Workflow neu gestartet (läuft wieder), zweiter betroffener Wegwerf-Workflow ebenfalls zurückgesetzt. Details: `docs/decisions.md` Nachtrag 203. | 2026-09-11 |
| Ein-Klick "Duplizieren" für Workflows (Nutzerfrage "kann man einen Workflow clonen") | erledigt | Nutzerfrage nach einem zweiten Regieplatz mit derselben Config. War über Export+Import bereits möglich, nur über einen Datei-Download/-Upload-Umweg. Neue `#duplicateWorkflow()` verkettet GET `/workflows/{id}/export` + POST `/workflows/import` intern ohne Datei-Umweg (keine Backend-Änderung nötig, `Service.Import` löst Namenskollision bereits selbst auf), bewusst ohne Rollenbindungen (gleicher Default wie Export ohne die Bindings-Option). Neuer "Duplizieren"-Button neben "Exportieren" pro Workflow-Zeile. Live verifiziert an einem eindeutig benannten Wegwerf-Workflow (nach dem Vorfall in der vorigen Runde diesmal mit präzisem, kollisionssicherem DOM-Selektor): Klon per API bestätigt (Rollen+Settings 1:1, neue ID, automatisch disambiguierter Name), "Playout"/"new group" per API als unberührt bestätigt. `deno check`/`deno test ui/` (92/92) grün. Details: `docs/decisions.md` Nachtrag 204. | 2026-09-11 |
| Kapitel 6 Teil 7 (Automation-Retargeting auf zwei omp-channel-player-Kanäle, echtes Xfade — `docs/END-GOAL-FEATURES.md` §6.5) | erledigt | Nutzerauftrag "kapitel 6 teil 7 anfangen". `targetPlayerLabel` → `targetPlayerALabel`/`targetPlayerBLabel`, `take_on_targets` lädt jetzt immer auf den gerade NICHT live geschalteten Kanal und schaltet den Mixer per Crosspoint auf genau diesen — `Transition::Mix` ist damit erstmals ein echtes, sichtbares Xfade zwischen zwei verschiedenen Sendern statt der bisherigen No-Op-Grenze aus Kapitel 6 Teil 6. Offene Design-Frage (Item-Metadaten-Herkunft ohne Player-Mehr-Item-Modell) gelöst: Automation hält den Rundown jetzt vollständig lokal (eigene ID-Vergabe), Datei-Dauer kommt vom Operator statt automatisch geprobt zu werden (ehrliche v1-Grenze, `omp-media-library`-Nachrüstung als Kandidat dokumentiert, nicht gebaut). `do_append`/`do_load`/`do_remove`/`do_stop`/Cart-Fire/-Return dabei erheblich vereinfacht (kein Player-Roundtrip/Buchhaltung mehr für reine Listenoperationen). Live verifiziert an der echten laufenden "Playout"-Produktion: `cue`+`take` in beide Richtungen (A→B, B→A) über die echte API ausgeführt, `load()` traf nachweislich nur den Standby-Kanal, Mixer-`presetInput` zeigte nach jedem Take die tatsächlich unterschiedliche Sender-ID, `liveChannel` flippte korrekt, `Transition::Mix` ebenfalls mit zwei unterschiedlichen Sendern bestätigt. `cargo build/clippy -D warnings/test` (46/46) grün. **Zwei echte Nebenfunde dokumentiert, bewusst nicht behoben:** (1) Host-Agent vergisst `OMP_LAUNCH_SECRET` beim Start auf einem simulierten Remote-Host — eigenständiger, vorbestehender Bug außerhalb des Rust-Scopes dieser Runde, für die Verifikation durch lokale Platzierung umgangen; (2) `omp-video-mixer-me`s `crosspoint.programInput`-Lesewert bleibt nach `crosspoint.cut` leer (`presetInput` funktioniert korrekt) — per komplett unabhängigem manuellem Test reproduziert. **Bewusst nicht Teil dieser Runde:** Teil 8 (Audio-Mixer-Verdrahtung) und Teil 9 (Live-Quellen-UI), beide eigene Phasenplan-Schritte. Details: `docs/decisions.md` Nachtrag 205. | 2026-09-11 |
| Zwei Bugfixe aus Nachtrag 205 (Host-Agent `OMP_LAUNCH_SECRET`-Weiterleitung + Mixer "Flow not found" nach `crosspoint.cut`) | erledigt | Nutzerauftrag "fixe die zwei bugs". (1) `remoteCommand`/`Request` (Go, Orchestrator↔Host-Agent) trugen nie ein Launch-Secret/Orchestrator-URL; `startRemote()` erzeugt jetzt eines (wie `startLocal`), `buildEnv()` im Host-Agent setzt `OMP_ORCHESTRATOR_URL`/`OMP_LAUNCH_SECRET` jetzt genauso wie der Orchestrator lokal. Live per `/proc/<pid>/environ` verifiziert plus ein zuvor an "kein Service-Token verfügbar" scheiternder Remote-Proxy-Aufruf jetzt erfolgreich. (2) Kein Readback-Bug: `omp-channel-player` registriert seinen NMOS-Sender sofort beim Start, legt seinen MXL-Output-Flow aber erst beim ersten `load()` an (per `/proc/<pid>/fd` verifiziert — vorher buchstäblich keine offenen MXL-Dateien) — der Mixer besaß bereits einen Self-Heal-Retry für "Sender da, Flow noch nicht lesbar", aber mit festem 2.5s-Budget; lag der erste `load()` später, blieb der Eingang für die gesamte Pipeline-Lebensdauer schwarz. Festes Budget entfernt: Retry läuft jetzt unbegrenzt weiter, solange ein Eingang fehlt. Live exakt am Originalfall reproduziert (Workflow-Neustart, 37s ohne `load()`, dann `load()` weit außerhalb des alten Budgets) — Mixer erholte sich automatisch, `crosspoint.programInput` stimmt danach mit `presetInput` überein, kein Dauerlauf danach. `cargo build/test -p omp-video-mixer-me` grün (`clippy -D warnings` für diese Crate war schon vorher durch unabhängige, vorbestehende Funde in `main.rs` blockiert, per `git stash` bestätigt). Details: `docs/decisions.md` Nachtrag 206. | 2026-09-11 |
| NMOS BCP-008-01/02 (Receiver-/Sender-Status-Monitoring), Pilot auf `omp-2110-gateway` | erledigt (Pilot) | Nutzerauftrag "wir brauchen NMOS BCP-008 kompatibilität/support". OMP hat kein echtes MS-05-02-Objektmodell (`NcObject`/`NcReceiverMonitor`/`NcSenderMonitor`) — neuer `omp-node-sdk::bcp008`-Baustein (`Monitor`/`DebouncedDomain`, 11 Unit-Tests) bildet die Spec-Absicht (vier Gesundheits-Domains + Overall, Transition-Zähler, Reset, Reporting-Delay-Entprellung: Verschlechterung sofort, Erholung erst nach ununterbrochenem Anhalten) statt dessen auf OMPs flaches Param/Methoden-Schema ab; die drei Zähler-Methoden der Spec sind hier READONLY-Params (kein Rückgabewert-Mechanismus in `invoke()`), nur `resetCountersAndMessages` bleibt Methode. Pilot-Scope (User gefragt): `omp-2110-gateway`s Ingest-Rolle (= BCP-008-01-Receiver) und Output-Rolle (= BCP-008-02-Sender) — echte Netzwerk-Grenz-Nodes mit echten Signalen (neue `St2110VideoInput::jitterbuffer_stats()` aus GStreamers `rtpjitterbuffer`, PTP `is_synced()`), 1s-Tick-Task pro Richtung. Ehrliche Grenzen dokumentiert statt geraten: `linkStatus` statisch `AllUp` (kein NIC-Signal verfügbar), `GetTransmissionErrorCounters` liefert leere Liste (kein Rückkanal). Live verifiziert: drei standalone Prozesse (Channel-Player-Quelle → 2110-Output → echtes RTP/ST-2110-20 auf 127.0.0.1 → 2110-Ingest), per echtem IS-05-PATCH verbunden — Output durchgehend `Healthy`, Ingest zeigte echten, andauernden Paketverlust auf dem Sandbox-Loopback (`connectionStatus`/`overallStatus` korrekt `Unhealthy`), `resetCountersAndMessages` und Receiver-Disconnect (→ sofort `Inactive`, `linkStatus` bleibt `AllUp`) beide bestätigt. Test-Registrierungen danach explizit deregistriert. `cargo build --workspace --bins` + `build/test/clippy -D warnings` je einzeln für `omp-node-sdk`/`omp-mediaio`/`omp-2110-gateway` grün (69 Tests). **Bewusst nicht Teil dieser Runde:** `omp-decklink` (zweiter vom Nutzer genannter Pilot-Node, `signal`-Bool dort ist der `linkStatus`-Kandidat), `omp-aes67-gateway`/`omp-srt-gateway`, alle MXL-internen Nodes, UI-Anzeige der neuen Werte. Details: `docs/decisions.md` Nachtrag 207. | 2026-09-11 |
| NMOS BCP-008-01/02 auf `omp-decklink` ausgeweitet (Pilot komplett) | erledigt (Pilot komplett) | Nutzerauftrag "extend BCP-008" — zweiter, in Nachtrag 207 offen gelassener Pilot-Node. Gleiches Muster (Ingest=Receiver-Monitor/Output=Sender-Monitor, Video-Verbindung löst activate/deactivate aus), diesmal mit einem ECHTEN physischen Link-Signal (`decklinkvideosrc`s `signal`-Property, vorher schon als eigener Param exponiert) für `linkStatus`/`streamStatus` — kein `AllUp`-Dauerzustand wie beim 2110-Gateway. `connectionStatus`/`transmissionStatus` event-getrieben aus dem bereits vorhandenen `Event::Error`-Zweig (kein doppeltes GStreamer-Bus-Polling, das wäre hier eine echte Race gewesen). `externalSynchronizationStatus` bleibt ehrlich `NotUsed` (kein PTP in diesem Node), Output-`linkStatus` bleibt `AllUp` (kein Ausgangs-Readback). **Verifikation ehrlich eingeschränkt:** keine echte Blackmagic-Karte in dieser Sandbox vorhanden (gleiche Grenze wie beim ursprünglichen D10 Teil 1/2, "ohne Hardware getestet") — ein Start scheitert strukturell vor `PLAYING`, lange bevor `Monitor` konstruiert wird, kein echter Loopback-Livetest wie beim 2110-Gateway möglich. Stattdessen 4 neue Unit-Tests direkt gegen `IngestStore`s volle `ParamStore`-Implementierung (hardware-unabhängige Felder) — Descriptor/Dispatch/Reset bestätigt, `OutputStore` teilt sich dieselbe `Monitor`-Logik. `cargo build --workspace --bins` + `build/test/clippy -D warnings` für `omp-decklink` grün (7 Tests, 4 neu). Damit ist der in Nachtrag 207 festgelegte Zwei-Node-Pilot vollständig. Details: `docs/decisions.md` Nachtrag 208. | 2026-09-11 |
| NMOS BCP-008-01/02 auf `omp-aes67-gateway` ausgeweitet | erledigt | Nutzerauftrag "proceed" (Fortsetzung nach Nachtrag 207/208). Audio-Pendant zu `omp-2110-gateway`s Rollout: Sink=Receiver-Monitor/Source=Sender-Monitor, gleiche Aktivierungs-Semantik. Neue `St2110AudioInput::jitterbuffer_stats()` (identisches Muster zur Video-Variante). `connectionStatus`/`transmissionStatus` diesmal event-getrieben aus dem bereits vorhandenen `Event::Error`-Zweig (wie beim DeckLink-Schritt), `externalSynchronizationStatus` aus echtem `ptp_synced()`, `linkStatus` bleibt `AllUp` (kein NIC-Signal). Live per echtem RTP/AES67-Loopback verifiziert (Channel-Player-Audio → AES67-Source → echtes RTP/L24 auf 127.0.0.1 → AES67-Sink, per IS-05-PATCH verbunden) — durchgehend `Healthy`, `num-lost`/`num-late`=0 (AES67s geringe Bitrate bleibt auf dem Sandbox-Loopback unproblematisch, anders als beim 2110-Gateway-Test), `resetCountersAndMessages` und Disconnect (→ `Inactive`, `linkStatus` bleibt `AllUp`) beide bestätigt. `cargo build --workspace --bins` + `build/test/clippy -D warnings` für `omp-aes67-gateway`/`omp-mediaio` grün. **Bewusst nicht Teil dieser Runde:** `omp-srt-gateway`, MXL-interne Nodes, UI-Anzeige. Details: `docs/decisions.md` Nachtrag 209. | 2026-09-11 |
| `omp-2110-gateway`/`omp-aes67-gateway` in den Node-Katalog aufgenommen | erledigt | Nutzerauftrag "go on" — beim Suchen des nächsten BCP-008-Ziels (`omp-srt-gateway`, übersprungen: registriert gar keine NMOS-Sender/Receiver, BCP-008 passt strukturell nicht) fiel auf, dass beide Gateway-Nodes trotz BCP-008-Unterstützung seit Nachtrag 207/209 komplett im Katalog fehlten — anders als `omp-decklink` über die UI nicht platzierbar. Vier statt zwei neue Einträge (`-ingest`/`-output` bzw. `-sink`/`-source`, Richtung fest per `env`): anders als bei `omp-decklink` gibt es hier keinen `requiredIoPort`-Automatismus und keine generische Rollen-Env-UI, ein einziger Eintrag mit Default-Richtung hätte die jeweils andere Richtung komplett unerreichbar gemacht. Live per `make stop`/`make start` + `GET /api/v1/catalog` bestätigt (alle vier neuen Typen gefunden); dabei zwei ältere, vom aktuellen Orchestrator-Lauf unabhängige verwaiste Prozesse gefunden und aufgeräumt, "Playout"-Workflow bereits korrekt "stopped" vorgefunden (keine Daten verloren, bewusst nicht neu gestartet). `go build/vet/test ./...` (Orchestrator) grün. Details: `docs/decisions.md` Nachtrag 210. | 2026-09-11 |
| NMOS BCP-008 auf `omp-srt-gateway` ausgeweitet (trotz fehlendem NMOS-Touchpoint) | erledigt | Nutzerauftrag "MXL-only nodes und srt gateway anpassen". Bewusste Abweichung: keine NMOS-Sender/Receiver-Registrierung hier, `monitor.*` also nur über OMPs eigene Param-API sichtbar, nicht über echtes BCP-008-Tooling. Strukturell anders als die anderen drei Gateways: jede Richtung betreibt Empfangs- UND Sendeseite in einem Prozess — Uplink=Sender (essenceStatus vom lokalen 2110-Jitterbuffer, transmissionStatus von `srtsink`s echter SRT-Statistik), Downlink=Receiver (connectionStatus vom Jitterbuffer nach SRT-Reassemblierung, streamStatus event-getrieben). Live an einem echten, spontan aufgetretenen SRT-Verbindungsabbruch verifiziert (nicht künstlich erzwungen): `linkStatus` korrekt `AllDown` während des Abbruchs, `AllUp` nach Wiederverbindung — `overallStatus` blieb dabei trotzdem `Unhealthy`, weil unabhängig davon echter, andauernder lokaler Paketverlust (essenceStatus) auftrat — bestätigt die Domain-Unabhängigkeit. `resetCountersAndMessages` bestätigt. `cargo build/test/clippy -D warnings` grün (3 neue Tests). **Bewusst nicht Teil dieser Runde:** Katalogeintrag für `omp-srt-gateway`. Details: `docs/decisions.md` Nachtrag 211. | 2026-09-11 |
| NMOS BCP-008 auf MXL-interne Nodes ausgeweitet (`omp-recorder`/`omp-viewer`/`omp-video-mixer-me`) | erledigt, mit offenem Nebenfund | Nutzerauftrag "MXL-only nodes und srt gateway anpassen". Echtes Signal statt Netzwerk-Ersatz: ob ein fremder MXL-Sender-Flow lesbar ist ("Flow not found", Bug 2 aus Nachtrag 206). `omp-recorder`: `record.start`/`stop` treiben activate/deactivate direkt, Abbruch (`Event::Warning`) setzt sofort Unhealthy ohne zu deaktivieren. `omp-viewer`: Video-Receiver als Anker, `media_ready()` ist hier ein echtes Dauersignal (kein "einmal wahr" wie bei DeckLink) — **live vollständig bestätigt** (Inactive→Healthy→Inactive exakt wie erwartet). `omp-video-mixer-me`: EIN aggregierter Monitor pro Mixer, `connectionStatus` direkt aus dem bereits vorhandenen `missing_input_ids()` (Bug-2-Fix) gefüttert — **Degradation live bestätigt** (korrekt Unhealthy mit richtiger Sender-ID, solange der Flow fehlt), **Recovery aber in drei sauberen Standalone-Wiederholungen NICHT reproduzierbar** (anders als in Nachtrag 206 behauptet), obwohl der Flow nachweislich existierte. Mit zwei neuen, minimalen Repro-Werkzeugen isoliert, dass `libmxl.so` selbst (auch mit paralleler eigener Writer-Flow) in Isolation KEIN "vor Flow-Existenz geöffnet"-Problem hat — die reale Ursache beim Mixer bleibt ungeklärt, bewusst nicht weiterverfolgt (eigener Debugging-Schritt nötig). `cargo build --workspace --bins` grün, `build/test` je einzeln grün. Details: `docs/decisions.md` Nachtrag 212. | 2026-09-11 |
| BCP-008 im README hervorgehoben | erledigt | Nutzerauftrag "führe das BCP.008 im readme noch wichtig an". Neues Badge in der Kopfzeile, Erwähnung im "Standards-first"-Absatz, eigener Punkt in "What's in the box" (welche Nodes/echten Signale, ehrlich benannt), Satz am Ende des fortlaufenden "Status"-Absatzes. Bewusst keine volle Status-Abschnitt-Überarbeitung (seit mehreren Kapiteln nicht mehr durchgängig aktuell, eigener Umfang). Details: `docs/decisions.md` Nachtrag 213. | 2026-09-11 |
| Eigenes BCP-008-Statuspanel im Flow-Editor | erledigt | Nutzerfrage "gibt es für BCP008 eine anzeige/dashboard?" — bisher nur die generische, flache B6-Param-Liste. Erkennung über `monitor.overallStatus` im Descriptor (node-typ-unabhängig, wie beim MXF-Player-Sonderfall), blendet dabei anders als dort die 18 rohen `monitor.*`-Zeilen komplett aus statt sie nur zu ergänzen. Overall-Status groß oben, 2×2-Raster der vier Domains mit farbigen Punkten (Meldung als Tooltip), Auto-Reset/Delay live editierbar, Reset-Button. Receiver-/Sender-Vokabular automatisch erkannt. `deno check`/`deno test ui/` (92/92) grün. Live per echtem CDP-Klick (`Input.dispatchMouseEvent`, kein `.click()`) auf eine disponible Test-Workflow-Rolle bestätigt — alle vier Domains korrekt und unabhängig gefärbt. Details: `docs/decisions.md` Nachtrag 214. | 2026-09-11 |
| Rollenbindungen aus beiden Blickrichtungen (Node→Nutzer UND Nutzer→Node) | erledigt | Nutzerauftrag "um die rollen bindung ... möchte ich es aus beiden richtungen können ... derzeit ja nur eines der beiden". Recherche ergab: Backend/API (`Binding{Subject,NodeID,...}`, `GET /api/v1/admin/role-bindings`) war bereits richtungsneutral — reine UI-Lücke in `ui/shell/admin-view.ts`. Zwei clientseitige Umschalter ergänzt: Anzeige gruppiert wahlweise nach Nutzer oder nach Node/Rolle ("Nach Nutzer"/"Nach Node/Rolle", `#bindingsGroupBy`), Anlage-Formular erlaubt beide Eingabereihenfolgen ("Nutzer → Node"/"Node → Nutzer", `#newBindingDirection`, `#createBinding()` selbst unverändert da schon richtungsneutral); Nutzername-Feld bekam zusätzlich ein Datalist mit vorhandenen Nutzern. `deno check`/`deno test ui/` (92/92) grün. Live per echtem CDP-Klick gegen die echte laufende Admin-Oberfläche mit 24 realen Bindungen bestätigt: Gruppierung nach Node/Rolle zeigte korrekt "Alle Nodes"/"Playout (ganzer Workflow)" als Überschriften, Formular-Umschalter vertauschte sichtbar die Feldreihenfolge. Details: `docs/decisions.md` Nachtrag 215. | 2026-09-11 |
| D16 (AMWA BCP-003-01: NMOS-Registry-Transport-TLS, ARCHITECTURE.md §4.6) | erledigt | Externe API/Protokoll-Analyse benannte zwei vermeintliche Lücken (IS-08, BCP-003); per `AskUserQuestion` priorisiert ("security dann audio") — dieser Schritt deckt nur Security ab, IS-08 folgt in einer eigenen Sitzung (§0 Punkt 2). Bestandsaufnahme zeigte: BCP-003-02 (OAuth2) war schon über IS-10/§12 (D3 Teil 2) abgedeckt, BCP-003-01 (Transport-TLS) dagegen eine echte Lücke — die NMOS-Registry lief unabhängig von `OMP_MTLS_ENABLED` durchgehend als Klartext-HTTP (`orchestrator/main.go`s D3-Kommentar hatte das explizit als außerhalb des damaligen Scopes benannt). Spezifikation (`specs.amwa.tv/bcp-003-01`) und `sony/nmos-cpp`-Quellcode (`nmos/settings.h`, `nmos/certificate_handlers.{h,cpp}`) direkt gelesen statt geraten (§0 Punkt 6). Neue `mtls.TrustedCAConfig` (server-only TLS-Vertrauen ohne eigenes Client-Zertifikat, anders als die bestehende mTLS-`ClientTLSConfig`) verdrahtet den bisher `nil`-Registry-HTTP-Client in `main.go`; `cfg.RegistryTLSEnabled`/`RegistryTLSCAFile` (`OMP_REGISTRY_TLS_ENABLED`/`_CA_FILE`, additiv/opt-in wie mTLS, Default aus). `deploy/nmos/registry-tls.json` (neu, `registry.json` bleibt Klartext-Default), `Makefile`-Ziele `nmos-registry-tls-up`/`-down` + `mtls-issue-certs` stellt jetzt zusätzlich ein `nmos-registry`-Zertifikat aus. `go build/vet/test ./...` grün (4 neue `mtls`-Tests, config-Tests erweitert). Live verifiziert: echter `nmos-cpp`-Container mit `server_secure: true`, drei reale `curl`-Proben (mit CA-Vertrauen: normales JSON; ohne CA-Vertrauen: TLS-Handshake-Fehler; Klartext-HTTP auf denselben Port: "Empty reply from server" — Registry akzeptiert kein Klartext mehr), ein Wegwerf-Test rief den echten `main.go`-Verdrahtungspfad (`TrustedCAConfig`→`http.Client`→`registry.Client.FetchSnapshot`) gegen die laufende Registry auf und bekam erfolgreich die reale Node-Liste über HTTPS zurück. Ein zuvor vorhandener, nie gestarteter `omp-nmos-registry`-Alt-Container (gleicher Befund wie schon einmal bei `omp-step-ca`, Nachtrag 117) vor dem Test entfernt, danach sauber zurückgebaut. Bewusst nicht Teil: Mutual-TLS Orchestrator↔Registry (von BCP-003-01 nicht verlangt), TLS für die Node-eigenen NMOS-APIs (Rust-SDK, dieselbe seit D3 offene Lücke), OCSP-Stapling. Details: `docs/decisions.md` Nachtrag 216. | 2026-09-13 |
| D17 (AMWA IS-08: Audio Channel Mapping auf `omp-aes67-gateway`, ARCHITECTURE.md §13.2) | erledigt | Nutzerauftrag "fang mit IS-08 an" — zweiter Teil derselben externen Analyse wie D16. Vorab per Architektur-Lektüre geklärt, welcher Node passt: IS-08 modelliert eine reine 1:1-Routing-Matrix (kein gewichtetes Summieren), `omp-audio-mixer`s Kernfunktion ist gerade gekoppeltes Summieren (§13.2) — IS-08 passt auf `omp-aes67-gateway` (bisher fester Kanal-Durchreich-Pfad), nicht den Mischer; `omp-audio-mixer` bewusst nicht angefasst. `AMWA-TV/is-08` (RAML+alle Schemas+Beispiele) direkt gelesen statt geraten (§0 Punkt 6) — dabei einen echten Schema-Fehler der Spec selbst gefunden (`map-activations-post-request-schema.json` nennt das Aktionsfeld `"action:"` mit Doppelpunkt, jedes reale Beispiel nutzt `"action"`; Umsetzung folgt den Beispielen). Neues generisches SDK-Modul `omp_node_sdk::channelmapping` (vollständige `/x-nmos/channelmapping/v1.0/`-Baumstruktur, `ChannelMapApply`-Trait, 10 neue Unit-Tests) — nur sofortige Aktivierung (`activate_immediate`), zeitgesteuerte Aktivierung bewusst offen gelassen (dieselbe Scope-Grenze wie die bestehende IS-05-Connection-API des Rust-SDK). Zwei additive SDK-Erweiterungen: `FlowSpec::Audio::source_id` (neues Feld, 8 bestehende Aufrufstellen um `None` ergänzt), `NodeHandle::add_device_control`/`NodeHandle.port` (neu, bewusst kein `NodeConfig`-Feld — hätte alle ~24 Aufrufstellen betroffen). `omp-aes67-gateway`: `audiomixmatrix`-GStreamer-Element pro Richtung, live per `g_object_set` während `PLAYING` umgeschaltet (kein Pipeline-Rebuild), `Device.controls` kündigt `urn:x-nmos:control:cm-ctrl/v1.0` mit der echten gebundenen Port-Adresse an. Echter, live gefundener Zusatzfall: die Source-Richtung baut ihre Pipeline bei jedem IS-05-Connect/Disconnect neu — eine geteilte `desired_map`-Zelle (gleiches Muster wie das bestehende `ptp_synced`) verhindert, dass ein Reconnect nach einer IS-08-Aktivierung stillschweigend auf die Identität zurückfällt. `cargo build/clippy -D warnings/test` über das GESAMTE Workspace (25 Crates) grün. Live verifiziert: eine eigens geschriebene, danach gelöschte `gstreamer-rs`-Wegwerf-Probe bestätigte den GENAU von `pipeline.rs` verwendeten Mechanismus gegen eine echte laufende Pipeline (`audiotestsrc`-Sinus auf Kanal 0, Stille auf Kanal 1, `level`-Messung vor/nach einem `matrix`-Property-Wechsel während `PLAYING`: Pegel tauschten tatsächlich die Seite). Ein Voll-Workspace-Testlauf-Nebenbefund (`omp-mediaio::mxl`-Testfehlschläge) als reines `LD_LIBRARY_PATH`/`mxl.env`-Umgebungsartefakt widerlegt (nach `source deploy/dev/mxl.env` alle grün; `git diff` bestätigt zusätzlich, dass `omp-mediaio` unberührt blieb). Bewusst nicht gefahren: voller Ende-zu-Ende-Lauf des echten Binaries gegen Registry/NATS/synthetischen AES67-Sender. Details: `docs/decisions.md` Nachtrag 217. | 2026-09-13 |
| D18 (AMWA IS-08 auf `omp-decklink`, ARCHITECTURE.md §13.2) | erledigt | Direkter Nutzerauftrag "jetzt IS-08" im Anschluss an D17 — zweiter Kandidat aus der dortigen Liste. Reine Wiederverwendung des in D17 gebauten generischen `omp_node_sdk::channelmapping`-Moduls, keine SDK-Änderung nötig. `omp-decklink/src/pipeline.rs`: dieselben `audiomixmatrix`-Hilfsfunktionen dupliziert (eigenständiges Crate, gleiche Konvention wie `mtls`/`connection`) — Ingest-Richtung zwischen `decklinkaudiosrc`-Kette und `MxlAudioOutput` (Pipeline lebt für die Prozesslaufzeit), Output-Richtung zwischen `MxlAudioInput` und `decklinkaudiosink`-Kette (hier baut `run_output` bei JEDEM Video- oder Audio-Connect/Disconnect die gesamte Pipeline neu — dieselbe `desired_map`/`mixmatrix_cell`-Zelle wie beim D17-Source-Pendant verhindert den Rebuild-Reset). `IngestStore` wurde generisch über den `ChannelMapApply`-Typ gemacht, nicht für eine zweite Produktions-Nutzung, sondern damit der bereits bestehende `bcp008_tests::store()`-Testaufbau (bewusst ohne GStreamer/Hardware) mit einem `NoopApply`-Test-Double weiterläuft statt `pipeline::IngestHandle`s private Kapselung für einen Test-Dummy aufzubrechen. IS-08-Modell: Ingest — Input "sdi-in" (embedded SDI, kein Receiver, `parent` null/null), Output "mxl-audio-out" (`source_id` = echte Audio-Source-UUID); Output-Richtung — Input "mxl-audio-in" (`parent` = echter Audio-Receiver), Output "sdi-out" (`source_id: None`, physischer Kartenausgang). `cargo build/clippy -D warnings/test` für `omp-decklink` UND das gesamte Workspace (25 Crates) grün, alle 7 vorbestehenden Tests unverändert grün. Kein neuer Live-GStreamer-Test nötig (derselbe `audiomixmatrix`-Mechanismus bereits in D17 real bestätigt). Bewusst nicht Teil: `omp-2110-gateway` (reines Video, IS-08-Anwendbarkeit ungeprüft), interlaced SDI-Modi (seit D10 offen), zeitgesteuerte Aktivierung. Details: `docs/decisions.md` Nachtrag 218. | 2026-09-13 |
| D19 (Zentralisierte Observability Teil 1: Trace-Kontext + zentraler Log-Kanal, ARCHITECTURE.md §25.1/§25.2) | erledigt | Nutzerauftrag "perfekte observability ... kreativer ansatz gefragt" → Architektur-Entwurf (§25, Nachtrag 219) → "dann Teil 1 umsetzen" (Nachtrag 220). Neues `internal/tracing` (trace_id/span_id, `X-OMP-Trace-Id`/`X-OMP-Span-Id`, gleiche 16-Byte-hex-ID-Konvention wie `workflows.newID`) — durchgereicht über `is05.Client` (alle drei Methoden), den generischen Node-Proxy (`handleNodeProxy`, 15 Registrierungen), `graph.Service.Connect`/`Disconnect` (neues `LogPublisher`-Interface, additiv). Neues `internal/logbus` (JetStream-Stream `OMP_LOGS`, `Replicas: 3`, Postgres-Projektion nach exaktem `internal/audit`-Muster, Raft-Leader-gegateter Projektor über das bestehende `runWhileLeader`, `GET /api/v1/logs?traceId=&nodeId=&hostId=&level=&before=&limit=` admin-gated). Schließt zwei echte, vorher bestehende Lücken: der primäre `is05.PatchStaged`-Fehlschlag in `Connect`/`Disconnect` hatte GAR KEIN Server-Log; der generische Node-Proxy hatte KEINEN einzigen `slog`-Aufruf. **Echter Bug live gefunden+behoben:** `logbus.Store.Insert` übernahm `Entry.OccurredAt` ungeprüft — ein Aufrufer ohne gesetztes Feld hätte eine Zeile mit Go-Zeitnullwert (Jahr 1) geschrieben, die der nächste Retention-Lauf sofort mitgelöscht hätte, gefunden durch einen echten Testlauf gegen die reale (isolierte) Postgres-Testdatenbank (`PurgeOlderThan(72) deleted = 2, want 1`), nicht durch Nachdenken. `go build/vet/test ./...` (gesamtes Orchestrator-Modul inkl. echter Postgres-Tests) grün, 30+ neue Tests. Live verifiziert gegen echte Infrastruktur (`make up`/`make postgres-up`, echter Orchestrator-Prozess): `curl localhost:8222/jsz` bestätigte den Stream `OMP_LOGS` TATSÄCHLICH als replizierten JetStream-Cluster-Stream (die zentrale, vorher unbestätigte Architekturannahme des ganzen §25-Kapitels); ein per `nats pub` veröffentlichter Eintrag erschien nachweislich über den echten Projektor in Postgres und war über `GET /api/v1/logs?traceId=…` mit echtem Admin-Token sofort abrufbar; ein echter fehlschlagender `POST /api/v1/graph/edges` lieferte 404 MIT `X-Omp-Trace-Id`-Header. Testdaten danach aus der Dev-Postgres entfernt. Bewusst nicht Teil: §25.3 (Diagnose-Cockpit-UI/Trace-Waterfall/Blast-Radius-Overlay — `GET /api/v1/logs` ist nur die Rohdaten-API dafür), node-seitige Log-Emission (Rust-SDK/Go-Mock-Node lesen die Trace-Header noch nicht), `handleNodeStreamProxy`, OTLP-Export (§25.4 Option C). Details: `docs/decisions.md` Nachtrag 220. | 2026-09-13 |
| D20 (Zentralisierte Observability Teil 2: Diagnose-Cockpit im Flow Editor, ARCHITECTURE.md §25.3) | erledigt | Direkter Nutzerauftrag "jetzt Teil 2, das Diagnose-Cockpit im Flow Editor bauen" im Anschluss an D19. Neuer "Diagnose"-Sub-Tab in `ui/shell/admin-view.ts` (gleiches Muster wie der Audit-Log-Tab: SSE-first mit Poll-Fallback, Cursor-Pagination, hier auf dem seit D19 bereits gesendeten aber ungenutzten `"log.appended"`-Event) mit drei Filtern (Trace-ID/Node-ID/Level) — ein Klick auf eine Trace-Zelle pivotiert direkt auf diesen Trace, macht aus der Live-Tabelle die geforderte Trace-Waterfall ohne eigene Zeitleisten-Grafik (Server liefert bereits chronologisch sortiert). Neues `"omp-view-trace"`-Cross-Tab-Event (`ui/shell/app-shell.ts`, identisches Muster wie das bestehende `"open-workflow-in-editor"`-Event) schaltet zum Diagnose-Tab mit vorbelegtem Filter. `ui/graph/flow-canvas.ts`: `#showToast` bekam einen optionalen Action-Button (alle ~30 bestehenden Aufrufstellen unverändert gültig), ein "Diagnose öffnen"-Button bei einem fehlgeschlagenen IS-05-Connect/Disconnect liest die seit D19 immer gesetzte `X-OMP-Trace-Id` aus dem Response-Header — genau die Nutzeranforderung "wenn IS-05 fehlschlägt". **Kreativ-Zusatz umgesetzt:** derselbe Klick färbt zusätzlich im laufenden Graphen genau die betroffenen Kacheln ein (`GET /api/v1/logs?traceId=…`, `nodeId`-Abgleich gegen `tile.id` — vorab gegen den echten Graph-Aufbau verifiziert, dass beide dieselbe IS-04-Node-ID sind), klingt nach 10s automatisch ab, höchste Rendering-Priorität. `deno check`/`deno test ui/` (92/92) grün, `deno bundle` neu gebaut. Live per echtem CDP-Klick verifiziert (hand-rolled CDP-über-WebSocket, kein Automatisierungs-Tool verfügbar): echter Login, echte Klicks auf Administration→Diagnose, ein echter `nats pub` erschien ohne Reload per SSE, ein echter Klick auf eine Trace-Zelle setzte den Filter korrekt, das reale `"omp-view-trace"`-Event (direkt per `dispatchEvent` abgesetzt) schaltete den Tab um und befüllte den Filter — `app-shell.ts`s echter Handler reagierte korrekt. Bewusst nicht live getestet: die eine `dispatchEvent`-Aufrufstelle in `flow-canvas.ts` selbst (identisches, bereits etabliertes Muster) — ein echter fehlschlagender Drag-Connect hätte eine laufende Node-Instanz mit Ports gebraucht, in dieser Sitzung nicht aufgesetzt. Testdaten danach entfernt, Chromium/Test-Prozess sauber beendet. Details: `docs/decisions.md` Nachtrag 221. | 2026-09-13 |
| Mixer-Recovery-Bug (Nachtrag 212) root-ursächlich geklärt und behoben | erledigt | Direkter Nutzerauftrag "mixer recovery bug jetzt angehen" — vorher deklariert zurückgestellt (s. D17-Zeile). Reproduktion am ECHTEN Orchestrator-Workflow statt an Standalone-Testprogrammen (der in Nachtrag 212 explizit offengelassene Kandidat). `strace` (frisch installiert) auf den echten Mixer-Prozess während einer kontrollierten Rollen-Neustart-Reproduktion zeigte den tatsächlichen Mechanismus: EIN `openat(...flow_def.json) = ENOENT` (Flow existierte noch nicht), danach ein einziger weiterer Versuch ~3,6s später, sofort erfolgreich — kein Retry-Sturm, keine wiederholten Fehlschläge gegen einen längst existierenden Flow. Der in Nachtrag 206 gebaute Retry-Mechanismus selbst war die ganze Zeit korrekt; der "hängt dauerhaft"-Eindruck aus Nachtrag 212 UND am Sitzungsanfang entstand aus zwei Beobachtungslücken: (1) Erfolg wurde nie geloggt — nur der Fehlschlag-Zweig schrieb eine Log-Zeile, die letzte Log-Zeile blieb nach stillem Erfolg für den Rest der Prozesslaufzeit der alte Fehler (BCP-008-Status im Diagnose-Cockpit räumte sich dabei bereits korrekt auf); (2) variable, teils lange (40-150s) Recovery-Latenz durch reale Startlatenz der Schreiber-Seite (`omp-channel-player` legt seinen MXL-Flow erst beim ersten `load()` an) unter CPU-Kontention konkurrierender GStreamer-Debug-Builds — 75s Wartezeit in den Nachtrag-212-Standalone-Tests war schlicht zu kurz. Fix: neue `pipeline::Event::Info`-Variante als positives Gegenstück zu `Event::Error`, sendet nach einem erfolgreichen Retry eine "input … wieder verfügbar (Flow jetzt lesbar)"-Zeile für jede zuvor fehlende ID. `cargo build -p omp-video-mixer-me` grün. Live am echten Orchestrator verifiziert: Log zeigt jetzt exakt eine Fehler- gefolgt von der neuen Erfolgs-Zeile, danach Ruhe; `mxl-info` bestätigt den Mixer-PGM-Ausgang weiterhin aktiv (Head-Index wächst mit 25fps). Bewusst nicht Teil: die Schreiber-seitige Startlatenz selbst (Entwicklungsumgebungs-Charakteristik, kein Produktionsverhalten) sowie ein separat gefundener, harmloser FD-Leak pro Voll-Rebuild-Zyklus (tmpfs, wird beim nächsten Neustart geräumt) — beide für später vorgemerkt. Details: `docs/decisions.md` Nachtrag 222. | 2026-09-13 |
| D21 (AMWA IS-08 auf `omp-2110-gateway`, ARCHITECTURE.md §13.2) | erledigt | Direkter Nutzerauftrag "fang mit omp-2110-gateway an" — dritter IS-08-Kandidat aus der D17/D18-Liste. Vorab per `AskUserQuestion` geklärt: der Node war video-only (Nachtrag 46), IS-08 braucht einen Audio-Signalweg — also erst ST2110-30-Audio-Ingest/-Output nachgerüstet (wiederverwendet `omp-mediaio::st2110::{St2110AudioInput, St2110AudioOutput}`, bereits von `omp-aes67-gateway` genutzt), dann IS-08 obendrauf. Ingest: Video+Audio teilen sich EIN `gst::Pipeline`-Objekt (wie `omp-decklink`s Video+Audio-Branches). Output: bewusst ZWEI unabhängige Pipelines mit eigenem Connect/Disconnect (wie `omp-aes67-gateway::pipeline::run_source`) statt `omp-decklink`s "Video als Anker"-Modell — ein ST2110-Netzwerkausgang kann anders als eine SDI-Karte real "nur Ton" senden. `sdp.rs` bekam `parse_audio_sdp` 1:1 aus `omp-aes67-gateway::sdp` übernommen; IS-08-Helfer (`audiomixmatrix`-Aufbau) 1:1 aus `omp-aes67-gateway::pipeline`. BCP-008: ein gemeinsamer Monitor pro Richtung (wie D18) — Ingest verknüpft `content` per UND über beide Essenzen, Output rührt `activate()`/`deactivate()` weiterhin nur von Video her (Audio additiv, kein Fehlalarm bei nie verbundenem Audio). `cargo build --workspace --bins`/`test`/`clippy -D warnings` für `omp-2110-gateway` sauber. **Live Ende-zu-Ende gegen echten ST2110/RTP-Traffic verifiziert** (zwei echte Testinstanzen, echte Registry/NATS): `gst-launch-1.0` sendete echtes Audio+Video per RTP an eine Ingest-Instanz — `mxl-info` bestätigte wachsende Head-Indizes auf beiden neuen MXL-Flows, `monitor.overallStatus` kippte nachweislich erst nach BEIDEN Essenzen auf Healthy. IS-08 auf beiden Seiten per echtem `POST .../map/activations/` + `GET .../map/active/`-Roundtrip bestätigt (Kanaltausch angewendet und abfragbar). Output-Seite: echte IS-05-PATCH auf den neuen Audio-Receiver verband eine reale MXL-Quelle — ein `select()`-basierter Rohsocket-Probe zählte 4981 reale UDP/RTP-Pakete/5s (konsistent mit L24 `ptime:1ms`), bestätigt tatsächlich ausgehenden 2110-30-Traffic. Alle Testprozesse/-Streams danach beendet, verwaiste MXL-Test-Flows per `mxl-info -g` eingesammelt. Bewusst nicht Teil: zeitgesteuerte IS-08-Aktivierung (gleiche Grenze wie D17/D18), ein zweiter ressourcengenauer BCP-008-Monitor pro Essenz, SAP-Discovery für Audio (reines ST2110, keine AES67-Erwartung). Details: `docs/decisions.md` Nachtrag 223. | 2026-09-13 |
| BCP-008 Fleet-Dashboard: neuer "Health"-Tab | erledigt | Nutzerauftrag "ein Dashboard, das alle BCP008 Daten intuitiv und fancy darstellen kann" (per `AskUserQuestion` auf systemweiten Top-Level-Tab statt Erweiterung des Diagnose-Cockpits präzisiert; zweiter Teil desselben Auftrags — ein Messgerät-Node — folgt in einer eigenen Sitzung). Additiv zum bestehenden Statuspanel im Flow-Editor (Nachtrag 214, EIN Node, nur im offenen Kachel-Panel): neuer Top-Level-Tab `ui/shell/health-view.ts` (`<omp-health-view>`) zeigt ALLE laufenden BCP-008-fähigen Instanzen gleichzeitig, unabhängig vom Workflow-Filter. Geteilte Helfer (Statusfarben, Receiver-/Sender-Vokabular) aus `flow-canvas.ts` nach neuem `ui/graph/bcp008.ts` ausgelagert statt dupliziert. Live-Fund beim Verifizieren: die Launcher-Instanz-ID (`GET /api/v1/instances`) ist NICHT dieselbe ID wie die vom generischen Node-Proxy erwartete NMOS-Node-ID (`graph.Node.ID`) — per echtem `curl` bestätigt (`"unknown node"` mit der falschen ID); neue `#nodeIdByInstance`-Brücke aus `GET /api/v1/graph`s `instanceId`-Feld behebt das. Kein neuer Backend-Endpunkt — SSE-first für die Instanzstruktur (`alarm-view.ts`-Muster), 4s-Poll für die `monitor.*`-Werte selbst (kein SSE-Event dafür vorhanden). `deno check`/`deno test ui/` (92/92) grün. Live gegen den echten, per `make start` gestarteten Orchestrator + eine echte `omp-2110-gateway-ingest`-Instanz per hand-rolled CDP-über-WebSocket verifiziert (kein `chromium-cli`/Puppeteer verfügbar): Tab zeigte korrekt "1 Unhealthy" mit exakt den per direktem `curl` gegengeprüften Domain-Werten (Link/Sync/Connection/Stream), Reset-Button-Zielendpunkt separat per `curl` bestätigt (200 OK), Screenshot geprüft. Test-Instanz danach gestoppt. Bewusst nicht Teil: Deep-Link von einer Health-Karte zurück ins Flow-Editor-Panel (bräuchte zusätzlich das Setzen des passenden Workflow-Filters), ein neues SSE-Event für `monitor.*`-Live-Push. Details: `docs/decisions.md` Nachtrag 224. | 2026-09-14 |
| Messgerät-Node `omp-scope`: Waveform/Vektorskop + Audio-Pegel/EBU-R128-Lautheit + Metadaten | erledigt | Zweiter Teil desselben Nutzerauftrags wie oben. Neuer, passiver Tap-Node (kein Sender) mit zwei unabhängigen optionalen IS-05-Receivern. Video: kombiniertes Waveform+Vektorskop-Bild aus reiner, GStreamer-freier Pixel-Mathematik (`scope_image.rs`, 6 `cargo test`-Fälle), über dieselbe `build_mjpeg_branch`-JPEG-Infrastruktur wie `omp-viewer` gestreamt; bewusst kein rotiertes SMPTE-Graticule (reines Cb/Cr-Streudiagramm, ehrlich benannt). Audio: Peak/RMS über `level`+`/levels`-SSE (1:1 `omp-viewer::audio_meters`-Muster), echte EBU-R128-Lautheit über die neue reine-Rust-Abhängigkeit `ebur128` (kein GStreamer-`ebur128`-Element in dieser Umgebung installiert; Minimal-Dependency-Regel bewusst als "geprüfte Bibliothek statt selbstgeschriebenem K-Weighting-Filter für eine Broadcast-Messgröße" ausgelegt). Daten: gemessene Ist-Werte aus tatsächlich verhandelten Caps statt geraten. **Zwei echte, nur per Live-Test gefundene Bugs behoben:** (1) die Pipeline hing dauerhaft in PAUSED ohne jeden Fehler, weil zwei unabhängige Live-Quellen in einer Pipeline koexistieren (`MxlVideoInput`s `appsrc` + das node-eigene `scope_src`) und der Analyse-Appsink ohne `async=false` am Preroll-Protokoll teilnahm — Fix analog zu `omp-viewer::audio_meters`s Level-Appsink, das das schon richtig macht; (2) "Auflösung" zeigte immer die selbst erzwungene Analyse-Größe (320×180) statt der echten Quellauflösung — Fix: Breite/Höhe/Framerate jetzt sofort nach Connect aus der `caps`-Property des rohen `MxlVideoInput`-`appsrc` gelesen, nicht aus den negotiated Analyse-Caps. `cargo build --workspace --bins`/`clippy -p omp-scope -D warnings`/`test -p omp-scope` grün, `node --check` auf `ui/bundle.js` grün. Live gegen echten Orchestrator + echte `omp-source`-Testinstanz (640×480@25fps/48kHz-Stereo) verifiziert: IS-05-Connect für Video UND Audio, Panel-Mount (echter `mountUIBundle`-Pfad im Browser nachgebildet, da ein Canvas-Klick an SVG-Pan/Zoom-Koordinaten scheiterte) zeigte echte LUFS-Werte/Metadaten/Quell-Label, das per Orchestrator-Proxy abgerufene JPEG zeigte visuell ein plausibles, für ein SMPTE-artiges Testsignal korrektes Waveform/Vektorskop-Muster. Nebenfund `omp-mediaio::preview::Broadcaster` (fehlendes `impl Default`, `clippy::new_without_default`) trivial mit behoben. Bewusst nicht Teil: BCP-008-Monitor (reiner Analyse-Tap), rotiertes SMPTE-Graticule, Deep-Link vom Health-Dashboard (Nachtrag 224) zu diesem Node. Details: `docs/decisions.md` Nachtrag 225. | 2026-09-14 |
| `omp-scope` wird ein Timing-Messgerät: MXL-Transportlatenz, Lipsync, Flow-Deklaration, QC-Alarme, True Peak | erledigt | Nutzerauftrag „können wir das messgerät node auch nutzen, um MXL-spezifische messungen zu machen und audio/video timing latenz zu messen (lipsync, und oder source timestamp vergleichen? oder was anderes kreatives)". Ausgewertet wird die schon lange mitreisende, aber nie gelesene `timestamp/x-mxl-tai`-Meta (ARCHITECTURE.md §15 Punkt 4): Transportlatenz = `MxlContext::now_ns()` − Grain-TAI, Lipsync = Differenz der beiden Latenzen (die gemeinsame Uhr kürzt sich weg, die Flows müssen nicht gleichzeitig abgetastet werden), bewertet nach **EBU R 37** (Ton max. 40 ms vor / 60 ms nach Bild). Neu außerdem: Jitter als Spitze-zu-Spitze-Latenzschwankung im 2-s-Fenster, Ist- gegen Soll-Kadenz, Zähler für ausgelassene Grains und Reader-Neuaufsetzer; die Flow-**Deklaration** aus `mxlGetFlowDef` (Media-Type/Rate/Bittiefe/Farbraum/Interlace/Grouphint + rechnerische Grain-Größe und Shared-Memory-Datenrate) bewusst getrennt vom gemessenen Ist geführt; QC-Alarme Schwarzbild/Standbild/Stille als *gehaltene* Bedingungen (1/2/2 s — ohne Haltezeit wäre jeder Schnitt ein Falschalarm); True Peak (dBTP) nach ITU-R BS.1770 plus R-128-Konformitätsampel. **Zwei echte Plattform-Befunde, unabhängig mit `mxl-info` gegengeprüft:** (1) die MXL-Schreiber driften gegen die MXL-Uhr — Video stempelt konstant ≈4 Bilder in die Zukunft (Scope −170 ms, `mxl-info` −152 ms), Audio fällt wachsend zurück (Scope +408 ms, `mxl-info` +406 ms) — die erstmals gemessene Auswirkung der in `MxlVideoOutput` seit jeher dokumentierten Vereinfachung „Index einmal aus `get_current_index()`, danach frei hochzählen, keine Driftkorrektur"; (2) Folge davon ist ein großer, wachsender A/V-Versatz laut Zeitstempeln (−296 → −579 ms), heute unhörbar (niemand spielt nach TAI aus), aber latent. Nebenbefund: OMPs Flow-Defs setzen als NMOS-Grouphint-Gruppe die jeweilige Flow-ID, Video und Audio einer Quelle landen also nie in derselben Gruppe. Alle drei Fixes gehören in den Schreibpfad und damit in eigene Schritte, nicht in diese Messgerät-Erweiterung. Neu auch `GET /measurements` (Node + eine Orchestrator-Routenzeile, exakt das `timeline/window`-Muster): 64 Parameter einzeln zu pollen wären >50 HTTP-Anfragen pro Sekunde und Panel; die Antwort wird aus denselben `descriptor()`/`get()`-Aufrufen erzeugt, ist also kein zweiter Wahrheitsort. Eigener Fehler beim Bauen gefunden: abgeleitetes `Default` hätte die QC-Haltezeiten auf 0 gesetzt und die Entprellung ausgehebelt. `cargo build/clippy --all-targets -D warnings/test -p omp-scope` (29 Tests, 23 neu), `go build ./...`, `node --check` grün. Live gegen echten Orchestrator + echte `omp-source` verifiziert, diesmal mit **echtem Kachel-Klick** im Flow-Editor (Kachel-`<g>` per `textContent` gesucht statt SVG-Koordinaten zu treffen): QC live ausgelöst und wieder gelöscht (`black` → Schwarz+Standbild, `ball` → beide weg), Stille live über einen stummgeschalteten `omp-audio-mixer` ausgelöst (dabei gefunden und behoben: digitale Stille zeigte überall „–" statt −∞). Drei Screenshots in `docs/screenshots/`. Details: `docs/decisions.md` Nachtrag 226. | 2026-09-15 |
| MXL v1.1.0-beta-1 → v1.1.0 (GA) + DMF/MXL-Interop-Test gegen unabhängige C++-Referenzwerkzeuge | erledigt | Nutzeranlass: Rückfrage zur IBC-2026-DMF/MXL-Interop-Recherche ("aktuellste MXL-Version?", "können wir kommerzielle MXL-Microservices von Drittanbietern nutzen? teste das"). `gh release list` bestätigt: v1.1.0 (GA, 2026-09-09) ist zwei Releases neuer als der bisherige Beta-Pin — dieselbe Version, die auf der IBC 2026 in einer herstellerübergreifenden DMF/MXL-Interop-Demo bereits im Einsatz war. Upgrade durchgeführt, nicht nur beantwortet: `install-mxl.sh` auf `v1.1.0` gehoben, `deploy/dev/mxl-patches/0001-…diff` neu portiert (Rust-Seite `gst-mxl-rs/src/mxlsink/{imp.rs,state.rs}` strukturell verschoben) — dabei gefunden, dass einer der beiden ursprünglichen Nachtrag-116-Fixes (set_caps-Idempotenz) von Upstream selbst korrekt behoben wurde (ersatzlos aus dem Patch gestrichen), der zweite (DOMAIN_LOCK gegen die GC/create_flow_writer-Race) weiterhin fehlt und neu eingehängt wurde. C++-Teil (`FlowManager.cpp`) unverändert übernehmbar. Verifiziert: `cargo clean -p mxl-sys -p mxl` + `cargo build --workspace --bins` (frisches Bindgen, kein C-API-Bruch über alle ~25 Nodes), `cargo test`/`clippy -D warnings` auf den berührten Crates grün, Live-Regression gegen die echte laufende Dev-Domain (frische `omp-source`-Instanz schreibt, `mxl-info` bestätigt). **Interop-Test** (echte Herstellerprodukte nicht verfügbar, daher bester Stand-in): OMPs eigener Rust-Produktionslesepfad gegen die im `dmf-mxl/mxl`-Repo mitgelieferten, unabhängigen C++-Referenzwerkzeuge (`mxl-gst-testsrc`/`-sink`, `mxl-info`) auf einer isolierten Test-Domain — beide Richtungen erfolgreich (fremd geschrieben→OMP liest: 30 Bilder empfangen; OMP schreibt→fremd liest: nach Fund+Fix eines eigenen Testtreiber-Bugs — `MxlVideoOutput`s `valve` startet bewusst geschlossen, `Output::set_active(true)` vergessen — sowohl `mxl-info` als auch `mxl-gst-sink`, Pixel-Ebene, unter `xvfb-run` erfolgreich). Bestätigt: OMP steht auf demselben offenen MXL-Fundament wie die auf der IBC 2026 gezeigte herstellerübergreifende DMF/MXL-Interop-Demo — ein Drittanbieter-Microservice mit kompatibler MXL-Version könnte ohne Gateway direkt einen Flow mit einem OMP-Node austauschen. Nicht geprüft: echte Hersteller-Binaries, deren proprietäre Steuerschichten. Details: `docs/decisions.md` Nachtrag 227. | 2026-09-15 |
| MXL-Transport auf AMWA BCP-007-03 v1.0.0 umgestellt (Punkt 1 der Qvest-Gap-Analyse) | erledigt | Nutzerauftrag "Punkt 1 (BCP 007 03) sowie Punkt 2 ... gleich umsetzen" nach der DMF/MXL-Standards-Recherche im selben Gespräch (identifizierte BCP-007-03 v1.0.0, 2026-08-21, als den in `is04.rs` seit Projektbeginn vorgemerkten Migrationspunkt). Spec exakt via `specs.amwa.tv/bcp-007-03` + den beiden JSON-Schema-Dateien nachgeschlagen (Regel §0.6): Transport-URN `urn:x-nmos:transport:mxl` (vorher projekteigen `urn:x-omp:transport:mxl`), Transport-Params `mxl_domain_id`/`mxl_flow_id` (`additionalProperties:false`), `"auto"` bei `mxl_flow_id` nur senderseitig erlaubt. Konkreter, verifizierter Befund vor dem Fix: JEDER OMP-MXL-Sender/-Receiver meldete unter dem MXL-Transport bisher RTP-geformte `transport_params` — für einen fremden BCP-007-03-Controller (z. B. Qvest-Showcase-Klasse) technisch unsichtbar/unbrauchbar. `nodes/omp-node-sdk/src/connection.rs`: `SenderResource::transport_params` von `Vec<TransportParams>` (fest RTP) auf `Vec<Value>` umgestellt (wie schon immer bei `ReceiverResource`), neue `default_leg()` liefert transport-abhängige Defaults (MXL-Sender: `{"mxl_domain_id":"auto","mxl_flow_id":"auto"}` — spec-konform korrekt für den Ein-Flow-pro-Sender-Fall dieses Projekts, kein Platzhalter), `with_transport()` setzt das Default-Leg jetzt beim Transport-Wechsel neu (der eigentliche strukturelle Bug: vorher blieb der RTP-Leg unabhängig vom gewählten Transport immer Erstzustand). Receiver-`patch_staged` merged `transport_params` jetzt erstmals überhaupt (vorher komplett ignoriert), inkl. normativer 400-Ablehnung von `mxl_flow_id:"auto"` auf Receiver-Seite. Konstraints-Endpoint liefert MXL-Feldnamen statt RTP-Legs. 8 neue + 2 aktualisierte Unit-Tests. **Bewusst nicht Teil:** `ReceiverControl::apply()`-Implementierungen (~10 Nodes) lösen Flows weiterhin nur über `sender_id`→eigene Registry auf, nicht über die jetzt korrekt transportierten `mxl_flow_id`/`mxl_domain_id`-Werte selbst — für einen fremden, nur transport-params-basierten Controller noch ohne Wirkung (ehrlich benannte Lücke, eigener künftiger Schritt); ebenso ein `SenderConnection`, die den echten Flow-UUID statt `"auto"` kennt (bräuchte Durchreichen der IS-04-`flow_id` in ~20 Aufrufstellen, für diesen Schritt nicht nötig, da `"auto"` spec-konform ist). `cargo build --workspace --bins`/`test -p omp-node-sdk` (74/74)/`clippy -p omp-node-sdk -p playout -D warnings` grün; `go build/vet/test` (`orchestrator`, `nodes/mock`) grün bis auf zwei als Sandbox-Flakes identifizierte, von dieser Änderung unberührte Tests (einer davon isoliert 3/3 nachgestellt); `deno check`/`deno test ui/` (92/92) grün. Trivial mitgezogen (Konstanten-/Label-Parität, kein Verhalten): `nodes/mock/internal/connection/transports.go`, `ui/graph/flow-canvas.ts`, `orchestrator/internal/registry/types.go`. Details: `docs/decisions.md` Nachtrag 229. | 2026-09-17 |
| BCP-007-03-Schema-Konformitätscheck in `contract-check` (Punkt 2 der Qvest-Gap-Analyse) | erledigt | Direkte Fortsetzung von Nachtrag 229. AMWA-`nmos-testing`-Tool hat keine BCP-007-03-Suite (verifiziert, sehr neue Spec) — per `AskUserQuestion` Nutzerentscheidung für "eigener Schema-Konformitätstest" statt Abwarten/Beides. Echte Schemas (`sender_transport_params_mxl.json`/`receiver_transport_params_mxl.json`) byte-genau von specs.amwa.tv geladen (`docs/bcp-007-03/`). Neuer `CheckBcp00703Transports` im bestehenden `tools/contract-check` (wiederverwendet dessen schon vorhandene `jsonschema/v6`-Abhängigkeit statt neuem Tool/neuer Sprache): validiert `staged.transport_params[0]` gegen das echte Schema für jeden Sender/Receiver mit `transporttype=urn:x-nmos:transport:mxl`. **Live-Fund:** im gesamten Repo implementiert einzig `nodes/playout` (RTP) `SenderConnection` — kein MXL-Sender exponiert überhaupt einen IS-05-Connection-Endpoint (architektonisch: Verbindungen laufen seit dem B1-Scope-Schnitt ausschließlich über den Receiver, ein Zero-Copy-MXL-Sender hat kein PATCH-bares "Ziel") — eine echte, separat zu entscheidende Konformitätslücke gegenüber einem Sender-seitig patchenden Controller, bewusst nicht in dieser Runde behoben. PASS live gegen echte, per IS-05-PATCH verbundene `omp-source`→`omp-viewer`-Instanzen bestätigt (`staged` zeigte live `{"mxl_domain_id":"auto","mxl_flow_id":null}`); Gegenprobe zweier neuer Unit-Tests bestätigt echtes FAIL bei RTP-geformten Feldern unter MXL-Transport (der historische Vor-Nachtrag-229-Bug). `go build/vet/test` für `tools/contract-check` grün (7/7, 2 neu). Bewusst nicht Teil: CI-Verdrahtung (bräuchte einen echten MXL-fähigen Node-Job, andere Form als der bestehende Go-Mock-Job; `contract-check` läuft bisher generell nur manuell). Details: `docs/decisions.md` Nachtrag 230. | 2026-09-17 |
...
| Kapitel 21 Phase 2 (Domain Model + Persistenz + Migrationen: Process-Engine + Asset/Content) | erledigt | Nutzerauftrag "fang mit Phase 2 an", direkt im Anschluss an Phase 1 samt bestätigter Entscheidungen 1+2. Neues, eigenständiges Go-Paket `orchestrator/internal/process` (bewusst NICHT `internal/workflows` erweitert, s. §6b/21.2): `ProcessDefinition`/`ProcessVersion` (A1/A7, alle 17 Step-Typen aus der Aufgabenstellung, `Definition.Validate()` prüft Graph-Referenzen — eindeutige Step-IDs, existierender Start, alle Next/Branches/CompensationStepID-Ziele existieren), `ProcessExecution`/`ProcessStepExecution` (A2, exakt die geforderten 10 Status-Werte PENDING…COMPENSATED, CorrelationID/CausationID/ParentExecutionID/TraceID, optimistisches Locking per `row_version`+CAS-UPDATE für A3), `HumanTask` (A6, alle geforderten Felder + Aktionen assign/claim/release/approve/reject/request-changes/delegate/cancel/escalate/timeout als validierte Zustandsübergänge). Neues, geteiltes Hilfspaket `internal/statemachine` (explizite (from,to)-Erlaubt-Listen statt verstreuter if/else, B8 wörtlich) — echte Zweitnutzung durch Process UND Asset rechtfertigt die Abstraktion. Neues Paket `orchestrator/internal/asset`: `Asset`/`AssetVersion`/`Representation`/`Metadata` (B2-B6, Asset-Typ und Representation-Typ bewusst freies TEXT statt harter Enum, B2-Vorgabe; Storage-Abstraktion B5 minimal als `StorageLocation{Provider,URI}` — ein echtes I/O-Interface erst ab Phase 4, kein Aufrufer dafür heute), Lifecycle-State-Machine mit den zehn geforderten B8-Status-Werten, `PublishVersion` setzt atomar (eine Transaktion) sowohl den Versions-Status als auch `assets.current_version_id` — veröffentlichte Versionen sind unveränderlich (zweites Publish schlägt fehl). Bewusst NICHT Teil dieser Runde (Phase 2s eigene "Implementiere zuerst"-Liste grenzt das explizit ab, kein Vorgriff): B1s volle Core-Content-Model-Liste (ContentObject/Collection/Sequence/Segment/Marker/Publication/Package), B7 (MetadataSchema/-Field/-Value-Validierung), jegliche Ausführungslogik/Runtime (Retry/Conditions/Recovery = Phase 3), keine HTTP-API/UI-Anbindung (Phase 5) — reines Domain Model + Persistenz wie von der Aufgabenstellung für Phase 2 verlangt. Migrationen `0018_process.sql`/`0019_assets.sql` (Namenskonvention/`schema_migrations`-Tracking übernommen); `assets.current_version_id ↔ asset_versions.id`-Zirkelbezug per nachträglichem `ALTER TABLE … ADD CONSTRAINT` aufgelöst. **Zwei echte Bugs per DB-Test gefunden+gefixt, nicht nur behauptet:** (1) `UpdateExecutionStatus`/`UpdateStepExecutionStatus` nutzten `statemachine.IsTerminal(newStatus)`, um `completed_at` zu setzen — für `failed` (hat in beiden Transitions-Tabellen einen gültigen Folgeübergang nach `compensating` bzw. `pending`/Retry) lieferte das fälschlich `false`, `completed_at` blieb leer, obwohl der aktuelle Lauf tatsächlich beendet war; behoben durch eine explizit vom Graph entkoppelte `isRunEndStatus`-Liste (dokumentiert, warum das etwas anderes ist als "graphisch terminal"). (2) Ein Test verglich JSONB-Roundtrip-Output per exaktem String-Vergleich — Postgres normalisiert Whitespace (`{"result":"ok"}` → `{"result": "ok"}`), kein Store-Fehler, Test auf semantischen Vergleich umgestellt. Beide Funde nur möglich, weil gegen die echte, laufende Dev-Postgres getestet wurde (isolierte `_test`-DB via `dbtest.Open`, Nachtrag 108), nicht nur `go vet`/Kompilierbarkeit. Verifikation: `go build ./...`/`go vet ./...`/`gofmt -l` sauber (drei vorbestehende, unberührte Formatierungsabweichungen in `httpapi` ignoriert — nicht Teil dieser Änderung), `go test ./...` für das gesamte Orchestrator-Modul grün (34 Pakete inkl. der beiden neuen), 8 neue statemachine-Tests + 9 neue process-Store-Tests + 10 neue process-Validate-Tests + 8 neue asset-Store-Tests, alle real gegen Postgres (kein Mock). Kein API-/UI-Code, keine main.go-Verdrahtung (bewusst — Migrationen laufen automatisch über den bestehenden `embed.FS`-Scan, unabhängig davon ob das Go-Paket importiert wird; API-Anbindung ist Phase 5). | 2026-09-22 |
| Kapitel 21 Phase 3 Teil 1 (Runtime: Execution Engine — state machine/retry/timeout/recovery/idempotency, strukturelle Step-Typen) | erledigt | Nutzerauftrag "fahre fort", direkt im Anschluss an Phase 2. Neue Datei `orchestrator/internal/process/engine.go` + `executors.go`: `Engine` treibt `ProcessExecution`s vollständig DB-zustandsgetrieben durch ihren Schritt-Graphen (kein In-Memory-Zustand außerhalb eines laufenden Zyklus — `Start`/`RecoverAll` rufen denselben `run()`-Code auf, Wiederaufnahme nach Neustart ist kein Sonderfall, s. u.). Frontier-Algorithmus (`computeFrontier`) generalisiert AND-Join (mehrere Next-Vorgänger, z. B. `Parallel`/`Join`) UND XOR-Branches (HumanTask-/Approval-Entscheidung wählt genau ein `Branches`-Ziel über ein `decision`-Feld im Output, `resolveSuccessors`) über dieselbe "alle Vorgänger terminal UND mindestens einer hat mich aktiviert"-Regel — kein Sonderfall-Code pro Join-Art nötig. Eingebaute, produktionsreife Executors NUR für rein strukturelle, keine externe Integration brauchende Typen: Wait/Timer (schläft bis `StepExecution.StartedAt + config.seconds`, nicht blind `now()+N` — übersteht daher einen Neustart mitten in der Wartezeit korrekt, statt die Wartezeit zu verlängern), Parallel/Join (Passthrough, die eigentliche Fan-out/Fan-in-Logik steckt bereits im Frontier-Algorithmus), HumanTask/Approval (legt real einen `HumanTask` an, `CompleteHumanTask`-API löst die Wartestellung auf — voll nutzbar, da Phase 2 das Domänenmodell dafür schon lieferte), Subworkflow (startet echte Kind-`ProcessExecution` mit `ParentExecutionID`, wartet auf deren Abschluss, reicht ihr Output durch — "nested workflows" aus der Aufgabenstellung), Notification (publiziert über den bestehenden, bewusst weiterhin fire-and-forget `eventbus`, A8 "workflow -> event"). **Bewusst UNREGISTRIERT** (ehrliches `ErrNoExecutorRegistered` statt Mock/Platzhalter, Projektgrundsatz): Task/MediaFunction/ServiceCall/Script (echte externe Integration, Phase 4), Condition/Branch/Loop (brauchen eine sichere Expression Language, A5 — eigene Sitzung, Teil 2), EventTrigger als Schritt-Typ (A8s JetStream-vs-Outbox-Zuverlässigkeitsfrage aus Kapitel-21-Phase-1 weiterhin offen, kein stilles Fire-and-forget an dieser sicherheitsrelevanten Stelle). Retry (A4): synchrone Backoff-Schleife INNERHALB eines einzelnen `runStep`-Aufrufs (neue Store-Methode `RecordAttemptAndContinue`, hält den Status bewusst durchgehend "running" statt zwischenzeitlich "failed" zu persistieren) — ein Crash mitten in der Backoff-Pause zeigt beim Wiederanlauf deshalb korrekt "noch in Bearbeitung", nicht fälschlich "endgültig gescheitert" (B16-Korrektheit). Timeout (A4): `context.WithTimeout` je Schrittaufruf, bei Erschöpfung `TIMED_OUT` statt `FAILED` (Phase-2-Lücke beim Implementieren gefunden+geschlossen: `StepExecutionTransitions` erlaubte `timed_out -> pending`-Retry ursprünglich nicht). Kompensation: bei endgültigem Fehlschlag mit `CompensationStepID` wird über dieselbe `runStep`-Maschinerie der Kompensationsschritt angesteuert (`FAILED -> COMPENSATING -> COMPENSATED/FAILED`); Erreichbarkeitsprüfung in `Definition.Validate()` erweitert (jeder Nicht-Start-Schritt braucht einen echten Vorgänger, Kompensationsschritte sind bewusst ausgenommen — sie werden nur über den Fehlerpfad erreicht, nie über den regulären Graphen, echte beim Runtime-Entwurf gefundene Phase-2-Lücke). `RecoverAll()` treibt jede nicht-terminale Execution (pending/running/waiting/paused/compensating) beim Orchestrator-Start erneut an — B16s Kernszenario ("Prozess killen, neu starten, Workflow läuft korrekt weiter") live über einen ECHTEN simulierten Neustart (zweite, komplett unabhängige `Engine`-Instanz auf demselben Store) verifiziert, nicht nur behauptet. **Mehrere echte Bugs per Test gefunden+gefixt:** (a) `StepExecutionTransitions` erlaubte `waiting -> completed` nicht (HumanTask-/Subworkflow-Abschlusspfad) — ergänzt. (b) Kind-Executions eines Subworkflow-Schritts wurden angelegt, aber nie "angetrieben" (`drive()` fehlte) — blieben für immer "pending" liegen; behoben durch einen `drive`-Callback in den Subworkflow-Executor. (c) Ausführungs-Output wurde nie gesetzt (`ProcessExecution.Output` blieb leer) — `computeExecutionOutput` sammelt die Outputs aller "Sink"-Schritte (keine Next/Branches), neue Store-Reihenfolge bewusst Output-ZUERST-dann-Status (ein Leser, der "completed" sieht, darf NIE ein noch leeres Output sehen — echter TOCTOU-Fund per Subworkflow-Test). (d) `driveCompensation` behandelte `ErrConcurrentModification` an drei Stellen stillschweigend als Erfolg (publizierte ein Ereignis, OHNE zu verifizieren, dass der eigene Schreibversuch tatsächlich griff) — auf reines Zurückkehren ohne Ereignis umgestellt (nächster Poll-Zyklus entscheidet auf Basis des echten DB-Stands neu). (e) Der `FAILED -> COMPENSATING`-Übergang in `finalize()` bekam einen kurzen Retry-Loop (bis zu 10 Versuche, kleine Pause dazwischen) statt eines einzelnen CAS-Versuchs — eine einzelne fehlschlagende Transition darf eine Execution nicht dauerhaft bei "failed" hängen lassen, obwohl eine Kompensation vorgesehen ist. **Wichtiger Infrastruktur-Befund (kein Code-Bug):** Ausführliches Stresstesten (100+ Wiederholungen) zeigte eine seltene (~2-10 % unter Last), aber reale Resttest-Flakiness spezifisch bei Verwendung der Mehr-Host-Patroni-DSN (`DEV_POSTGRES_URL`, `target_session_attrs=read-write` über 3 Knoten) — mit einer Einzelhost-DSN liefen 60/60 Wiederholungen fehlerfrei bei identischem Code. Nicht weiter verfolgt (außerhalb des Scopes dieser Sitzung, betrifft die projektweite Patroni-Verbindungsschicht, nicht Kapitel-21-Logik) — als offener Punkt dokumentiert, s. `docs/decisions.md`. Verifikation: `go build ./...`/`go vet ./...`/`gofmt -l` sauber, `go test ./...` für das gesamte Orchestrator-Modul grün (bei erneutem Lauf reproduzierbar, s. o. zur bekannten DSN-Flakiness), 13 neue Engine-Tests (linearer Ablauf, Paralleler Fan-out/Join, HumanTask-Entscheidungsrouting, Retry-Erfolg, Retry-Erschöpfung, Timeout, Kompensation-Erfolg, Kompensation-Fehlschlag, Subworkflow, Cancel-Unterbrechung, Neustart-Wiederaufnahme, Notification/Events, kein-Executor-Ehrlichkeit) plus 2 neue Validate-Tests (Erreichbarkeit), alle real gegen Postgres. Kein API-/UI-Code (Phase 5). | 2026-09-22 |
| Kapitel 21 Phase 3 Teil 2 (A5: Expression Language für Condition/Branch) | erledigt | Nutzerauftrag "fahre fort", direkt im Anschluss an Phase 3 Teil 1. Neue Datei `orchestrator/internal/process/expr.go` — erste Wahl `cel-go` verworfen (zieht trotz reiner Dyn-Type-Nutzung Protobuf/ANTLR/YAML-Abhängigkeiten mit, passt nicht zur Minimal-Dependency-Linie, ARCHITECTURE.md §4.1a), stattdessen `expr-lang/expr` gewählt: keine einzige externe Abhängigkeit (`go.sum`-Diff zwei Zeilen), dieselbe Sicherheitseigenschaft (kein Host-Zugriff ohne explizit registrierte Funktionen — per `TestEvaluatorHasNoHostAccess` nachgewiesen, nicht nur behauptet). Drei feste Top-Level-Variablen (`input`/`outputs`/`workflow`) statt beliebiger zur Laufzeit erratener Bezeichner. `Condition`-/`Branch`-Executors (`executors.go`) nutzen dieselbe generische Branches/`decision`-Routing-Mechanik, die Teil 1 bereits für HumanTask/Approval gebaut hatte — kein neuer Sonderfall in der Engine nötig. Condition = ein Ausdruck, TrueLabel/FalseLabel (Default "true"/"false"); Branch = mehrere Fälle der Reihe nach geprüft, erster Treffer gewinnt, optionales DefaultLabel, sonst ehrlicher Fehler. Loop bewusst weiterhin nicht umgesetzt (Iterationszustand braucht eigene Design-Sitzung, kein Notbehelf). **Zwei echte, per Stresstest gefundene Testfehler (keine Engine-Bugs) behoben:** `awaitExecutionStatus` in einem Test akzeptierte einen bewusst kurzlebigen Zwischenzustand ("failed", bevor `finalize()` unmittelbar danach auf "compensating" weiterschreibt) als vorzeitigen Erfolgs-Abbruch — `TestEngineCompensationRunsAfterFailure` wartet jetzt nur noch auf `StatusCompensated`; ein zweiter Test prüfte ein publiziertes Event einmalig statt zu pollen, obwohl das Publizieren erst nach dem beobachteten DB-Schreiben passiert — neuer `awaitPublished`-Poll-Helfer. Nebenbefund (real, aber letztlich nicht die Hauptursache): `omp-patroni-3` (dritter Knoten der Dev-Postgres-DSN) lief seit 21. August nicht mehr (leere `pg_hba.conf`) — auf Nutzerbestätigung per `patronictl reinit` repariert, Cluster wieder vollständig gesund. Verifikation: `go build ./...`/`go vet ./...`/`gofmt -l` sauber, 5 neue Condition/Branch-Engine-Tests + 6 neue Evaluator-Tests, alle grün; nach beiden Testkorrekturen zwei unabhängige Stressläufe des `internal/process`-Pakets (u. a. 40/40) ohne Fehlschlag. Details: `docs/decisions.md` Nachtrag 260 (inkl. Korrektur zu Nachtrag 259). | 2026-09-22 |
| Kapitel 21 Phase 4 Teil 1 (A8/B9: Event-Zuverlässigkeit — Postgres-Outbox + NATS JetStream) | erledigt | Nutzerauftrag "fahre fort", direkt im Anschluss an Phase 3 Teil 2. Vor Phase-4-Start musste die seit Phase 1 offene Zuverlässigkeitsfrage geklärt werden — vom Nutzer nach kurzer Gegenüberstellung als Kombination entschieden: Postgres-Outbox (Schreibseite, Atomarität) + NATS JetStream (Zustellseite, Durabilität/Fan-out; läuft bereits seit D14 mit `-js`, bisher ungenutzt). Neues Paket `orchestrator/internal/outbox` (Migration `0021_outbox_events.sql`): `Store.Enqueue(tx,...)` nimmt bewusst ein `*sql.Tx` (Atomaritäts-Zwang, per Test bewiesen: ein Rollback macht das Event nie sichtbar), `Relay` pollt unversendete Zeilen und veröffentlicht zu JetStream (dasselbe DB-zustandsgetriebene Entwurfsmuster wie die Process-Engine), `EnsureStream` legt Streams idempotent an — keine neue Abhängigkeit (`jetstream` ist Teil des bereits vorhandenen `nats.go`-Moduls, `go mod tidy` bestätigt: kein Diff). Asset-Domäne angebunden (B9): `asset.Store` bekommt eine optionale, nil-sichere `WithOutbox(...)`-Option, `CreateAsset`/`UpdateAssetStatus`/`UpdateAssetMetadata`/`CreateVersion`/`PublishVersion` veröffentlichen ihr Domain-Event jetzt atomar mit dem State-Change (`omp.asset.<id>.<event>`), per Test bewiesen dass ein fehlschlagender State-Change nie ein Event erzeugt. Konsumentenseite `process.TriggerListener` (A8 "event -> workflow trigger", die bislang fehlende Richtung): hält für jede veröffentlichte ProcessVersion mit `Definition.Triggers` einen durablen JetStream-Consumer aufrecht (periodischer Sync, neue/deprecatete Trigger ohne Neustart erkannt), startet bei passender Nachricht eine neue Execution, idempotent gegen Mehrfachzustellung über eine aus (Version, Stream-Sequenznummer) abgeleitete CorrelationID (dasselbe Muster wie der Subworkflow-Executor aus Phase 3 Teil 1). Bewusst weiterhin nicht umgesetzt: `StepTypeEventTrigger` als Schritt innerhalb eines laufenden Graphen (andere, engere Fähigkeit). **Ein echter Deadlock per Test gefunden+behoben:** `TriggerListener.Stop()` brach ursprünglich nur den Sync-Schleifen-Kontext ab, nicht die einzelnen, separat erzeugten Consumer-Kontexte — ein Test hing bis zum Timeout. **Ein nicht reproduzierbarer Einzel-Hang beobachtet:** ein `go test ./...`-Lauf ohne `-timeout`-Flag hing 600s (Gos Default) in NATS-internen Goroutinen; zwei unabhängige Wiederholungen (mit explizitem Timeout) liefen beide sauber durch — als offene, nicht bestätigte Beobachtung dokumentiert statt als "behoben" behauptet. Verifikation: `go build ./...`/`go vet ./...`/`gofmt -l` sauber, `go mod tidy` ohne Diff, 6 neue outbox-Tests + 3 neue asset-Outbox-Tests + 2 neue TriggerListener-Tests (echter Ende-zu-Ende-Nachweis gegen laufendes JetStream), alle real, kein Mock; zwei unabhängige volle `go test ./...`-Läufe grün. Details: `docs/decisions.md` Nachtrag 261. | 2026-09-22 |
| Kapitel 21 Phase 4 Teil 2 (A1: echte externe Integrations-Schritte — ServiceCall/MediaFunction/Script) | erledigt | Nutzerauftrag "fahre fort", direkt im Anschluss an Phase 4 Teil 1. Drei neue, produktionsreife Executors in `executors.go`, keiner automatisch in `NewEngine` registriert (brauchen echte, erst zur Laufzeit bekannte Infrastruktur — HTTP-Client/Registry/Allow-Liste — `Register()` ist bewusst der dafür vorgesehene Erweiterungspunkt, s. Phase 3 Teil 1). **ServiceCall**: generischer HTTP-Aufruf (Method/URL/Headers/Body/Timeout), Output `{status, body}`, Antwortkörper auf 1 MiB begrenzt, nicht-2xx-Status lässt den Schritt ehrlich fehlschlagen (bleibt über A4-Retry wiederholbar). **MediaFunction**: neuer `nodeclient.go` (eigenständig implementiert, NICHT aus `internal/workflows` importiert — bewusste Domänentrennung, s. §6b/21.2 — spiegelt aber exakt denselben, aus `nodes/omp-node-sdk/src/server.rs`s `route()` recherchierten Node-Contract-Pfad `POST /methods/<name>`): `NodeResolver`-Interface löst eine STABILE Launcher-Instanz-ID (nicht die flüchtige NMOS-Node-ID) zur aktuellen Basis-URL auf, dafür `registry.Store` um `GetByInstanceID(instanceID)` ergänzt (additiv, 2 neue Tests). Instanz offline/unbekannt → ehrlicher Fehlschlag statt stillem No-op. **Script**: allow-listete Kommandoausführung (`newScriptExecutor(allowedCommands map[string]string, eval)`) — Command MUSS ein Schlüssel der übergebenen Allow-Liste sein (Name → echter Programmpfad), dasselbe Sicherheitsmuster wie `internal/launcher`s "Katalog statt beliebiger Kommandos"; Argumente unterstützen `${expr-lang-Ausdruck}`-Templating gegen denselben `input`/`outputs`/`workflow`-Kontext wie Condition/Branch (dafür `Evaluator` um eine generische, typlose `Eval()`-Methode ergänzt, `EvalBool` ruft sie jetzt intern auf statt zu duplizieren); Stdout/Stderr je auf 1 MiB gekappt (Schutz vor außer Kontrolle geratenen Prozessen), Timeout via `context.WithTimeout`, nicht-Null-Exitcode lässt den Schritt fehlschlagen (Stderr in der Fehlermeldung, gekürzt auf 500 Zeichen). `StepTypeTask` bewusst weiterhin unregistriert/undefiniert — Semantik laut Aufgabenstellung genuin unklar, keine erfundene Annahme (Projektgrundsatz "kein Raten"). Verifikation: `go build ./...`/`go vet ./...`/`gofmt -l` sauber, 9 neue Tests (2 ServiceCall-Ende-zu-Ende gegen echten `httptest.Server` inkl. Nicht-2xx-Fehlerfall, 1 ServiceCall-Unit-Validierung, 2 MediaFunction-Ende-zu-Ende inkl. unbekannte-Instanz-Fehlerfall, 4 Script-Ende-zu-Ende inkl. Allow-Liste-Ablehnung/Exitcode-Fehlschlag/Timeout — alle real, kein Mock: echte laufende HTTP-Server, echte `/bin/echo`/`/bin/false`/`/bin/sleep`-Prozesse), volle `internal/process`-Suite grün, volle Modul-Suite (`go test ./...`, alle 34 Pakete) grün mit explizitem Timeout (kein Wiederauftreten des in Phase 4 Teil 1 beobachteten Einzel-Hangs). Kein API-/UI-Code (Phase 5). Details: `docs/decisions.md` Nachtrag 262. | 2026-09-22 |
| Kapitel 21 Phase 5 Teil 1 (Runtime-Verdrahtung: Process Engine + Outbox-Relay + TriggerListener in main.go) | erledigt | Nutzerauftrag "proceed", direkt im Anschluss an Phase 4 Teil 2. Bis hierhin lief keine Zeile der Phase-2-4-Pakete (`internal/process`/`internal/asset`/`internal/outbox`) außerhalb von Tests — dieser Teil macht die Engine im echten Orchestrator-Prozess lauffähig. Neuer gemeinsamer JetStream-Stream `OMP_EVENTS` (Subjects `omp.asset.>`/`omp.process.>`, 30 Tage Retention, 3 Replicas — gleiches Muster wie `logbus.StreamName`/OMP_LOGS, bewusst NICHT über "omp.>" um Overlap mit OMP_LOGS/Host-Metrics zu vermeiden), Setup läuft ungegatet auf jeder Cluster-Instanz (`CreateOrUpdateStream` ist idempotent). `process.Engine` wird mit `WithEventPublisher(nc)` (fire-and-forget, wie der Rest von `internal/eventbus`) konstruiert und bekommt sofort `ServiceCall`/`MediaFunction` registriert (brauchen nur HTTP-Client/Registry, keine Sicherheitsentscheidung) sowie `Script` mit einer per `exec.LookPath` ERMITTELTEN (nicht geratenen) Allow-Liste aus `ffmpeg`/`ffprobe`, falls auf dem Host installiert (Aufgaben-Zusatzwunsch). `RecoverAll()`/`Shutdown()`, `outbox.Relay.Run`, `process.TriggerListener.Start/Stop` sind alle über das bestehende `runWhileLeader`-Muster (D12 Teil 3) an Raft-Führerschaft gegated — dieselbe Mehrfacharbeits-/Doppelstart-Vermeidung wie `placementEngine.Run`/`workflowScheduler.Run`. `internal/asset` (asset.NewStore) bewusst NOCH NICHT konstruiert — ohne eine HTTP-API, die seine Methoden aufruft, hätte ein hier angelegter Store keinen Aufrufer (Go: "declared and not used"), tote Verdrahtung wäre schlechter als ehrliches Aufschieben auf Phase 5 Teil 2. Dafür wurden die drei Phase-4-Teil-2-Executor-Konstruktoren exportiert (`NewServiceCallExecutor`/`NewScriptExecutor` umbenannt, `NewMediaFunctionExecutor` neu als schlanker Wrapper um den testbaren internen Konstruktor) — main.go kannte sie vorher nicht, da sie absichtlich paketintern waren. **Live gegen den echten laufenden Orchestrator verifiziert** (Nutzer hat einem Neustart des produktiven Prozesses zugestimmt): `make stop`/`make start`, sauberer Bootlog ohne neue Fehler-/Warn-Zeilen aus `process:`/`outbox:`, `GET /healthz` liefert `ok`, `OMP_EVENTS`-Stream über die NATS-Monitoring-API bestätigt vorhanden. **Nebenbefund (kein Regressions-Bug, aber real und jetzt erstmals reproduzierbar statt nur einmalig beobachtet, s. Nachtrag 261):** `go test ./internal/process/...` hing bei einem von mehreren Läufen exakt in `TriggerListener`-Testteardown (`consumeCtx.Stop()`/nats.go-Pull-Consumer-Goroutinen blieben nach einem stressigen Vorlauf hängen) — in Isolation lief es danach mehrfach sauber (0,3-0,6s). Kein Produktionsrisiko für main.go: `runWhileLeader` wartet nicht auf die Rückkehr der gestarteten Funktion, ein hängendes `Stop()` blockiert daher nicht den Prozess-Exit, anders als in Tests, wo `t.Cleanup` synchron wartet. Root Cause nicht weiterverfolgt (dritte Bibliotheks-interne nats.go-Ebene, nicht unser Code — passt zum bereits dokumentierten, jetzt bestätigten Verdacht aus Nachtrag 261), 19 verwaiste `TEST_*`-JetStream-Streams aus vorherigen abgebrochenen Testläufen aufgeräumt. Verifikation: `go build ./...`/`go vet ./...`/`gofmt -l` sauber (3 vorbestehende, unberührte httpapi-Abweichungen ignoriert), volle Modul-Suite (`go test ./...`, 34 Pakete) grün. Kein HTTP-API-/UI-Code (Phase 5 Teil 2, eigene Sitzung). Details: `docs/decisions.md` Nachtrag 263. | 2026-09-22 |
| Kapitel 21 Phase 5 Teil 2 (HTTP-API für die Process Engine: Definitions/Versions/Executions/Steps/HumanTasks) | erledigt | Nutzerauftrag "fahre fort", direkt im Anschluss an Phase 5 Teil 1. 22 neue Endpunkte unter eigenem, disjunktem Namensraum `/api/v1/process-definitions`, `/api/v1/process-versions`, `/api/v1/process-executions`, `/api/v1/human-tasks` (exakt wie in Phase 1 als Entscheidung dokumentiert, UMSETZUNG.md §6b), neue Datei `orchestrator/internal/httpapi/process_handlers.go` — Muster 1:1 an `workflow_handlers.go` gespiegelt (dünne Handler, `writeJSON`/`writeProcessError`, `authGate.requireAuth`/`requireVerbGlobal`). Zwei neue Interfaces in `server.go` (`ProcessStoreService`/`ProcessEngineService`, getrennt weil `*process.Store` und `*process.Engine` zwei unterschiedliche konkrete Typen sind — ein einzelnes Interface könnte beide nicht gleichzeitig fassen). Verb-Einstufung: Definition/Version anlegen = `configure` (wie Graph-Kanten/Layouts/Snapshots), Version veröffentlichen UND jede Execution-Aktion (Start/Cancel/Pause/Resume) = `admin` (ab Veröffentlichung ist eine Version tatsächlich startbar, ein Start kann über ServiceCall/MediaFunction/Script echte externe Wirkung entfalten, B14, s. Nachtrag 262/263 — dieselbe Schwelle wie Workflow-Start/-Stop), HumanTask-Zuweisen/-Entscheiden = `operate` (Bedienhandlung an einem laufenden Prozess). **Zwei neue Fehler-Sentinels in `internal/process`, damit die API 400/409 statt pauschal 500 melden kann** (Lücke aus Phase 2/3, wo `Definition.Validate()` und mehrere Pflichtfeld-Prüfungen nur unstrukturierte `fmt.Errorf`-Strings lieferten): `ErrValidation` (alle Validate()-Fehler + Pflichtfeld-Prüfungen in `CreateDefinition`/`UpdateDefinitionMeta`/`CreateExecution`/`CreateStepExecution`(×2)/`CreateHumanTask` jetzt per `%w` gewrappt) und `ErrVersionNotPublished` (Engine.Start()s bisherige "not published"-Fehlermeldung). **Ein echter Bug beim Wrappen selbst gefunden+behoben:** ein erster automatisierter Umbau-Versuch hängte den Sentinel bei mehreren mehrargumentigen `fmt.Errorf`-Aufrufen ans ENDE der Argumentliste statt direkt hinter den Format-String — `%w` verzehrt aber immer den ERSTEN Format-Arg, das hätte falsche Werte in die Fehlermeldungen eingesetzt (Kompilierfehler blieb aus, `go vet`s Printf-Checker fing es aber ab, bevor es committet wurde). `writeProcessError` meldet `ErrConcurrentModification`/`ErrVersionNotPublished`/`statemachine.ErrInvalidTransition` bewusst einheitlich als 409 (Zustands-Vorbedingung verletzt, Aufrufer soll neu lesen statt blind wiederholen), `ErrValidation` als 400, `ErrNotFound` als 404. `internal/asset` weiterhin nicht verdrahtet (Asset-API ist Phase 5 Teil 3, eigene Sitzung). **Live Ende-zu-Ende gegen den echten laufenden Orchestrator verifiziert** (Nutzer hat einem weiteren Neustart zugestimmt): kompletter Zyklus Definition anlegen → Version anlegen (draft) → Start gegen draft schlägt mit 409 fehl → Publish → Start → Engine treibt Wait-Schritt real bis "completed" (per Login als `admin`, echtes Bearer-Token, echte Postgres-Persistenz); zweiter Zyklus mit einem `human_task`-Schritt: Assign setzt nur den Assignee (Status bleibt "pending", wie `Store.AssignHumanTask` es tatsächlich tut — kein Bug, per Test entdeckt statt angenommen), zwei separate Complete-Aufrufe (`claimed`, dann `approved`) bringen die wartende Execution korrekt zu Ende; Cancel eines pending-Laufs verifiziert; Definition-Update verifiziert; unauthentifizierter Request liefert 401. Bootlog danach ohne neue Fehler-/Warn-Zeilen. Verifikation: `go build ./...`/`go vet ./...`/`gofmt -l` sauber, 2 neue Sentinel-Tests (`TestValidateErrorsAreErrValidation`, `TestEngineStartRejectsUnpublishedVersion`), volle Modul-Suite (`go test ./...`, 34 Pakete) grün. Details: `docs/decisions.md` Nachtrag 264. | 2026-09-22 |
| Kapitel 21 Phase 5 Teil 3 (HTTP-API für die Asset/Content-Domäne: Assets/Versionen/Representations/Metadaten/Lifecycle) | erledigt | Nutzerauftrag "fahre fort", direkt im Anschluss an Phase 5 Teil 2. 14 neue Endpunkte unter `/api/v1/assets`, `/api/v1/asset-versions`, `/api/v1/representations` (Namensraum neu entschieden, Phase 1 hatte hierfür noch keinen dokumentiert — spiegelt aber exakt dasselbe Muster wie der Process-Namensraum aus Phase 1/5 Teil 2). Neue Datei `orchestrator/internal/httpapi/asset_handlers.go`, neues Interface `AssetService` (anders als Process EIN Interface, weil es nur einen konkreten Typ `*asset.Store` gibt, keine getrennte Engine). Verb-Einstufung: Asset/Version/Representation ANLEGEN = `configure` (Katalog-Pflege, kein Sofortwirkungsrisiko wie ein Process-Start), Status-/Metadaten-Änderung an einem BESTEHENDEN Asset (Lifecycle-Fortschritt B8) = `operate` (redaktionelle Bedienhandlung an existierendem Content, gleiche Einstufung wie HumanTask-Zuweisen/-Entscheiden). Neuer `ErrValidation`-Sentinel in `internal/asset` (analog zu Phase 5 Teil 2s `process.ErrValidation`) — wrappt die beiden bisherigen unstrukturierten Pflichtfeld-Fehler in `CreateAsset`/`CreateRepresentation`. `main.go` konstruiert jetzt endlich `asset.NewStore(database, asset.WithOutbox(outboxStore))` — der seit Phase 5 Teil 1 bewusst aufgeschobene erste echte Aufrufer für die Asset-Domäne, `WithOutbox` damit zum ersten Mal in Produktion tatsächlich aktiv (B9, seit Phase 4 Teil 1 im Store vorbereitet, aber bis jetzt nie ausgelöst). **Live Ende-zu-Ende gegen den echten laufenden Orchestrator verifiziert** (dritter Neustart dieser Sitzungsreihe, Nutzerzustimmung eingeholt): Asset anlegen → Version anlegen (draft) → Publish → `assets.currentVersionId` atomar aktualisiert nachgewiesen → Representation anlegen+listen → Status-Übergang `ingesting → registered` → ein ungültiger Übergang `registered → published` (überspringt `ready`/`in_review`/`approved`) liefert korrekt 409 → Metadaten-Update → gefilterte Liste (`?type=VIDEO`) → Representation löschen, zweites Löschen bleibt idempotent 200 → unauthentifizierter Request liefert 401. **Erstmals bestätigt, dass B9 tatsächlich Nachrichten zustellt**: `OMP_EVENTS`-Stream zeigte nach dem Testlauf reale Nachrichten (`nats stream info`), nicht nur eine leere, aber korrekt angelegte Infrastruktur wie noch in Phase 5 Teil 1. Bootlog danach ohne neue Fehler-/Warn-Zeilen. Verifikation: `go build ./...`/`go vet ./...`/`gofmt -l` sauber, 2 neue Sentinel-Tests (`TestCreateRepresentationRequiresStorage` umgestellt auf `errors.Is`-Prüfung, neu `TestCreateAssetMissingFieldsIsErrValidation`), volle Modul-Suite (`go test ./...`, 34 Pakete) grün. Damit ist Kapitel 21s Backend (Domain Model, Runtime, HTTP-API für beide Domänen) vollständig; offen bleibt nur noch UI (braucht Nutzerentscheidung zu Design-Frage 3 aus Phase 1) sowie die bereits mehrfach dokumentierten, bewusst zurückgestellten Einzelpunkte (Loop, StepTypeEventTrigger, B1-Vollmodell, B7, B14-Autorisierungsfeinheit). Details: `docs/decisions.md` Nachtrag 265. | 2026-09-22 |
| Kapitel 21 Phase 6 Teil 1 (Process-Tab in der Orchestrator-UI: Definitions/Versions/Executions/HumanTasks) | erledigt | Nutzerauftrag "fahre fort", direkt im Anschluss an Phase 5 Teil 3 — zuvor per Rückfrage die seit Phase 1 offene Design-Entscheidung 3 eingeholt: **eigener Editor auf `ui/graph`-Basis (empfohlene Option) bestätigt, kein Blockly.** Neuer Tab "Prozesse" (`ui/shell/process-view.ts`, ~700 Zeilen, Muster an `instances-view.ts`/`workflows-view.ts` gespiegelt: `apiFetch`+SSE-Fallback-Poll, `.omp-card`/`.omp-badge`/`.omp-modal`-Klassen, keine neue Abhängigkeit), registriert in `app-shell.ts` zwischen Workflows und Hosts. Master-Detail-Layout: links Definitionsliste (+Neu-Modal), rechts pro ausgewählter Definition Versionen (Status-Badges, Veröffentlichen/Deprecaten/Archivieren, +Neue-Version-Modal) und Executions (Start/Abbrechen/Pausieren/Fortsetzen, Klick auf eine Zeile zeigt Schritte+HumanTasks dieser Execution); separates "Meine Human Tasks"-Panel (gefiltert auf den eigenen Nutzernamen aus `whoami()`) mit Für-mich-beanspruchen/Genehmigen/Ablehnen/Änderungen-anfordern-Aktionen. **Bewusste Scope-Grenze, im Code dokumentiert:** der Schritt-Graph (`Definition.steps`) wird noch als rohes JSON in einem `<textarea>` bearbeitet, NICHT im geplanten visuellen Editor (der die geteilten `geometry.ts`/`compatibility.ts`-Primitiven aus `ui/graph` wiederverwenden soll, UMSETZUNG.md §6b 21.1 "Präzedenzfall genau für A10") — dieser Teil macht zunächst das komplette Phase-2-5-Backend in der UI nutzbar/testbar, der visuelle Editor (Phase 6 Teil 2) ersetzt später nur dieses eine `<textarea>`. **Live per echtem Browser-Klicktest verifiziert** (Nutzerwunsch "UI click-test, not just API", CDP-über-rohem-WebSocket-Client selbst geschrieben — keine Python-Pakete verfügbar, reines stdlib `socket`+RFC6455-Framing, headless Chromium): Login → Tab-Klick → "+ Neu"-Modal ausgefüllt+abgeschickt (echter POST, Liste aktualisiert sich) → Definition ausgewählt → "+ Neue Version" mit Beispiel-JSON abgeschickt → "Veröffentlichen" geklickt (Status-Badge wechselt live) → "Starten" geklickt → Execution-Zeile geklickt → Schritt-Status zeigt "running" dann (per API nachverifiziert) "completed"; zweiter Durchlauf mit einer HumanTask-Definition: Execution gestartet, "Für mich beanspruchen" geklickt (Assign+Claim in einem Klick, wie im Code dokumentiert), `window.prompt` für den Kommentar-Dialog gestubbt (headless Chrome kann native Dialoge nicht bedienen), "Genehmigen" geklickt → Execution lief bis "completed" mit korrektem `decision`/`comment` im Output — alles per direktem API-Read gegenverifiziert, nicht nur per DOM-Text vertraut. Keine JavaScript-Fehler während des gesamten Durchlaufs (`window.onerror`/`unhandledrejection`-Sammler leer). Verifikation: `deno check` sauber, `deno test ui/` (97/97) grün, `deno bundle` erfolgreich (38 Module). Details: `docs/decisions.md` Nachtrag 266. | 2026-09-22 |
| Kapitel 21 Phase 6 Teil 2 (visueller Drag&Drop-Schritt-Graph-Editor `<omp-process-editor>`) | erledigt | Nutzerauftrag "fahre fort", direkt im Anschluss an Phase 6 Teil 1 — ersetzt das dort bewusst dokumentierte JSON-`<textarea>`-Provisorium durch den in Phase 1 als "Präzedenzfall genau für A10" identifizierten Editor-Typ. Zwei neue Dateien: `ui/graph/process-editor-logic.ts` (DOM-frei, `deno test`-geprüft — `DraftStep`/`DraftDefinition` spiegeln `orchestrator/internal/process/types.go` `Step`/`Definition` 1:1 im JSON-Wire-Format; Funktionen `addStep`/`removeStep`/`renameStepId`/`addNextConnection`/`addBranchConnection`/`setStartStep`/`setCompensationStep`/`updateStepFields` — `removeStep`/`renameStepId` räumen jede next-/branches-/compensationStepId-/startStepId-Referenz mit auf, gleiches Muster wie `role-designer-logic.ts#removeRole`/`#renameRole`) und `ui/graph/process-editor.ts` (`<omp-process-editor>`, Custom Element). Wiederverwendet `geometry.ts` UNVERÄNDERT (Pan/Zoom, `NODE_WIDTH`/`HEADER_HEIGHT`, `arrangeByFlow` für "Auto-Anordnen" — exakt die layer-basierte Topologie-Anordnung, die eine Schritt-Kette links-nach-rechts sinnvoll aufreiht). Interaktionsmuster (Pointer-Zustandsmaschine für Pan/Kachel-Verschieben/Verbinden, Klick-oder-Ziehen-Palette, Kante/×-Löschen) bewusst 1:1 von `role-designer.ts` gespiegelt (dem dokumentierten Präzedenzfall) statt einer dritten gemeinsamen Basisklasse — Begründung identisch zu `role-designer.ts`s eigener Kopfkommentar-Begründung. Ein Ausgangs-Anker pro Kachel trägt beliebig viele Kanten; nach dem Ablegen entscheidet ein kleines Modal zwischen unbedingter `next`-Kante (blau, durchgezogen) und benannter `branches`-Kante (orange, gestrichelt, Label-Textfeld) — `compensationStepId` ist bewusst KEINE Zieh-Kante (die Domäne behandelt sie separat vom normalen Graph-Vorgänger-Pfad, `validate.go`), sondern ein Dropdown im Konfigurations-Modal je Kachel, visuell nur als rot-gepunktete Linie mitgezeichnet. Konfigurations-Modal je Kachel deckt Name/Config(JSON)/Retry(JSON)/TimeoutSeconds/CompensationStepID ab — die STRUKTUR des Graphen (welche Schritte, wie verbunden, welcher Start) ist vollständig visuell, die pro-Schritt-Feinkonfiguration bleibt bewusst ein kleineres, in sich geschlossenes JSON-Feld (17 Schritt-Typen mit je eigener Config-Form wären eine eigene, deutlich größere Design-Sitzung wert gewesen). `ui/shell/process-view.ts` vereinfacht: `#renderVersionFormModal`/`#showVersionForm`/`EXAMPLE_STEP_GRAPH` entfernt, "+ Neue Version" öffnet jetzt `<omp-process-editor>` als Vollbild-Overlay (gleiches Einhänge-Muster wie `workflows-view.ts#openRoleDesigner`, direkt an `document.body`, kein zusätzliches umschließendes `.omp-modal`) — der Editor trägt sein eigenes `changeReason`-Feld in der Toolbar, `#createVersion` nimmt jetzt eine strukturierte `DraftDefinition` statt eines rohen JSON-Strings entgegen. **Live per echtem Browser-Interaktionstest verifiziert** — diesmal mit ECHTEN synthetischen Maus-Events über CDP `Input.dispatchMouseEvent` (nicht nur `element.click()`/DOM-Events wie in Phase 6 Teil 1, da `setPointerCapture()` bei rein DOM-dispatchten `PointerEvent`s ohne einen vom Browser selbst verfolgten aktiven Zeiger wirft — echte Maus-Input-Simulation über die Chrome-Input-Pipeline umgeht das, exakt wie Puppeteer/Playwright intern arbeiten): zwei Schritte per Paletten-Klick angelegt, ECHTER Ziehen-Vorgang vom Ausgangs-Anker zur Zielkachel (Koordinaten aus echten `getBoundingClientRect()`-Werten berechnet) öffnet das Kantentyp-Modal, "Direkt (Next)" gewählt → Kante gezeichnet; Start-Schritt umgeschaltet (Toolbar aktualisiert sich); Konfigurations-Modal geöffnet, Config-JSON gesetzt, übernommen; "Speichern" geklickt → Version erfolgreich angelegt (Toast, Editor schließt, Version erscheint in der Tabelle). **Eine echte Backend-Validierungsablehnung dabei absichtlich provoziert und bestätigt statt als Bug missverstanden**: ein zweiter Testlauf verband zwei Schritte und verschob danach den Start-Schritt so, dass der ursprüngliche Start-Schritt unerreichbar wurde (kein Vorgänger mehr, nicht mehr Start) — `Definition.Validate()` lehnte das korrekt mit 400/`ErrValidation` ab, die UI zeigte den Fehler-Toast (per erneutem, gezieltem Timing-Check nachgewiesen — der erste Beobachtungsversuch hatte den 4s-Toast schlicht verpasst) und ließ den Editor offen, statt eine ungültige Version stillschweigend anzulegen oder abzustürzen — korrektes Verhalten auf beiden Seiten, kein Fund. `Auto-Anordnen` und `Abbrechen` (schließt ohne zu speichern) ebenfalls verifiziert. Keine JavaScript-Fehler während des gesamten, mehrteiligen Testlaufs. Verifikation: `deno check` sauber, `deno test ui/` 113/113 grün (16 neue `process-editor-logic_test.ts`-Tests), `deno bundle` erfolgreich (40 Module). Details: `docs/decisions.md` Nachtrag 267. | 2026-09-22 |
| Prozess-Editor: "Bearbeiten"-Button für bestehende Versionen (Nachtrag 268) | erledigt | Nutzerfrage "wie editiere ich prozesse?" — Editor konnte bestehende Versionen laden, UI nutzte es nie. Jetzt "Bearbeiten" je Versionszeile → neue Draft-Version auf Basis von vN; latenter Trigger-Verlust beim Bearbeiten mitbehoben (+1 Regressionstest, 114/114). Live per CDP-Mausklick verifiziert. | 2026-09-23 |
| Kapitel 21 Phase 6 Teil 3 (Assets-Tab) + B3-Fix (Representations nur an Drafts) + TriggerListener-Stop-Rennen + Test-Stream-Leck (Nachtrag 269) | erledigt | Nutzerauftrag "danach starte mit assets-tab". Neuer Tab `ui/shell/asset-view.ts` (+ `asset-view-logic.ts`, 6 Tests), neuer Endpunkt `GET /api/v1/asset-lifecycle` (`statemachine.Graph()`), Backend erzwingt B3 jetzt (409 `ErrVersionImmutable`, Zeilensperre gegen Publish-Rennen, +2 DB-Tests). Beim Voll-Testlauf echten Produktions-Deadlock in `TriggerListener.Stop()` per Stack-Dump gefunden+behoben (deterministischer Regressionstest, ohne Fix rot), `defer closeNC()`-Leck der TEST_*-Streams behoben. `internal/process` 20/20, Voll-Suite 35/35 (ein unabhängiger Workflows-Timing-Flake dokumentiert), `deno test` 120/120, Live-CDP-Klicktest 9 Szenarien nach Neustart mit Nutzerzustimmung. | 2026-09-23 |
| Prozess-Editor ohne JSON: Formulare je Schritt-Typ, Variablen-Picker, Regel-Baukasten, Auslöser; Test-Isolation je Paket + 4 Flakes (Nachtrag 270) | erledigt | Nutzerauftrag "normale User sind mit JSON überfordert … dann fahre fort mit den offenen Punkten". `ui/graph/process-step-config(.ts|-logic.ts)`, `GET /api/v1/process-capabilities`, `${…}` eingebettet in Script-Args/URL/Header. Live: Prozess nur über Formulare gebaut, echter ffprobe-Lauf, Auslöser Ende-zu-Ende, Node-Funktion gegen echten omp-recorder. Offene Punkte aus 269 erledigt (process-view-Poll, workflows-Flake); dabei geteilte Testdatenbank als Ursache mehrerer Flakes gefunden → `dbtest` je Paket, 6 Pakete von echter Dev-DB auf dbtest umgestellt. Voll-Suite 3×35/35, deno 128/128. | 2026-09-23 |
| Zeitbasis-Bug (PRIO HOCH): Medienzeit = Grain-Index auf TAI (Nachtrag 271); WHEP-Video-Regression root-caused (Nachtrag 272) + gefixt, alle betroffenen Hot-Swap-Aufrufer live verifiziert (Nachtrag 273) | erledigt | Nutzerauftrag "fix timebase bug". `MxlVideoInput`/`MxlAudioInput::new()` in `new_unsynced()`+`activate()` gesplittet (extern verlinken vor dem PLAYING-Hochziehen, GStreamer-eigenes Muster) — behebt den in Nachtrag 272 root-gecausten `not-linked`-Race. Alle ~25 Aufrufer einzeln klassifiziert: 4 echte Hot-Swap-Fälle gefixt (`omp-webrtc-gateway::monitor`, `omp-switcher::build_branch`, `omp-audio-mixer::add_channel_branch`, `omp-viewer::audio_meters::build_branch` — letzterer live gefunden, nicht vorher vermutet), Rest bestätigt sicher (baut jeweils frische, noch nicht PLAYING `gst::Pipeline`). Live verifiziert: derselbe WHEP-Repro liefert jetzt >1100 Frames/15s statt 0; Switcher hält 5× schnellen Input-Reselect ohne `not-linked`. `cargo test --workspace` grün, Clippy ohne neue Warnungen. Separater, andersartiger "Session-Churn"-Bug (Nachtrag 247/252, schnelle automatisierte Reconnects) bleibt offen — nicht Teil dieses Fixes. OFFEN: echte Handy-Bestätigung durch den Nutzer. | 2026-09-23 |
...
| Kapitel 21 B14: Mandantenfähigkeit implementiert + live verifiziert (Nachtrag 283) | erledigt | Nutzerauftrag "start B14 implementation". Migration `0025_organizations.sql` (`organizations`-Tabelle, `users.org_id`, `owner_org_id` auf Workflows/ProcessDefinitions/Assets/Collections). Neues Paket `internal/organizations` (CRUD, Default-Org unlöschbar, echter FK-Schutz gegen noch existierende Mitglieder). `auth.User`/`Principal` um `OrgID` erweitert — bewusst NICHT im JWT (frisch aus DB pro Request, wie `SessionsEpoch`). Domain-Create-Methoden stampeln `OwnerOrgID`; nested Entitäten (Versionen/Executions/HumanTasks/Representations/Members) leiten ihre Organisation über die Elternkette ab, kein eigenes Feld. Neues gemeinsames `internal/httpapi/org_enforcement.go`: 404 (nicht 403) bei Cross-Org-Zugriff, Bootstrap-/Service-Token-Prinzipale bewusst ausgenommen (geteilte Infrastruktur). Jede Workflow-Aktion (auch Start/Stop/Migrate-Role/Pause) ist org-gescopt, nicht nur CRUD. Neue admin-only, global gescopte Organisations-Verwaltung (`/api/v1/organizations`). Bewusste, dokumentierte Lücken: `human-tasks?assignee=`-Liste und `DELETE /asset-relationships/{id}` bleiben ungefiltert, kein Cross-Org-Superadmin-Bypass (live beim Testdaten-Aufräumen gefunden: `admin` konnte fremde Org-Daten nicht per API löschen — konsistent mit "1 Org/Nutzer", aber ein bewusster Gap). Volle Suite (38 Pakete) grün. **Live verifiziert:** echte zweite Organisation + Nutzer angelegt, je ein Workflow/Asset/ProcessDefinition/Collection pro Organisation, Listen zeigen nur die eigene Organisation, Cross-Org-GET/-Start → 404, Bestandsdaten der Default-Organisation unverändert sichtbar, danach vollständig aufgeräumt. | 2026-09-23 |
| UI-Anbindung: Organisationen/Collections/Beziehungen/Asset-Links + Prozessliste durchsuchbar/sortierbar (Nachtrag 284) | erledigt | Nutzerauftrag "zuerst UI Anbindung, dann B11" + "Prozessliste durchsuchbar und sortierbar nach name/datum/user/status". Schließt die UI-Lücke für B14/B12/B10 (nur APIs existierten). Kleine Backend-Ergänzungen: `userResponse.orgId`, neu `PUT /api/v1/auth/users/{name}/org`, `process.Store.ListDefinitions()` liefert `latestVersionStatus` (LEFT JOIN LATERAL, Grundlage der Status-Sortierung). `admin-view.ts`: neuer Sub-Tab "Organisationen" (Liste/Anlegen/Löschen, 409-Fehler in verständliche Sätze übersetzt), Nutzer-Tabelle mit Organisation-Spalte + Inline-Wechsel. `asset-view.ts`: Assets/Collections-Umschalter, Collections-CRUD+Mitgliederverwaltung, neue Karten "Beziehungen" und "Verknüpfte Prozessläufe" am Asset. `process-view.ts`: Suchfeld+Sortierung im Seitenpanel (dafür Umbau von Voll-Rerender auf stabilen DOM-Rahmen, sonst Fokusverlust pro Tastendruck), neue Karte "Verknüpfte Assets" an der Execution (B10-Anlage). Live-gefundener+gefixter Bug: `#applyViewModeStyles()` griff beim ersten Tab-Öffnen auf noch nicht zugewiesene Felder zu (`TypeError`), per echtem CDP-Klicktest gefunden. Live verifiziert (hand-gerolltes CDP-Skript, Chromium headless): Organisation anlegen/löschen inkl. übersetzter 409-Konfliktmeldung, Nutzer-Org-Wechsel, Collection+Mitglied, Beziehung (korrekte Richtung), Asset-Link Execution↔AssetVersion beidseitig sichtbar, Status-Sortierung gruppiert echte Einträge korrekt — keine Konsolenfehler, danach vollständig aufgeräumt. `deno check`/`deno test ui/` (128/128), `go test -count=1 ./...` (38/38, echte Mehr-Host-DSN) grün. OFFEN: B11 (Volltextsuche), letzter Punkt aus Kapitel 21 Teil B. | 2026-09-24 |
| Kapitel 21 B11: Asset-Volltextsuche (tsvector/GIN) + Fix Alarmleiste überlappte Nutzer-Widget (Nachtrag 285) | erledigt | Nutzerauftrag "proceed B11" + Nebenbefund "alarm footer wird von 'angemeldet als ...' und Abmeldebutton überlappt". Schließt Kapitel 21 Teil B vollständig ab. B11: Migration `0026_asset_search.sql` (generierte `assets.search_vector tsvector`, gewichtet Titel>Typ>Beschreibung>Metadaten, `'simple'`-Konfiguration bewusst statt sprachspezifisch, GIN-Index), neue `Store.SearchAssets` (`websearch_to_tsquery`), neuer Endpunkt `GET /api/v1/assets/search?q=...` (org-gefiltert). `asset-view.ts`: Suchfeld liefert sofortiges Client-Substring-Feedback, ersetzt es nach 250ms Debounce durch das echte relevanzsortierte Backend-Ergebnis, Typ/Status/showDeleted bleiben on top wirksam. Live verifiziert: Suche nach einem eigens gesetzten Metadatenwert findet das Asset (das konnte die alte Substring-Suche nie), Titelsuche ebenso, Leeren fällt korrekt zurück. Store-Tests belegen Rangfolge (Titeltreffer vor Beschreibungstreffer). Nebenbefund: `<omp-alert-bar>`-Footer überlappte das fest positionierte "Angemeldet als …"-Widget (beide unabhängig in der unteren rechten Ecke) — Fix per `ResizeObserver` in `auth.ts` (beobachtet die tatsächliche Höhe eines evtl. vorhandenen `<omp-alert-bar>`-Elements, weicht automatisch aus, keine feste Kopplung zu app-shell.ts/alert-bar.ts nötig, da auth.ts auch von der Console-Ansicht ohne Alarmleiste genutzt wird). Live per CDP verifiziert: `getBoundingClientRect()` beider Elemente überschneiden sich nicht mehr. `go test -count=1 ./...` (38/38), `deno check`/`test ui/` (128/128) grün. | 2026-09-24 |
| Super-admin-verwaltete Storage-Backends: live hinzufügbar/entfernbar, keine hidden configs (Nachtrag 286) | erledigt | Nutzerauftrag "asset folder should be able to be dynamically added/removed without the need of restart... only super admins may config this... i dont like hidden configs at all... full control (with warnings, protection guards, hints, wizards) and full overview" — im Anschluss an eine Architektur-Rückfrage zu Gruppen-Rechten/AD-Anbindung (Empfehlung: Gruppen als neuer Binding-Subject-Typ statt Ersatz für Nutzer-Bindungen, noch nicht umgesetzt) und wo Prozesse/Assets/Storage tatsächlich gehostet sind. Löst B5s einzelnes, fest über `OMP_MINIO_*` konfiguriertes Objektspeicher-Setup (Nachtrag 280) durch eine echte, mehrere Backends gleichzeitig verwaltende Registry ab — zwei Design-Entscheidungen vorab per `AskUserQuestion` bestätigt (mehrere Backends gleichzeitig; Zugangsdaten serverseitig verschlüsselt, AES-256-GCM, Go-Standardbibliothek). Neues Paket `internal/storagebackends`: Store hält einen Live-Cache verbundener `objectstore.Client`-Instanzen (der "kein Neustart nötig"-Mechanismus), Create/UpdateMeta verbinden VOR dem Schreiben (ein unerreichbares Backend wird nie persistiert). Migration `0027_storage_backends.sql`, `representations.storage_backend_id` als Fremdschlüssel OHNE Kaskade (derselbe Lösch-Schutz-Mechanismus wie Organisationen, Nachtrag 283) + `CountRepresentations` für eine konkrete Fehlermeldung vor dem Löschversuch. Lifecycle active→deprecated→gelöscht als abgestufter Schutz-Zwischenschritt. `objectstore_handlers.go` umgeschrieben auf Backend-ID-Auswahl statt einer globalen Instanz. `main.go`: einmalige Migrations-Komfortfunktion (bestehendes `OMP_MINIO_*`-Dev-Setup wird beim ersten Start automatisch als sichtbares Backend registriert), klarer Log-Eintrag bestätigt den aktiven Zustand. Neuer Administration-Sub-Tab "Storage" (`VerbAdmin` global = "Super-Admin" in diesem System): volle Übersichtstabelle, einseitiger Wizard mit "Verbindung testen"-Schritt, zentraler Protection-Guard (Abschicken ohne erfolgreichen Test löst eine explizite Rückfrage aus, jede Formularänderung entwertet einen alten Testerfolg), Bearbeiten mit leer vorbefülltem Secret Key, Deaktivieren/Reaktivieren/Entfernen. Deaktiviertes Feature zeigt einen erklärenden Hinweis inkl. Erzeugungsbefehl statt sich zu verstecken. Live verifiziert (echte Dev-MinIO, CDP): Backend angelegt+getestet+erscheint sofort, Lifecycle-Toggle, Bearbeiten-Vorbefüllung, Untested-Save-Guard als echter Dialog, Lösch-Blockade mit exaktem Dateizähler bei einer echten referenzierenden Representation, danach sauber aufgeräumt. `go test -count=1 ./...` (39/39 Pakete inkl. neuem `internal/storagebackends` mit echten MinIO-Integrationstests), `deno check`/`test ui/` (128/128) grün. | 2026-09-24 |
| Gruppenbasierte Rechteverwaltung + OMP_STORAGE_SECRET_KEY dauerhaft persistiert (Nachtrag 287) | erledigt | Nutzerauftrag "ja schluessel dauerhaft speichern. dann gruppen basierte rechteverwaltung. (ist dann Organisation noch noetig?)". OMP_STORAGE_SECRET_KEY: `deploy/dev/start-omp.sh` erzeugt+sourct jetzt automatisch `deploy/dev/storage-secret.env` (gitignored, gleiches Muster wie mxl.env) — live verifiziert, dass ein Neustart ohne manuelles Exportieren denselben Schluessel behaelt. Organisation bleibt noetig: sie steuert Sichtbarkeit (welche Objekte), Gruppen steuern Rechte (welche Verben) — unabhaengige, komplementaere Mechanismen. Migration `0028_groups.sql` (`groups`+`group_members` mit echten Fremdschluesseln, `role_bindings.subject_type` Default `user`/neu `group`). Neues Paket `internal/groups`; `authz.Store.Check`/`CheckWorkflow` loesen jetzt auch Gruppen-Mitgliedschaft auf; neue `CreateGroupBinding`-Methode statt Signaturaenderung der bestehenden `Create` (kein Blast-Radius auf die drei gruppenunabhaengigen Aufrufer). Live-gefundene+geschlossene Luecke: die "letzter Admin"-Selbstschutzpruefung erfasste Gruppen-Admin-Bindungen nicht — `globalAdminSubjects` loest jetzt Gruppen auf, `handleDeleteGroup` bekam eine eigene Sperre fuer den Kaskaden-Pfad, drei gezielte Tests. Neuer Administration-Sub-Tab "Gruppen" (CRUD+Mitgliederverwaltung), Rollenbindungen-Formular bekam einen Nutzer/Gruppe-Umschalter. Live verifiziert (CDP+curl): Gruppe angelegt, Mitglied hinzugefuegt, globale "configure"-Bindung auf die Gruppe — als Mitglied eingeloggt: Zugriff vorher verweigert, nach Gruppenbindung erlaubt, nach Entfernen aus der Gruppe wieder verweigert (403) — voller Gewaehren-und-Widerrufen-Kreislauf ueber Gruppenmitgliedschaft bestaetigt. `go test -count=1 ./...` (40/40 Pakete), `deno check`/`test ui/` (128/128) gruen. | 2026-09-24 |
| Rollenbindungen: verwaiste Instanz-Bindungen beim Stop/Neustart aufräumen (commit 3f20e65) | erledigt | Backfill dieser Status-Checkliste — Arbeit wurde am 2026-09-24 geleistet und gepusht, aber nicht hier protokolliert. Live-Audit fand 25/27 Karteileichen in `role_bindings` (verwaiste Service-Token-Instanz-IDs); neue `authz.Store.DeleteInstanceBinding`, aufgerufen aus `runStop`/`runRestartRole`. Details `docs/decisions.md`. | 2026-09-24 |
| Rechte-Übersicht je Nutzer/Gruppe: Rollenbindungen+Gruppen+Organisation vereinfacht sichtbar gemacht (commit 9c46f09) | erledigt | Backfill (s. Zeile oben, gleiche Session, gepusht). UI-Konsolidierung in `admin-view.ts`: "Rechte ▼"-Panel je Nutzer (direkt + über Gruppen geerbt), Gruppen-Tab zeigt "Rechte dieser Gruppe" direkt darunter. Kein Backend-Modell geändert. | 2026-09-24 |
| Operator-Home-Button (Regieplatz wechseln) + Handbuch-Screenshots + Assets-Doku für lokale/Netzlaufwerk-Pfade (commit dd71aa1) | erledigt | Backfill (s. Zeilen oben). `buildUserWidget`/`shell.ts` bekamen einen Home-Button für Operatoren mit mehreren Workflow-Bindungen; `docs/BENUTZERHANDBUCH.md` neues §5b.1 (lokale/Netzlaufwerk-Assets, kein Browser-Upload). Live per echtem Browser-Klicktest + echten Workflows verifiziert. Dabei live bestätigt gefunden, NICHT in dieser Session gefixt: `DELETE /api/v1/auth/users/{name}` räumt `role_bindings` nicht auf (s. Nachtrag 288 unten). | 2026-09-24 |
| Home-Button/Kachel-Auswahl neutral auf "Workflow" statt "Regieplatz" umbenannt (commit e47f717) + Viewer/Multiviewer-Vorschau: Freeze bei Bildrate-Wechsel behoben, Teilfix (commit 56998a5) | erledigt | Backfill (s. Zeilen oben). Drei reale Pad-Races im alten Tee-Branch-Rebuild-Design gefixt — Nutzer-Repro (8 Änderungen/100ms) fror danach aber weiterhin ein, s. nächste Zeile für die echte Root-Cause. | 2026-09-24 |
| Handy-Kamera: Retourbild/Monitor über kombinierte Einladung + Kachel-UI (commit d437d4d) | erledigt | Backfill (s. Zeilen oben, gepusht). Ein Invite erzeugt jetzt Kamera+Monitor-Zugriff gemeinsam (`camera.html` volles State-Machine-Rework: Setup-only vor Verbindung, Half/Half-Split danach, Tap-to-Enlarge mit PIP). 13+15 CDP-Assertionen grün, `cargo test -p omp-webrtc-gateway` 16/16 — echter Phone-E2E-Test noch offen (kein Hardware-Zugriff in dieser Umgebung). | 2026-09-24 |
| Viewer-Vorschau-Freeze: echte Root Cause gefunden und behoben (commit d92e651) | erledigt | Backfill (s. Zeilen oben, gepusht). Wahre Ursache war eine Tee-Request-Pad-Race BEIM ANHÄNGEN einer zweiten Branch an einen bereits laufenden Tee (nicht nur Timing wie zuvor angenommen) — Fix: `previewFps` ändert nur noch `capsfilter.caps` live, keine Pad-Verknüpfung/-Trennung mehr nötig (−82 Zeilen). 6/8/20-Änderungen-Bursts live fehlerfrei bestätigt. | 2026-09-24 |
| Fix: `DELETE /api/v1/auth/users/{name}` ließ verwaiste `role_bindings` zurück (Nachtrag 288) | erledigt | Nutzerauftrag "proceed" — Fix des in der Operator-Home-Button-Session (dd71aa1, s. oben) live gefundenen, damals nur dokumentierten Folgefunds. Neue `authz.Store.DeleteBySubject(username)`, `subject_type='user'`-gefiltert (Gruppen-/Service-Token-Bindungen bleiben unberührt), aufgerufen aus `handleDeleteUser` nach erfolgreichem `authSvc.DeleteUser`, best effort (`slog.Warn`), gleiches Muster wie `DeleteByWorkflow`/`DeleteInstanceBinding`. Neue Tests: `internal/authz/store_test.go` (echte Dev-Postgres), zwei in `internal/httpapi/auth_handlers_test.go` (Recording-Wrapper-Pattern). `go build`/`vet`/`test ./...` grün (39 Pakete, inkl. `internal/authz`+`internal/httpapi` real gegen laufende Dev-Patroni). Live verifiziert: Orchestrator neu gebaut+neugestartet (`make stop && make start`), Testnutzer+Bindung angelegt, per `psql` vor/nach `DELETE /api/v1/auth/users/…` bestätigt (1 Zeile → 0 Zeilen). Details `docs/decisions.md` Nachtrag 288. Noch offen: derselbe Karteileichen-Check für Gruppen-Löschung wurde nicht geprüft (out of scope). | 2026-09-25 |
| Root-Cause "Viewer/Handy-Monitor/etc. zeigen dauerhaft 'nicht verbunden'" gefunden + für elf Nodes gefixt (Nachtrag 289) | erledigt | Nutzerauftrag: externe `bugliste 25.09.2026.txt`, Punkt 3 zuerst (Nutzerwahl). Root Cause: IS-05-`Connect` vor Flow-Existenz der Quelle lässt `MxlVideoInput::new`/`MxlAudioInput::new` einmalig scheitern, KEIN Node retried das bisher — betrifft `omp-viewer` (Video + dynamische Audio-Eingänge), `omp-multiviewer(-custom)`, `omp-scaler`, `omp-audio-monitor`, `omp-scope` (Video+Audio), `omp-decklink`, `omp-aes67-gateway`, `omp-2110-gateway` (Video+Audio), `omp-audio-mixer`, `omp-webrtc-gateway` (Handy-Monitor/Retourbild, Nutzers eigenes Beispiel — eigene Architektur, zwei neue Tokio-Interval-Tasks statt Kommando-Loop-Tick). Fix überall: letzter fehlgeschlagener Verbindungswunsch wird gemerkt, beim nächsten Tick automatisch erneut versucht, kein Alert-Spam. `cargo build --workspace --bins` + `cargo test` für alle elf Pakete grün. Live end-to-end verifiziert (Orchestrator-gesteuert, exakt der Nutzer-Ablauf): `omp-mxf-player-direct`+`omp-viewer`, Connect VOR `play()` → `503`, nach `play()` ohne manuellen Reconnect → echtes JPEG-Frame binnen < 1s. Bewusst NICHT gefixt (gleiche Klasse, andere Architektur/Risiko): `omp-video-mixer-me` (eigener, seit Nachtrag 212 ungeklärter Retry-Bug), `omp-switcher` (Pad-Block-Hot-Swap zu riskant für blinden Rebuild-Retry), `omp-channel-player`s `Item::Live`-Zweig, `omp-recorder` (bereits synchron fehlerfrei). Details `docs/decisions.md` Nachtrag 289. | 2026-09-25 |
| `omp-video-mixer-me` CPU von ~110-120% auf ~55-58% halbiert, zwei echte Root Causes gefunden (Nachtrag 290) | erledigt | Nutzerauftrag "fahre fort" — Bugliste #5, vom Nutzer selbst "wichtig und prioritär" markiert. Root Cause 1 (Hauptanteil): `discover()` schließt den eigenen PGM-Sender seit dem Mastereben-Feature (2026-08-14) bewusst nicht mehr aus der Eingangs-Erkennung aus — bei `level_count==1` (Standardfall) liest der Mixer dadurch dauerhaft seinen EIGENEN Ausgang als nie auswählbaren Phantom-Eingang zurück (voller `MxlVideoInput`+`tee`-Zweig, nur per Live-Thread-Aufschlüsselung gefunden: `appsrc`+2 auffällig teure `queue`-Threads trotz 0 externer Quellen). Fix: Ausschluss jetzt zusätzlich bei `own_sender_ids.len() <= 1`, Mehr-Ebenen-Verhalten unverändert. Root Cause 2 (kleiner): `compositor` hat pro Ebene immer 4 aktive Sink-Pads (fg/bg/Keyer/PIP) — die 3 synthetischen Fallback-Zweige (bg-schwarz, Keyer-Farbfläche, PIP-schwarz) durchliefen bisher unnötig die volle `videoconvert`/`videoscale`/`videorate`-Kette. Neue schlankere `build_synthetic_branch` (nur `queue`+`capsfilter`) für alle 4 synthetischen Zweige, echte externe Quellen behalten die volle Kette. Bewusst nicht versucht: dynamisches Pad-Ab-/Anhängen (gleiche Klasse wie der in Kapitel 15 Teil 3 zurückgebaute Hot-Swap-Freeze). Live verifiziert (3 Messreihen, Orchestrator-Standardinstanz, 0 Quellen, `/proc`-basiert): Baseline ~103-119%/29 Threads → nach beiden Fixes ~55-58%/24 Threads, kein `appsrc` mehr, nur noch 4 statt 7 Queues; `comp_l0` bleibt größter Posten (~29%, architektureigen). `cargo build --workspace` + `cargo test -p omp-video-mixer-me` grün, Clippy unverändert bei denselben 8 vorbestehenden Fundstellen (Nachtrag 206/212). Details `docs/decisions.md` Nachtrag 290. | 2026-09-25 |
| Bugliste #1+#2: Handy-Kamera trennt aktive Verbindung bei Widerruf (optional) + sichtbare Fehleranzeige beim Verbinden (Nachtrag 291) | erledigt | Nutzerauftrag "fahre fort". #1: `invite::route()` bekommt `on_revoke(token, also_disconnect)`-Callback, neues `disconnect=true`-Query-Flag auf `DELETE /invites`; `CameraStore`/`MonitorStore` bekommen `active_token`-Feld (gesetzt bei erfolgreichem `POST /whip`/`/whep`) und trennen per `gateway.teardown()`/`monitor.teardown()`, aber nur wenn der widerrufene Token zur aktuell aktiven Sitzung gehört. UI fragt "optional durch Abfrage" per `window.confirm()` (Muster wie `omp-audio-mixer`/`omp-mxf-player`, kein `ui/kit`-Zugriff aus Node-Bundles). #2: drei Fehlerpfade in `camera.html`s `start()` schrieben bisher nur in den standardmäßig eingeklappten `#log` innerhalb von `#live` — bei fehlgeschlagenem Connect nie sichtbar (`#setup` bleibt aktiv). Neues `#setupError`-Banner direkt im Setup-Bildschirm + `friendlyConnectError()` übersetzt `getUserMedia`-Fehler/`WHIP`/`WHEP`-Statuscodes (401 speziell) in laienverständliche Sätze. Live end-to-end verifiziert mit echter WebRTC-Verbindung (kein Mock): `chromium --headless=new --use-fake-device-for-media-stream` per roh selbstgeschriebener CDP-über-WebSocket-Steuerung — widerrufener Token vor Connect → sichtbares `401`-Banner; kein Token → sichtbares Banner; echte Sitzung aufgebaut+ohne `disconnect=true` widerrufen → bleibt `connected` (Rückwärtskompatibilität); neue Sitzung MIT `disconnect=true` widerrufen → `connectionState`/`sessionActive` fallen tatsächlich auf `none`/`false`. `cargo build --workspace` + `cargo test -p omp-webrtc-gateway` (18/18, 2 neu) + Clippy grün. Details `docs/decisions.md` Nachtrag 291. | 2026-09-25 |
