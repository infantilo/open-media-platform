...
// fakeNodeSettingsStore ist ein einfacher In-Memory-Test-Double für
// NodeSettingsStore (gleiches Muster wie fakeLayoutStore).
type fakeNodeSettingsStore struct{ data map[string]json.RawMessage }

func (f fakeNodeSettingsStore) Get(nodeType string) (json.RawMessage, error) {
	if f.data == nil {
		return nil, launcher.ErrNodeSettingsNotFound
	}
	data, ok := f.data[nodeType]
	if !ok {
		return nil, launcher.ErrNodeSettingsNotFound
	}
	return data, nil
...
func (f fakeNodeSettingsStore) Put(nodeType string, data json.RawMessage) error {
	if !json.Valid(data) {
		return launcher.ErrNodeSettingsInvalidJSON
	}
	if f.data != nil {
		f.data[nodeType] = data
	}
	return nil
}

// fakeBackupSvc — Test-Double für BackupService (Nutzerwunsch
// 2026-08-13: Backup/Restore über das Browser-UI), kein echter
// podman-Aufruf nötig für Handler-Tests.
type fakeBackupSvc struct{}

func (fakeBackupSvc) Create(context.Context) (backup.Result, error) {
	return backup.Result{Name: "omp-fake.sql.gz", Path: "/tmp/omp-fake.sql.gz"}, nil
}

func (fakeBackupSvc) List() ([]string, error) { return []string{"omp-fake.sql.gz"}, nil }
...
func (f fakeSupervisorClient) TriggerRestore(context.Context, string) error { return f.err }

// fakeClusterService — Test-Double für ClusterService (D12 Teil 1), fest
// vorgegebener Status statt eines echten *cluster.Node (kein TCP-
// Listener/BoltDB-Log in reinen Handler-Tests nötig).
...
func (f fakeClusterService) Status() cluster.Status            { return f.status }
func (f fakeClusterService) IsLeader() bool                    { return f.isLeader }
func (f fakeClusterService) Join(string, string, string) error { return f.joinErr }
func (f fakeClusterService) Leave(string) error                { return f.leaveErr }
func (f fakeClusterService) LeaderHTTPAddr() (string, bool) {
	return f.leaderHTTPAddr, f.leaderKnown
}
...
func (f fakeAuthSvc) UpdateUserOrg(ctx context.Context, username, orgID string) error {
	return f.updateOrgErr
}
...
func TestHandleListSnapshots(t *testing.T) {
	...
	h := NewHandler(config.Config{UIDir: t.TempDir()}, fakeNodeLister{}, fakeEventSubscriber{ch: make(chan sse.Event)}, &fakeGraphService{}, fakeLayoutStore{}, svc, fakeLauncherService{}, fakeConsoleResolver{}, nil, fakeAuthSvc{}, fakeAuthzSvc{}, &fakeAuditSvc{}, &fakeAuditSvc{}, fakeHostRegistry{}, fakeHostMetrics{}, fakeHostHistory{}, fakeWorkflowService{}, fakePlacementAdvisor{}, fakeProfileReader{}, placement.Thresholds{}, fakeNodeSettingsStore{}, fakeBackupSvc{}, fakeSupervisorClient{}, fakeClusterService{}, nil, nil, nil, nil, nil, nil)
...
// TestHandleGetMxfPlayerSettingsDefaults prüft, dass GET die
// eingebauten ORF-Standardwerte liefert, solange nie über PUT
// gespeichert wurde (unverändertes Verhalten für jeden, der die neue
// Einstellungsseite nie öffnet).
func TestHandleGetMxfPlayerSettingsDefaults(t *testing.T) {
	h := NewHandler(config.Config{UIDir: t.TempDir()}, fakeNodeLister{}, fakeEventSubscriber{ch: make(chan sse.Event)}, &fakeGraphService{}, fakeLayoutStore{}, fakeSnapshotService{}, fakeLauncherService{}, fakeConsoleResolver{}, nil, fakeAuthSvc{}, fakeAuthzSvc{}, &fakeAuditSvc{}, &fakeAuditSvc{}, fakeHostRegistry{}, fakeHostMetrics{}, fakeHostHistory{}, fakeWorkflowService{}, fakePlacementAdvisor{}, fakeProfileReader{}, placement.Thresholds{}, fakeNodeSettingsStore{}, fakeBackupSvc{}, fakeSupervisorClient{}, fakeClusterService{}, nil, nil, nil, nil, nil, nil)

	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/api/v1/node-types/omp-mxf-player/settings", nil))

	if rec.Code != http.StatusOK {
		t.Fatalf("status = %d, want 200, body=%s", rec.Code, rec.Body.String())
	}
	var body mxfPlayerSettings
	if err := json.Unmarshal(rec.Body.Bytes(), &body); err != nil {
		t.Fatalf("invalid JSON: %v", err)
	}
	if len(body.Groups) != 5 {
		t.Errorf("groups = %d, want 5", len(body.Groups))
	}
	if len(body.Presets) != 13 {
		t.Errorf("presets = %d, want 13", len(body.Presets))
	}
}

// TestHandlePutThenGetMxfPlayerSettingsRoundTrips prüft das
// Ganz-Speichern (explizites Save, nicht PATCH-per-Feld) und dass die
// gespeicherte Fassung anschließend statt der Defaults geliefert wird.
func TestHandlePutThenGetMxfPlayerSettingsRoundTrips(t *testing.T) {
	store := fakeNodeSettingsStore{data: map[string]json.RawMessage{}}
	h := NewHandler(config.Config{UIDir: t.TempDir()}, fakeNodeLister{}, fakeEventSubscriber{ch: make(chan sse.Event)}, &fakeGraphService{}, fakeLayoutStore{}, fakeSnapshotService{}, fakeLauncherService{}, fakeConsoleResolver{}, nil, fakeAuthSvc{}, fakeAuthzSvc{}, &fakeAuditSvc{}, &fakeAuditSvc{}, fakeHostRegistry{}, fakeHostMetrics{}, fakeHostHistory{}, fakeWorkflowService{}, fakePlacementAdvisor{}, fakeProfileReader{}, placement.Thresholds{}, store, fakeBackupSvc{}, fakeSupervisorClient{}, fakeClusterService{}, nil, nil, nil, nil, nil, nil)

	body := `{"groups":[{"id":"pt","label":"Programmton","channels":2}],"presets":[{"id":"stereo","label":"Stereo","routes":[{"srcTrack":1,"group":"pt","groupChannel":0},{"srcTrack":2,"group":"pt","groupChannel":1}]}]}`
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, httptest.NewRequest(http.MethodPut, "/api/v1/node-types/omp-mxf-player/settings", strings.NewReader(body)))
	if rec.Code != http.StatusOK {
	...
	var got mxfPlayerSettings
	if err := json.Unmarshal(rec.Body.Bytes(), &got); err != nil {
		t.Fatalf("invalid JSON: %v", err)
	}
	if len(got.Groups) != 1 || got.Groups[0].ID != "pt" {
		t.Fatalf("groups = %+v, want one pt group", got.Groups)
	}
	if len(got.Presets) != 1 || got.Presets[0].ID != "stereo" {
		t.Fatalf("presets = %+v, want one stereo preset", got.Presets)
	}
}

// TestHandlePutMxfPlayerSettingsRejectsUnknownGroupReference prüft die
// zentrale Invariante, die pipeline.rs beim Pipeline-Aufbau
// voraussetzt: eine Preset-Route darf keine Gruppe referenzieren, die
// nicht in derselben Einstellung existiert.
func TestHandlePutMxfPlayerSettingsRejectsUnknownGroupReference(t *testing.T) {
	h := NewHandler(config.Config{UIDir: t.TempDir()}, fakeNodeLister{}, fakeEventSubscriber{ch: make(chan sse.Event)}, &fakeGraphService{}, fakeLayoutStore{}, fakeSnapshotService{}, fakeLauncherService{}, fakeConsoleResolver{}, nil, fakeAuthSvc{}, fakeAuthzSvc{}, &fakeAuditSvc{}, &fakeAuditSvc{}, fakeHostRegistry{}, fakeHostMetrics{}, fakeHostHistory{}, fakeWorkflowService{}, fakePlacementAdvisor{}, fakeProfileReader{}, placement.Thresholds{}, fakeNodeSettingsStore{data: map[string]json.RawMessage{}}, fakeBackupSvc{}, fakeSupervisorClient{}, fakeClusterService{}, nil, nil, nil, nil, nil, nil)

	body := `{"groups":[{"id":"pt","label":"Programmton","channels":2}],"presets":[{"id":"stereo","label":"Stereo","routes":[{"srcTrack":1,"group":"does-not-exist","groupChannel":0}]}]}`
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, httptest.NewRequest(http.MethodPut, "/api/v1/node-types/omp-mxf-player/settings", strings.NewReader(body)))
...
// TestHandlePutMxfPlayerSettingsRejectsOutOfRangeChannel prüft, dass
// eine groupChannel außerhalb der Kanalzahl ihrer Gruppe abgelehnt
// wird (sonst würde presets::matrix_for still eine Route verwerfen,
// statt dem Admin sofort zu sagen, dass die Eingabe unsinnig ist).
func TestHandlePutMxfPlayerSettingsRejectsOutOfRangeChannel(t *testing.T) {
	h := NewHandler(config.Config{UIDir: t.TempDir()}, fakeNodeLister{}, fakeEventSubscriber{ch: make(chan sse.Event)}, &fakeGraphService{}, fakeLayoutStore{}, fakeSnapshotService{}, fakeLauncherService{}, fakeConsoleResolver{}, nil, fakeAuthSvc{}, fakeAuthzSvc{}, &fakeAuditSvc{}, &fakeAuditSvc{}, fakeHostRegistry{}, fakeHostMetrics{}, fakeHostHistory{}, fakeWorkflowService{}, fakePlacementAdvisor{}, fakeProfileReader{}, placement.Thresholds{}, fakeNodeSettingsStore{data: map[string]json.RawMessage{}}, fakeBackupSvc{}, fakeSupervisorClient{}, fakeClusterService{}, nil, nil, nil, nil, nil, nil)

	body := `{"groups":[{"id":"pt","label":"Programmton","channels":2}],"presets":[{"id":"stereo","label":"Stereo","routes":[{"srcTrack":1,"group":"pt","groupChannel":5}]}]}`
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, httptest.NewRequest(http.MethodPut, "/api/v1/node-types/omp-mxf-player/settings", strings.NewReader(body)))
...
// TestHandlePutMxfPlayerSettingsRejectsEmptyGroups prüft, dass ein
// Dokument ohne jede Gruppe (die Instanz hätte dann keinen einzigen
// Sender mehr) abgelehnt wird statt gespeichert zu werden.
func TestHandlePutMxfPlayerSettingsRejectsEmptyGroups(t *testing.T) {
	h := NewHandler(config.Config{UIDir: t.TempDir()}, fakeNodeLister{}, fakeEventSubscriber{ch: make(chan sse.Event)}, &fakeGraphService{}, fakeLayoutStore{}, fakeSnapshotService{}, fakeLauncherService{}, fakeConsoleResolver{}, nil, fakeAuthSvc{}, fakeAuthzSvc{}, &fakeAuditSvc{}, &fakeAuditSvc{}, fakeHostRegistry{}, fakeHostMetrics{}, fakeHostHistory{}, fakeWorkflowService{}, fakePlacementAdvisor{}, fakeProfileReader{}, placement.Thresholds{}, fakeNodeSettingsStore{data: map[string]json.RawMessage{}}, fakeBackupSvc{}, fakeSupervisorClient{}, fakeClusterService{}, nil, nil, nil, nil, nil, nil)
...
func TestHandleListInstancesMergesRemoteInstanceMetrics(t *testing.T) {
	...
	h := NewHandler(config.Config{UIDir: t.TempDir()}, fakeNodeLister{}, fakeEventSubscriber{ch: make(chan sse.Event)}, &fakeGraphService{}, fakeLayoutStore{}, fakeSnapshotService{}, svc, fakeConsoleResolver{}, nil, fakeAuthSvc{}, fakeAuthzSvc{}, &fakeAuditSvc{}, &fakeAuditSvc{}, fakeHostRegistry{}, metrics, fakeHostHistory{}, fakeWorkflowService{}, fakePlacementAdvisor{}, fakeProfileReader{}, placement.Thresholds{}, fakeNodeSettingsStore{}, fakeBackupSvc{}, fakeSupervisorClient{}, fakeClusterService{}, nil, nil, nil, nil, nil, nil)
...
func TestHandlePostInstanceWithRequiredIOPortClaimsAPort(t *testing.T) {
	...
	h := NewHandler(config.Config{UIDir: t.TempDir()}, fakeNodeLister{}, fakeEventSubscriber{ch: make(chan sse.Event)}, &fakeGraphService{}, fakeLayoutStore{}, fakeSnapshotService{}, svc, fakeConsoleResolver{}, nil, fakeAuthSvc{}, fakeAuthzSvc{}, &fakeAuditSvc{}, &fakeAuditSvc{}, fakeHostRegistry{}, fakeHostMetrics{}, fakeHostHistory{}, fakeWorkflowService{}, fakePlacementAdvisor{}, fakeProfileReader{}, placement.Thresholds{}, fakeNodeSettingsStore{}, fakeBackupSvc{}, fakeSupervisorClient{}, fakeClusterService{}, ioPorts, nil, nil, nil, nil, nil)
...
func TestHandleMeConsoles(t *testing.T) {
	...
	h := NewHandler(config.Config{UIDir: t.TempDir()}, fakeNodeLister{}, fakeEventSubscriber{ch: make(chan sse.Event)}, &fakeGraphService{}, fakeLayoutStore{}, fakeSnapshotService{}, fakeLauncherService{}, resolver, nil, fakeAuthSvc{}, fakeAuthzSvc{}, &fakeAuditSvc{}, &fakeAuditSvc{}, fakeHostRegistry{}, fakeHostMetrics{}, fakeHostHistory{}, fakeWorkflowService{}, fakePlacementAdvisor{}, fakeProfileReader{}, placement.Thresholds{}, fakeNodeSettingsStore{}, fakeBackupSvc{}, fakeSupervisorClient{}, fakeClusterService{}, nil, nil, nil, nil, nil, nil)
