    ...
    loop {
        ...
        match commands_rx.recv_timeout(Duration::from_millis(500)) {
            Ok(Command::Connect(source_flow_id)) => {
                ...
                match build(&context, &source_flow_id, &config, flowed.clone(), output_delay.clone()) {
                    ...
                    Err(e) => {
                        ...
                        retry_target = Some(source_flow_id);
                    }
                }
            }
            ...
            Err(std::sync::mpsc::RecvTimeoutError::Timeout) => {
                if active.is_none()
                    && let Some(source_flow_id) = retry_target.clone()
                    && let Ok(p) = build(&context, &source_flow_id, &config, flowed.clone(), output_delay.clone())
                {
                    active = Some(p);
                    retry_target = None;
                }
