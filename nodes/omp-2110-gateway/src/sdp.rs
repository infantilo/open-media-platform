...
/// Audio-Pendant zu `parse_video_sdp` (Kapitel 19 Teil 4/D21 — Audio-
/// Ingest/-Output für `omp-2110-gateway`, bisher video-only, s.
/// `pipeline.rs`-Moduldoku). Exakt nach dem Vorbild von
/// `omp-aes67-gateway/src/sdp.rs::parse_audio_sdp` (bewusst dupliziert
/// statt geteilt, gleiche Begründung wie bei `is_multicast` in
/// `main.rs`: jeder Gateway-Node ist ein eigenständiges Binary) — beide
/// Nodes senden/erwarten dasselbe ST2110-30/AES67-`a=rtpmap:96 L24/
/// <rate>/<channels>`-Feldformat.
#[derive(Debug, Clone, PartialEq)]
...
mod tests {
    ...
    #[test]
    fn parses_own_generated_audio_sdp() {
        let sdp = "v=0\r\n\
        ...
        assert_eq!(
            parsed,
            ParsedAudioSdp { host: "127.0.0.1".to_string(), port: 6100, sample_rate: 48000, channels: 2 }
        );
    }

    #[test]
    fn strips_multicast_ttl_suffix_audio() {
    ...
    #[test]
    fn missing_field_reports_which_one_audio() {
        let sdp = "v=0\r\nc=IN IP4 127.0.0.1\r\n";
