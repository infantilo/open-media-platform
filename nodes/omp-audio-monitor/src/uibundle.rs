//! `/ui/manifest.json` + `/ui/bundle.js` des Audio-Monitors — Rust-
//! Pendant zu `omp-viewer/src/uibundle.rs` (`<audio>` statt `<img>`).
...
pub fn route(method: &str, path: &str) -> Option<RawResponse> {
    ...
    // s. omp-audio-mixer/src/uibundle.rs für die Herleitung: der
    // Orchestrator hängt `?access_token=` an, dieser Schnitt macht den
    // exakten Pfadvergleich unten davon unabhängig.
